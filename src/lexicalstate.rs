...
impl LexicalState {
    pub unsafe fn create_label(
        &mut self, interpreter: *mut Interpreter, function_state: *mut FunctionState, name: *mut TString, line: i32, is_last: bool,
    ) -> bool {
        unsafe {
            ...
            if is_last {
                (*((*labeldescriptions).vectort_pointer).offset(l as isize)).labeldescription_countactivevariables =
                    (*(*function_state).functionstate_blockcontrol).blockcontrol_countactivevariables;
            }
            if solvegotos(
                interpreter,
                self,
                function_state,
                &mut *((*labeldescriptions).vectort_pointer).offset(l as isize),
            ) {
                code_abck(
                    interpreter,
                    self,
    ...
    pub unsafe fn parse_expression(
        &mut self, interpreter: *mut Interpreter, function_state: *mut FunctionState,
    ...
    pub unsafe fn parse_expression_list(
        ...
        unsafe {
            ...
            while testnext(interpreter, self, function_state, Character::Comma as i32) != 0 {
                luak_exp2nextreg(interpreter, self, self.lexicalstate_functionstate, expression_description);
                self.parse_expression(interpreter, function_state, expression_description);
                count += 1;
            }
            return count;
    ...
    pub unsafe fn add_prototype(&mut self, interpreter: *mut Interpreter) -> *mut Prototype {
        unsafe {
            ...
            if (*function_state).functionstate_countprototypes >= (*prototype).prototype_prototypes.get_size() as i32 {
                let mut oldsize = (*prototype).prototype_prototypes.get_size();
                (*prototype).prototype_prototypes.grow(
                    interpreter,
                    (*function_state).functionstate_countprototypes as usize,
            ...
            let np = (*function_state).functionstate_countprototypes;
            (*function_state).functionstate_countprototypes = (*function_state).functionstate_countprototypes + 1;
            ...
            *target = clp;
            if (*prototype).get_marked() & 1 << 5 != 0 && (*clp).get_marked() & (1 << 3 | 1 << 4) != 0 {
                Object::luac_barrier_(interpreter, &mut (*(prototype as *mut Object)), &mut (*(clp as *mut Object)));
            } else {
            };
            return clp;
        }
...
pub unsafe fn find_label(
    ...
    unsafe {
        for i in (*function_state).functionstate_firstlabel..(*dynamic_data).dynamicdata_labels.get_length() as i32 {
            let candidate = &mut *((*dynamic_data).dynamicdata_labels.vectort_pointer).offset(i as isize);
...
pub unsafe fn newlabelentry(
    ...
    l: *mut VectorT<LabelDescription>, name: *mut TString, line: i32, program_counter: i32,
) -> i32 {
    unsafe {
        let n = (*l).get_length();
        (*l).grow(
        ...
        let ref mut fresh44 = (*((*l).vectort_pointer).offset(n as isize)).labeldescription_name;
        *fresh44 = name;
        (*((*l).vectort_pointer).offset(n as isize)).labeldescription_line = line;
        (*((*l).vectort_pointer).offset(n as isize)).labeldescription_countactivevariables =
            (*function_state).functionstate_countactivevariables;
        (*((*l).vectort_pointer).offset(n as isize)).labeldescription_close = 0;
        (*((*l).vectort_pointer).offset(n as isize)).labeldescription_programcounter = program_counter;
...
pub unsafe fn solvegotos(
    ...
    unsafe {
        ...
        let mut i: i32 = (*(*function_state).functionstate_blockcontrol).blockcontrol_firstgoto;
        let mut needsclose = false;
        while i < (*gl).get_length() as i32 {
            if (*((*gl).vectort_pointer).offset(i as isize)).labeldescription_name == (*label_description).labeldescription_name {
                needsclose = needsclose || (0 != (*((*gl).vectort_pointer).offset(i as isize)).labeldescription_close);
...
pub unsafe fn undefgoto(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, _function_state: *mut FunctionState,
    goto_label_description: *mut LabelDescription,
) -> ! {
    unsafe {
        let mut message: *const i8;
        if (*goto_label_description).labeldescription_name
            == luas_newlstr(interpreter, c"break".as_ptr(), (size_of::<[i8; 6]>()).wrapping_sub(1))
        {
            message = c"break outside loop at line %d".as_ptr();
            message = luao_pushfstring(interpreter, message, (*goto_label_description).labeldescription_line);
        } else {
    ...
    unsafe {
        ...
        init_exp(
            ...
            luak_codeabx(
                ...
                OPCODE_CLOSURE,
                0,
                ((*function_state).functionstate_countprototypes - 1) as u32,
            ),
        );
...
pub unsafe fn open_function(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState,
    previous: *mut FunctionState, block_control: *mut BlockControl,
) {
    unsafe {
        let prototype: *mut Prototype = (*function_state).functionstate_prototype;
        (*function_state).functionstate_previous = previous;
        (*lexical_state).lexicalstate_functionstate = function_state;
        (*function_state).functionstate_programcounter = 0;
        (*function_state).functionstate_previousline = (*prototype).prototype_linedefined;
        (*function_state).functionstate_iwthabs = 0;
        (*function_state).functionstate_lasttarget = 0;
        (*function_state).functionstate_freereg = 0;
        (*function_state).functionstate_countconstants = 0;
        (*function_state).functionstate_countabslineinfo = 0;
        (*function_state).functionstate_countprototypes = 0;
        (*function_state).functionstate_countupvalues = 0;
        (*function_state).functionstate_countdebugvariables = 0 as i16;
        (*function_state).functionstate_countactivevariables = 0;
        (*function_state).functionstate_needsclose = false;
        (*function_state).functionstate_firstlocal = (*(*lexical_state).lexicalstate_dynamicdata)
            .dynamicdata_activevariables
            .get_length() as i32;
        (*function_state).functionstate_firstlabel =
        ...
        BlockControl::enterblock(lexical_state, function_state, block_control, false);
    }
}
pub unsafe fn close_function(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState) {
    unsafe {
        ...
        (*prototype)
            .prototype_code
            .shrink(&mut *interpreter, (*function_state).functionstate_programcounter as usize);
        (*prototype)
            .prototype_lineinfo
            .shrink(&mut *interpreter, (*function_state).functionstate_programcounter as usize);
        (*prototype)
            .prototype_absolutelineinfo
            .shrink(&mut *interpreter, (*function_state).functionstate_countabslineinfo as usize);
        (*prototype)
            .prototype_constants
            .shrink(&mut *interpreter, (*function_state).functionstate_countconstants as usize);
        (*prototype)
            .prototype_prototypes
            .shrink(&mut *interpreter, (*function_state).functionstate_countprototypes as usize);
        (*prototype)
            .prototype_localvariables
            .shrink(&mut *interpreter, (*function_state).functionstate_countdebugvariables as usize);
        (*prototype)
            .prototype_upvalues
            .shrink(&mut *interpreter, (*function_state).functionstate_countupvalues as usize);
        (*lexical_state).lexicalstate_functionstate = (*function_state).functionstate_previous;
        (*interpreter).do_gc_step_if_should_step();
    }
}
pub unsafe fn parse_statement_list(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState,
) {
    unsafe {
        const TK_RETURN: i32 = 273;
...
pub unsafe fn yindex(
    ...
    unsafe {
        ...
        code_expression_to_value(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate, v);
...
pub unsafe fn recfield(
    ...
    unsafe {
        let reg: i32 = (*(*lexical_state).lexicalstate_functionstate).functionstate_freereg as i32;
        let mut key: ExpressionDescription = ExpressionDescription::new();
        ...
        checklimit(
            ...
            lexical_state,
            function_state,
            (*constructor_control).constructorcontrol_counttable,
            0x7FFFFFFF as i32,
...
pub unsafe fn listfield(
    ...
    unsafe {
        ...
        (*constructor_control).constructorcontrol_counttostore += 1;
...
pub unsafe fn constructor(
    ...
    unsafe {
        ...
        let program_counter: i32 = code_abck(interpreter, lexical_state, function_state, OPCODE_NEWTABLE, 0, 0, 0, 0);
        let mut constructor_control: ConstructorControl = ConstructorControl::new();
        luak_code(interpreter, lexical_state, function_state, 0);
        ...
        while !((*lexical_state).lexicalstate_token.token == Character::BraceRight as i32) {
            ...
            if !(testnext(interpreter, lexical_state, function_state, Character::Comma as i32) != 0
                || testnext(interpreter, lexical_state, function_state, Character::Semicolon as i32) != 0)
            {
                break;
            }
        }
...
pub unsafe fn parlist(interpreter: *mut Interpreter, lexical_state: *mut LexicalState) {
    unsafe {
        ...
        if (*lexical_state).lexicalstate_token.token != Character::ParenthesisRight as i32 {
            loop {
                match (*lexical_state).lexicalstate_token.token {
                    ...
                    | _ => {
                        luax_syntaxerror(interpreter, lexical_state, c"<name> or '...' expected".as_ptr());
                    },
                }
        ...
        (*prototype).prototype_countparameters = (*function_state).functionstate_countactivevariables;
...
pub unsafe fn body(
    ...
    unsafe {
        ...
        new_fs.functionstate_prototype = (*lexical_state).add_prototype(interpreter);
        ...
        if is_method {
            new_localvar(
                ...
                luax_newstring(
                    ...
                    lexical_state,
                    c"self".as_ptr(),
                    (size_of::<[i8; 5]>() as usize).wrapping_sub(1 as usize),
                ),
            );
        ...
        close_function(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate);
    }
}
pub unsafe fn funcargs(
    ...
    unsafe {
        ...
        match (*lexical_state).lexicalstate_token.token {
            ...
            | _ => {
                luax_syntaxerror(interpreter, lexical_state, c"function arguments expected".as_ptr());
            },
        }
    ...
    unsafe {
        ...
        match (*lexical_state).lexicalstate_token.token {
            | TK_CHARACTER_PARENTHESIS_LEFT => {
                ...
                luak_dischargevars(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate, v);
                return;
            },
            | TK_NAME => {
            ...
            | _ => {
                luax_syntaxerror(interpreter, lexical_state, c"unexpected symbol".as_ptr());
            },
        };
    }
}
...
pub unsafe fn simpleexp(
    ...
    unsafe {
        const TK_CHARACTER_BRACE_LEFT: i32 = Character::BraceLeft as i32;
        const TK_INTEGER: i32 = 290;
        const TK_FLOAT: i32 = 289;
        const TK_STRING: i32 = 292;
        const TK_DOTS: i32 = 280;
        const TK_FUNCTION: i32 = 264;
        const TK_FALSE: i32 = 262;
        const TK_TRUE: i32 = 275;
...
pub unsafe fn error_expected(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, _function_state: *mut FunctionState, token: i32,
) -> ! {
    unsafe {
        luax_syntaxerror(
            interpreter,
            ...
            luao_pushfstring(
                interpreter,
                c"%s expected".as_ptr(),
                luax_token2str(interpreter, lexical_state, token),
            ),
        );
    }
}
pub unsafe fn testnext(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, _function_state: *mut FunctionState, c: i32,
) -> i32 {
    unsafe {
...
pub unsafe fn checknext(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, c: i32,
) {
    unsafe {
...
pub unsafe fn check_match(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, what: i32, who: i32,
    where_0: i32,
) {
    unsafe {
        if testnext(interpreter, lexical_state, function_state, what) == 0 {
            if where_0 == (*lexical_state).lexicalstate_linenumber {
...
pub unsafe fn registerlocalvar(
    ...
    unsafe {
        ...
        (*prototype).prototype_localvariables.grow(
            interpreter,
            (*function_state).functionstate_countdebugvariables as usize,
        ...
        while oldsize < (*prototype).prototype_localvariables.get_size() {
            ...
            oldsize = oldsize + 1;
            let ref mut fresh34 =
                (*((*prototype).prototype_localvariables.vectort_pointer).offset(fresh33 as isize)).localvariable_variablename;
            *fresh34 = null_mut();
        }
        let ref mut fresh35 = (*((*prototype).prototype_localvariables.vectort_pointer)
            .offset((*function_state).functionstate_countdebugvariables as isize))
        .localvariable_variablename;
        *fresh35 = variable_name;
        (*((*prototype).prototype_localvariables.vectort_pointer)
            .offset((*function_state).functionstate_countdebugvariables as isize))
        .localvariable_startprogramcounter = (*function_state).functionstate_programcounter;
        ...
        let fresh36 = (*function_state).functionstate_countdebugvariables;
        (*function_state).functionstate_countdebugvariables = (*function_state).functionstate_countdebugvariables + 1;
...
pub unsafe fn new_localvar(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, name: *mut TString) -> i32 {
    unsafe {
        ...
        checklimit(
            ...
            lexical_state,
            function_state,
            (*dynamic_data).dynamicdata_activevariables.get_length() as i32 + 1 - (*function_state).functionstate_firstlocal,
            200,
            c"local variables".as_ptr(),
        );
        (*dynamic_data).dynamicdata_activevariables.grow(
            interpreter,
            ((*dynamic_data).dynamicdata_activevariables.get_length() + 1) as usize,
        ...
        let fresh37 = (*dynamic_data).dynamicdata_activevariables.get_length();
        (*dynamic_data)
            .dynamicdata_activevariables
            .set_length(((*dynamic_data).dynamicdata_activevariables.get_length() + 1) as usize);
        var = &mut *((*dynamic_data).dynamicdata_activevariables.vectort_pointer).offset(fresh37 as isize)
        ...
        return (*dynamic_data).dynamicdata_activevariables.get_length() as i32 - 1 - (*function_state).functionstate_firstlocal;
    }
}
pub unsafe fn check_readonly(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, expression_description: *mut ExpressionDescription,
) {
    unsafe {
        let function_state: *mut FunctionState = (*lexical_state).lexicalstate_functionstate;
...
pub unsafe fn adjustlocalvars(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, count_variables: i32) {
    unsafe {
        ...
        for _ in 0..count_variables {
            let fresh39 = (*function_state).functionstate_countactivevariables;
            (*function_state).functionstate_countactivevariables =
                ((*function_state).functionstate_countactivevariables).wrapping_add(1);
            let vidx = fresh39 as i32;
            let var = getlocalvardesc(lexical_state, function_state, vidx);
            let fresh40 = reglevel_0;
            reglevel_0 += 1;
            (*var).variabledescription_content.variabledescriptioncontent_registerindex = fresh40 as u8;
...
pub unsafe fn singlevar(
    ...
    unsafe {
        ...
        if (*var).expressiondescription_expressionkind == ExpressionKind::Void {
            let mut key: ExpressionDescription = ExpressionDescription::new();
            singlevaraux(
                interpreter,
                lexical_state,
...
pub unsafe fn adjust_assign(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, count_variables: i32, count_expressions: i32,
    expression_description: *mut ExpressionDescription,
) {
    unsafe {
        let function_state: *mut FunctionState = (*lexical_state).lexicalstate_functionstate;
        ...
        if needed > 0 {
            ...
            (*function_state).functionstate_freereg = ((*function_state).functionstate_freereg as i32 + needed) as u8;
        };
    }
}
pub unsafe fn jumpscopeerror(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, goto_label_description: *mut LabelDescription,
) -> ! {
    unsafe {
...
pub unsafe fn solvegoto(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, goto_offset: i32, label_description: *mut LabelDescription,
) {
    unsafe {
        ...
        if ((((*goto_label_description).labeldescription_countactivevariables as i32)
            < (*label_description).labeldescription_countactivevariables as i32) as i32
            != 0) as i64
            != 0
        {
...
pub unsafe fn subexpr(
    ...
    unsafe {
        ...
        while op as u32 != OperatorBinary::NoBinaryOperation as u32 && PRIORITY[op as usize].priority_left as i32 > limit {
            ...
            let line_0: i32 = (*lexical_state).lexicalstate_linenumber;
            luax_next(interpreter, lexical_state);
            luak_infix(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate, op, v);
            let nextop = subexpr(
                interpreter, lexical_state, function_state, &mut v2, PRIORITY[op as usize].priority_right as i32,
            );
            luak_posfix(
                interpreter,
                lexical_state,
...
pub unsafe fn handle_block(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState) {
    unsafe {
        let mut block_control = BlockControl::new();
        BlockControl::enterblock(lexical_state, function_state, &mut block_control, false);
        parse_statement_list(interpreter, lexical_state, function_state);
        BlockControl::leaveblock(interpreter, lexical_state, function_state);
    }
}
pub unsafe fn check_conflict(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, mut lhs_assign: *mut ExpressionDescription,
    v: *mut ExpressionDescription,
) {
    unsafe {
        let function_state: *mut FunctionState = (*lexical_state).lexicalstate_functionstate;
        let extra: i32 = (*function_state).functionstate_freereg as i32;
...
pub unsafe fn restassign(
    ...
    unsafe {
        ...
        if !((*lhs_assign).expressiondescription_expressionkind.is_index_plus()) {
            luax_syntaxerror(interpreter, lexical_state, c"syntax error".as_ptr());
        }
        ...
        if testnext(interpreter, lexical_state, function_state, Character::Comma as i32) != 0 {
            ...
            if count_expressions != count_variables {
                adjust_assign(
                    interpreter, lexical_state, count_variables, count_expressions, &mut expression_description,
                );
            } else {
...
pub unsafe fn cond(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState) -> i32 {
    unsafe {
        ...
        return v.expressiondescription_f;
    }
}
    ...
    unsafe {
        ...
        if label_description.is_null() {
            ...
            if luay_nvarstack(lexical_state, function_state) > level {
                code_abck(interpreter, lexical_state, function_state, OPCODE_CLOSE, level, 0, 0, 0);
            }
            luak_patchlist(
                interpreter,
                lexical_state,
...
pub unsafe fn checkrepeated(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, name: *mut TString) {
    unsafe {
        ...
        if !lb.is_null() {
            ...
            message = luao_pushfstring(interpreter, message, (*name).get_contents_mut(), (*lb).labeldescription_line);
            luak_semerror(interpreter, lexical_state, message);
        }
    }
}
pub unsafe fn handle_label_statement(
    ...
    unsafe {
        checknext(interpreter, lexical_state, function_state, Token::Dbcolon as i32);
...
pub unsafe fn handle_while_statement(
    ...
    unsafe {
        ...
        BlockControl::enterblock(lexical_state, function_state, &mut block_control, true);
...
pub unsafe fn handle_repeat_statement(
    ...
    unsafe {
        ...
        BlockControl::enterblock(lexical_state, function_state, &mut bl1, true);
        BlockControl::enterblock(lexical_state, function_state, &mut bl2, false);
        luax_next(interpreter, lexical_state);
        parse_statement_list(interpreter, lexical_state, function_state);
        check_match(
            interpreter,
        ...
        BlockControl::leaveblock(interpreter, lexical_state, function_state);
        if bl2.blockcontrol_countupvalues != 0 {
            ...
            code_abck(
                ...
                function_state,
                OPCODE_CLOSE,
                reglevel(lexical_state, function_state, bl2.blockcontrol_countactivevariables as i32),
                0,
                0,
                0,
            );
...
pub unsafe fn handle_forbody(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, base: i32, line: i32,
    count_variables: i32, isgen: i32,
) {
    unsafe {
        static mut FOR_PREP: [u32; 2] = [OPCODE_FORPREP, OPCODE_TFORPREP];
        static mut FOR_LOOP: [u32; 2] = [OPCODE_FORLOOP, OPCODE_TFORLOOP];
        let mut block_control = BlockControl::new();
        checknext(interpreter, lexical_state, function_state, Token::Do as i32);
        ...
        if isgen != 0 {
            code_abck(
                interpreter, lexical_state, function_state, OPCODE_TFORCALL, base, 0, count_variables, 0,
            );
            luak_fixline(interpreter, lexical_state, function_state, line);
        }
...
pub unsafe fn handle_for_numeric(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState,
    variable_name: *mut TString, line: i32,
) {
    unsafe {
        let base: i32 = (*function_state).functionstate_freereg as i32;
        let s = c"(for interpreter)".as_ptr();
        new_localvar(
            interpreter,
            lexical_state,
            luax_newstring(interpreter, lexical_state, s, libc::strlen(s) as usize),
        );
        new_localvar(
            interpreter,
            lexical_state,
...
pub unsafe fn handle_for_list(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, indexname: *mut TString,
) {
    unsafe {
        let mut expression_description: ExpressionDescription = ExpressionDescription::new();
        ...
        checknext(interpreter, lexical_state, function_state, Token::In as i32);
        let line: i32 = (*lexical_state).lexicalstate_linenumber;
        adjust_assign(
            interpreter,
            lexical_state,
...
pub unsafe fn handle_for_statement(
    ...
    unsafe {
        ...
        match (*lexical_state).lexicalstate_token.token {
            ...
            | _ => {
                luax_syntaxerror(interpreter, lexical_state, c"Character::Equal or 'in' expected".as_ptr());
            },
        }
        check_match(
            interpreter,
...
pub unsafe fn handle_test_then_block(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, escapelist: *mut i32,
) {
    unsafe {
        let mut block_control = BlockControl::new();
        let mut v: ExpressionDescription = ExpressionDescription::new();
        ...
        luax_next(interpreter, lexical_state);
        (*lexical_state).parse_expression(interpreter, function_state, &mut v);
        checknext(interpreter, lexical_state, function_state, Token::Then as i32);
...
pub unsafe fn handle_if_statement(
    ...
    unsafe {
        ...
        if testnext(interpreter, lexical_state, function_state, Token::Else as i32) != 0 {
            handle_block(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate);
        }
        check_match(
            interpreter,
...
pub unsafe fn handle_local_function(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState,
) {
    unsafe {
        let mut expression_description: ExpressionDescription = ExpressionDescription::new();
        let fvar: i32 = (*function_state).functionstate_countactivevariables as i32;
        new_localvar(
            interpreter,
            lexical_state,
            str_checkname(interpreter, lexical_state, function_state),
...
pub unsafe fn getlocalattribute(
    ...
    unsafe {
        if testnext(interpreter, lexical_state, function_state, Character::AngleLeft as i32) != 0 {
            ...
            if libc::strcmp(attr, c"const".as_ptr()) == 0 {
                return 1;
            } else if libc::strcmp(attr, c"close".as_ptr()) == 0 {
                return 2;
            } else {
                luak_semerror(
                    interpreter,
                    lexical_state,
                    luao_pushfstring(interpreter, c"unknown attribute '%s'".as_ptr(), attr),
                );
            }
        }
        return 0;
...
pub unsafe fn handle_local_statement(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState,
) {
    unsafe {
        ...
        loop {
            ...
            if kind == 2 {
                ...
                toclose = (*function_state).functionstate_countactivevariables as i32 + count_variables;
            }
            count_variables += 1;
            if !(testnext(interpreter, lexical_state, function_state, Character::Comma as i32) != 0) {
                break;
        ...
        {
            (*var).variabledescription_content.variabledescriptioncontent_kind = 3 as u8;
...
pub unsafe fn handle_expression_statement(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState,
) {
    unsafe {
            ...
            || (*lexical_state).lexicalstate_token.token == Character::Comma as i32
        {
            ...
            let inst: *mut u32 = &mut *((*(*function_state).functionstate_prototype).prototype_code.vectort_pointer)
                .offset(new_lhs_assign.expressiondescription_value.value_info as isize)
                as *mut u32;
            *inst = *inst & !(!(0xFFFFFFFFu32 << 8) << POSITION_C) | (1 as u32) << POSITION_C & !(0xFFFFFFFFu32 << 8) << POSITION_C;
        };
    }
}
pub unsafe fn handle_return_statement(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState,
) {
    unsafe {
        let mut expression_description: ExpressionDescription = ExpressionDescription::new();
            ...
            || (*lexical_state).lexicalstate_token.token == Character::Semicolon as i32
        {
                ...
                || expression_description.expressiondescription_expressionkind == ExpressionKind::VariableArguments
            {
                ...
                if expression_description.expressiondescription_expressionkind == ExpressionKind::Call
                    && nret == 1
                    && !(*(*function_state).functionstate_blockcontrol).blockcontrol_isinsidetbc
                {
                    *((*(*function_state).functionstate_prototype).prototype_code.vectort_pointer)
                        ...
                        *((*(*function_state).functionstate_prototype).prototype_code.vectort_pointer)
                            .offset(expression_description.expressiondescription_value.value_info as isize)
                            & !(!(0xFFFFFFFFu32 << 7) << 0)
                            | (OPCODE_TAILCALL as u32) << 0 & !(0xFFFFFFFFu32 << 7) << 0;
                }
                nret = -1;
            } else if nret == 1 {
...
pub unsafe fn handle_main_function(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState,
) {
    unsafe {
        let mut block_control = BlockControl::new();
        ...
        if (*(*function_state).functionstate_prototype).get_marked() & 1 << 5 != 0
            && (*(*env).upvaluedescription_name).get_marked() & (1 << 3 | 1 << 4) != 0
        {
            Object::luac_barrier_(
                interpreter,
                &mut (*((*function_state).functionstate_prototype as *mut Object)),
                &mut (*((*env).upvaluedescription_name as *mut Object)),
            );
        } else {
        };
        luax_next(interpreter, lexical_state);
        parse_statement_list(interpreter, lexical_state, function_state);
        check(interpreter, lexical_state, function_state, Token::EndOfStream as i32);
        close_function(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate);
    }
}
pub unsafe fn save(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, c: i32) {
    unsafe {
        ...
        if ((*b).buffer_loads.get_length() as usize).wrapping_add(1 as usize) > (*b).buffer_loads.get_size() as usize {
            ...
            (*b).buffer_loads.resize(interpreter, newsize as usize);
        }
        let fresh49 = (*b).buffer_loads.get_length();
        (*b).buffer_loads
            .set_length((((*b).buffer_loads.get_length()).wrapping_add(1)) as usize);
        *((*b).buffer_loads.loads_pointer).offset(fresh49 as isize) = c as i8;
    }
}
    ...
    unsafe {
        if token < 127 as i32 * 2 + 1 + 1 {
            if Character::from(token).is_printable() {
                return luao_pushfstring(interpreter, c"'%c'".as_ptr(), token);
            } else {
                return luao_pushfstring(interpreter, c"'<\\%d>'".as_ptr(), token);
            }
        } else {
            let s: *const i8 = TOKENS[(token - (127 as i32 * 2 + 1 + 1)) as usize];
            if token < Token::EndOfStream as i32 {
                return luao_pushfstring(interpreter, c"'%s'".as_ptr(), s);
            } else {
                return s;
            }
        };
    ...
    unsafe {
        ...
        if token != 0 {
            luao_pushfstring(
                ...
                c"%s near %s".as_ptr(),
                message,
                text_token(interpreter, lexical_state, token),
            );
        }
...
pub unsafe fn luax_syntaxerror(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, message: *const i8) -> ! {
    unsafe {
        lexerror(interpreter, lexical_state, message, (*lexical_state).lexicalstate_token.token);
    }
}
pub unsafe fn luax_newstring(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, str: *const i8, length: usize,
) -> *mut TString {
    unsafe {
        ...
        if (*tvalue).get_tagvariant().to_tag_type().is_nil() {
            let fresh50 = (*interpreter).interpreter_top.stkidrel_pointer;
            ...
            luah_finishset(interpreter, (*lexical_state).lexicalstate_table, stv, tvalue, stv);
            (*interpreter).do_gc_step_if_should_step();
            (*interpreter).interpreter_top.stkidrel_pointer = (*interpreter).interpreter_top.stkidrel_pointer.offset(-1);
        } else {
            tstring = &mut (*((*(tvalue as *mut Node)).node_key.tvalue_value.value_object as *mut TString));
        }
        return tstring;
    }
}
pub unsafe fn inclinenumber(interpreter: *mut Interpreter, lexical_state: *mut LexicalState) {
    unsafe {
        ...
        if (*lexical_state).lexicalstate_linenumber >= 0x7FFFFFFF as i32 {
            lexerror(interpreter, lexical_state, c"chunk has too many lines".as_ptr(), 0);
        }
    }
}
pub unsafe fn luax_setinput(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, zio: *mut ZIO, source: *mut TString, firstchar: i32,
) {
    unsafe {
        ...
        (*lexical_state).lexicalstate_environment =
            luas_newlstr(interpreter, c"_ENV".as_ptr(), (size_of::<[i8; 5]>()).wrapping_sub(1));
        (*(*lexical_state).lexicalstate_buffer)
            .buffer_loads
            .resize(interpreter, 32 as usize);
    }
}
...
pub unsafe fn check_next2(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, set: *const i8) -> bool {
    unsafe {
        if (*lexical_state).lexicalstate_current == *set.offset(0 as isize) as i32
            || (*lexical_state).lexicalstate_current == *set.offset(1 as isize) as i32
        {
...
pub unsafe fn read_numeral(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, semantic_info: *mut Value) -> i32 {
    unsafe {
        ...
        if obj.get_tagvariant() == TagVariant::NumericInteger {
            (*semantic_info).value_integer = obj.tvalue_value.value_integer;
            return Token::Integer as i32;
        } else {
            (*semantic_info).value_number = obj.tvalue_value.value_number;
            return Token::Float as i32;
        };
    }
}
pub unsafe fn skip_sep(interpreter: *mut Interpreter, lexical_state: *mut LexicalState) -> usize {
    unsafe {
        ...
        return if (*lexical_state).lexicalstate_current == s {
            count.wrapping_add(2 as usize)
        } else {
...
pub unsafe fn read_long_string(
    interpreter: *mut Interpreter, lexical_state: *mut LexicalState, semantic_info: *mut Value, sep: usize,
) {
    unsafe {
        let line: i32 = (*lexical_state).lexicalstate_linenumber;
        save(interpreter, lexical_state, (*lexical_state).lexicalstate_current);
            ...
            match Character::from_negative((*lexical_state).lexicalstate_current as i32) {
                | None => {
                    ...
                    let message: *const i8 =
                        luao_pushfstring(interpreter, c"unfinished long %s (starting at line %d)".as_ptr(), what, line);
                    lexerror(interpreter, lexical_state, message, Token::EndOfStream as i32);
                },
        ...
        if !semantic_info.is_null() {
            (*semantic_info).value_tstring = luax_newstring(
                interpreter,
                lexical_state,
                ((*(*lexical_state).lexicalstate_buffer).buffer_loads.loads_pointer).offset(sep as isize),
                ((*(*lexical_state).lexicalstate_buffer).buffer_loads.get_length() as usize)
                    .wrapping_sub((2 as usize).wrapping_mul(sep as usize)) as usize,
            );
        }
    }
}
...
pub unsafe fn gethexa(interpreter: *mut Interpreter, lexical_state: *mut LexicalState) -> i32 {
    unsafe {
        ...
        return Character::from((*lexical_state).lexicalstate_current).get_hexadecimal_digit_value() as i32;
    }
}
pub unsafe fn readhexaesc(interpreter: *mut Interpreter, lexical_state: *mut LexicalState) -> i32 {
    unsafe {
        ...
        (*(*lexical_state).lexicalstate_buffer)
            .buffer_loads
            .set_length(((*(*lexical_state).lexicalstate_buffer).buffer_loads.get_length()).wrapping_sub(2) as usize);
        return r;
    }
}
pub unsafe fn readutf8esc(interpreter: *mut Interpreter, lexical_state: *mut LexicalState) -> usize {
    unsafe {
        ...
        loop {
            ...
            r = (r << 4)
                .wrapping_add(Character::from((*lexical_state).lexicalstate_current).get_hexadecimal_digit_value() as usize);
        }
        esccheck(
            interpreter,
            lexical_state,
        ...
        (*(*lexical_state).lexicalstate_buffer)
            .buffer_loads
            .set_length(((*(*lexical_state).lexicalstate_buffer).buffer_loads.get_length() as usize).wrapping_sub(i as usize));
        return r;
    }
}
...
pub unsafe fn readdecesc(interpreter: *mut Interpreter, lexical_state: *mut LexicalState) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        let mut r: i32 = 0;
        while i < 3 && Character::from((*lexical_state).lexicalstate_current).is_digit_decimal() {
            r = 10 as i32 * r + (*lexical_state).lexicalstate_current - Character::Digit0 as i32;
            save(interpreter, lexical_state, (*lexical_state).lexicalstate_current);
            (*lexical_state).lexicalstate_current = (*(*lexical_state).lexicalstate_zio).get_char();
            i += 1;
        }
...
pub unsafe fn read_string(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, del: i32, semantic_info: *mut Value) {
    unsafe {
            ...
            match Character::from_negative((*lexical_state).lexicalstate_current) {
                | None => {
                    lexerror(
                        interpreter,
                        lexical_state,
                        c"unfinished string".as_ptr(),
                        Token::EndOfStream as i32,
                    );
                },
                | Some(Character::LineFeed) | Some(Character::CarriageReturn) => {
                    lexerror(interpreter, lexical_state, c"unfinished string".as_ptr(), Token::String as i32);
                },
                | Some(Character::Backslash) => {
                    let c: i32;
                    save(interpreter, lexical_state, (*lexical_state).lexicalstate_current);
                    (*lexical_state).lexicalstate_current = (*(*lexical_state).lexicalstate_zio).get_char();
                    match Character::from((*lexical_state).lexicalstate_current) {
                        | Character::LowerA => {
                        ...
                        | Character::LowerZ => {
                            (*(*lexical_state).lexicalstate_buffer).buffer_loads.set_length(
                                ((*(*lexical_state).lexicalstate_buffer).buffer_loads.get_length()).wrapping_sub(1) as usize,
                            );
                            (*lexical_state).lexicalstate_current = (*(*lexical_state).lexicalstate_zio).get_char();
                    ...
                    (*(*lexical_state).lexicalstate_buffer)
                        .buffer_loads
                        .set_length(((*(*lexical_state).lexicalstate_buffer).buffer_loads.get_length()).wrapping_sub(1) as usize);
        ...
        (*semantic_info).value_tstring = luax_newstring(
            interpreter,
            lexical_state,
            ((*(*lexical_state).lexicalstate_buffer).buffer_loads.loads_pointer).offset(1 as isize),
            ((*(*lexical_state).lexicalstate_buffer).buffer_loads.get_length()).wrapping_sub(2) as usize,
        );
    }
}
        ...
        loop {
            ...
            match Character::from_negative((*lexical_state).lexicalstate_current) {
                ...
                | Some(Character::Quote) | Some(Character::DoubleQuote) => {
                    read_string(interpreter, lexical_state, (*lexical_state).lexicalstate_current, semantic_info);
                    return Token::String as i32;
                },
...
pub unsafe fn luax_next(interpreter: *mut Interpreter, lexical_state: *mut LexicalState) {
    unsafe {
        ...
        if (*lexical_state).lexicalstate_lookahead.token != Token::EndOfStream as i32 {
            ...
            (*lexical_state).lexicalstate_token.token = llex(
                interpreter,
                lexical_state,
                &mut (*lexical_state).lexicalstate_token.semantic_info,
            );
        };
    }
}
pub unsafe fn luax_lookahead(interpreter: *mut Interpreter, lexical_state: *mut LexicalState) -> i32 {
    unsafe {
        (*lexical_state).lexicalstate_lookahead.token = llex(
            interpreter,
            lexical_state,
            &mut (*lexical_state).lexicalstate_lookahead.semantic_info,
        );
        return (*lexical_state).lexicalstate_lookahead.token;
    }
}
...
pub fn block_follow_without_until(token: i32) -> bool {
    const TK_ENDOFSTREAM: i32 = 288;
    const TK_END: i32 = 261;
    const TK_ELSEIF: i32 = 260;
    const TK_ELSE: i32 = 259;
    match token {
...
pub unsafe fn parse_statement(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState) {
    unsafe {
        ...
        const TK_FUNCTION: i32 = 264;
        const TK_GOTO: i32 = 265;
        const TK_BREAK: i32 = 257;
        const TK_RETURN: i32 = 273;
        const TK_LOCAL: i32 = 268;
        const TK_REPEAT: i32 = 272;
        const TK_FOR: i32 = 263;
        const TK_DO: i32 = 258;
        const TK_IF: i32 = 266;
        match (*lexical_state).lexicalstate_token.token {
            ...
            | TK_IF => {
                handle_if_statement(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate, line);
            },
            | TK_WHILE => {
                handle_while_statement(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate, line);
            },
            | TK_DO => {
                luax_next(interpreter, lexical_state);
                handle_block(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate);
            ...
            | TK_FOR => {
                handle_for_statement(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate, line);
            },
            | TK_REPEAT => {
                handle_repeat_statement(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate, line);
            },
            | TK_FUNCTION => {
                handle_function_statement(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate, line);
            },
            | TK_LOCAL => {
                luax_next(interpreter, lexical_state);
                if testnext(interpreter, lexical_state, function_state, TK_FUNCTION as i32) != 0 {
                    handle_local_function(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate);
                } else {
                    handle_local_statement(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate);
                }
            },
            | TK_DOUBLECOLON => {
                luax_next(interpreter, lexical_state);
            ...
            | TK_RETURN => {
                luax_next(interpreter, lexical_state);
                handle_return_statement(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate);
            },
            | TK_BREAK => {
                breakstat(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate);
            },
            | TK_GOTO => {
                luax_next(interpreter, lexical_state);
                gotostat(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate);
            },
            | _ => {
                handle_expression_statement(interpreter, lexical_state, (*lexical_state).lexicalstate_functionstate);
            },
        }
