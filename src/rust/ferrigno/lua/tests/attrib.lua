...
local DIR = string.format("ferrigno_test_%d_%d", os.time(), math.random(100000, 999999)) .. dirsep
...
local function removefiles (files)
  for n in pairs(files) do
    os.remove(D(n))
    -- remove parent subdirectory if file was nested (e.g. "P1/init.lua")
    local sub = string.match(n, "^([^/]+)/")
    if sub then os.remove(DIR .. sub) end
  end
  os.remove(DIR:sub(1, -2))  -- remove DIR itself (strip trailing separator)
end
...
try('A', 'A.lua', true, DIR .. "A.lua")
package.loaded.A = nil
os.remove(D'A.lua')
AA = {}
try('A', 'A.lc', AA, DIR .. "A.lc")  -- now must find second option
assert(package.searchpath("A", package.path) == D"A.lc")
assert(require("A") == AA)
AA = false
try('K', 'L', false, DIR .. "L")     -- default option
try('K', 'L', false, DIR .. "L")     -- default option (should reload it)
assert(rawget(_G, "_REQUIREDNAME") == nil)

AA = "x"
try("X", "XXxX", AA, DIR .. "XXxX")
...
if not st then
  ...
  assert(string.find(ext, DIR .. "lib2-v2", 1, true))
  ...
  assert(string.find(ext, DIR .. "lib1", 1, true))
  assert(fs.id(45) == 45)
  _ENV.x, _ENV.y = nil
end
...
do  print("testing external strings")
  package.cpath = DC"?"
  local ok, lib2 = pcall(require, "lib2-v2")
  if not ok then print(" >>> C module 'lib2-v2' not available, skipping <<<") goto skip_extstr end
  local lib2 = lib2
  local t = {}
