        ...
        for i in 1..=nup as i32 {
            ...
            match upval.get_tagvariant() {
                TagVariant::ClosureL => {
                    ...
                    let proto = (*upval.as_closure().unwrap())
                        .closure_payload
...
unsafe fn disassemble_prototype(prototype: *const Prototype, buf: &mut String, indent: usize) {
    unsafe {
        ...
        for pc in 0..code_len {
            ...
            let opname = if (op as usize) < OPCODE_NAMES.len() {
            ...
            let mode = if (op as usize) < OPMODES.len() {
                OPMODES[op as usize]
            } else {
                0
            };
...
unsafe fn format_instruction_comment(
    ...
    unsafe {
        match op {
            ...
            OPCODE_ADDK | OPCODE_SUBK | OPCODE_MULK | OPCODE_MODK | OPCODE_POWK | OPCODE_DIVK | OPCODE_IDIVK | OPCODE_BANDK
            | OPCODE_BORK | OPCODE_BXORK => {
                buf.push_str("\t; ");
                format_constant(prototype, c as usize, buf);
            }
            OPCODE_LOADK => {
...
pub unsafe fn dis_info(state: *mut State) -> i32 {
    unsafe {
        ...
        let source = (*prototype).prototype_source;
        if !source.is_null() {
            lua_pushlstring(state, (*source).get_contents_mut(), (*source).get_length());
