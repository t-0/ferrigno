...
unsafe fn do_request(
    ...
    url: &[u8],
    body: Option<&[u8]>,
    headers: *mut std::ffi::c_void, // curl_slist* or null
    ...
    unsafe {
        ...
        // User-Agent
        curl_easy_setopt(curl, CURLOPT_USERAGENT, c"ferrigno-urllib/1.0".as_ptr());
        ...
        Ok((body_buf, status_code, header_buf))
    }
}
        ...
        for line in raw.split(|&b| b == b'\n') {
            ...
            if let Some(colon) = memchr(line, b':') {
                ...
                if ty == TagType::String {
                    ...
                    let existing_ptr = lua_tolstring(state, -1, &mut existing_len);
...
pub unsafe fn urllib_encode(state: *mut State) -> i32 {
    unsafe {
        let mut len = 0usize;
        let ptr = lual_checklstring(state, 1, &mut len);
        let input = std::slice::from_raw_parts(ptr as *const u8, len);
        let encoded = percent_encode(input);
        lua_pushlstring(state, encoded.as_ptr() as *const i8, encoded.len());
        1
    }
}

/// urllib.decode(str) → percent-decoded string
pub unsafe fn urllib_decode(state: *mut State) -> i32 {
    unsafe {
        let mut len = 0usize;
        let ptr = lual_checklstring(state, 1, &mut len);
        let input = std::slice::from_raw_parts(ptr as *const u8, len);
        let decoded = percent_decode(input);
        lua_pushlstring(state, decoded.as_ptr() as *const i8, decoded.len());
        1
    }
}

/// urllib.parse(url) → table
pub unsafe fn urllib_parse(state: *mut State) -> i32 {
    unsafe {
        let mut len = 0usize;
        let ptr = lual_checklstring(state, 1, &mut len);
        ...
        let push_field = |interp: *mut State, key: &[u8], val: &[u8]| {
            lua_pushlstring(interp, key.as_ptr() as *const i8, key.len());
            lua_pushlstring(interp, val.as_ptr() as *const i8, val.len());
            lua_rawset(interp, -3);
        };

        push_field(state, b"scheme", &p.scheme);
        push_field(state, b"user", &p.user);
        push_field(state, b"password", &p.password);
        push_field(state, b"host", &p.host);
        push_field(state, b"path", &p.path);
        push_field(state, b"query", &p.query);
        push_field(state, b"fragment", &p.fragment);

        lua_pushlstring(state, b"port".as_ptr() as *const i8, 4);
        if let Some(port) = p.port {
            (*state).push_integer(port as i64);
        } else {
            (*state).push_nil();
        }
        lua_rawset(state, -3);
...
pub unsafe fn urllib_request(state: *mut State) -> i32 {
    unsafe {
        let mut mlen = 0usize;
        ...
        // Optional body (arg 3)
        let body_data: Option<Vec<u8>> = if lua_type(state, 3) == Some(TagType::String) {
            let mut blen = 0usize;
            let bptr = lua_tolstring(state, 3, &mut blen);
        ...
        let mut slist: *mut std::ffi::c_void = null_mut();
        if lua_type(state, 4) == Some(TagType::Table) {
            slist = build_header_slist(state, 4);
        }

        let result = do_request(method, url, body_data.as_deref(), slist);
...
pub unsafe fn urllib_get(state: *mut State) -> i32 {
    unsafe {
        let mut ulen = 0usize;
        ...
        let mut slist: *mut std::ffi::c_void = null_mut();
        if lua_type(state, 2) == Some(TagType::Table) {
            slist = build_header_slist(state, 2);
        }
...
pub unsafe fn urllib_post(state: *mut State) -> i32 {
    unsafe {
        let mut ulen = 0usize;
        let uptr = lual_checklstring(state, 1, &mut ulen);
        let url = std::slice::from_raw_parts(uptr as *const u8, ulen);
        ...
        let mut blen = 0usize;
        let bptr = lual_checklstring(state, 2, &mut blen);
        let body = std::slice::from_raw_parts(bptr as *const u8, blen).to_vec();

        let mut slist: *mut std::ffi::c_void = null_mut();
        if lua_type(state, 3) == Some(TagType::Table) {
            slist = build_header_slist(state, 3);
        }
...
pub const URLLIB_FUNCTIONS: [RegisteredFunction; 6] = [
    RegisteredFunction {
        ...
        registeredfunction_function: Some(urllib_encode as unsafe fn(*mut State) -> i32),
    },
    RegisteredFunction {
        registeredfunction_name: c"decode".as_ptr(),
        registeredfunction_function: Some(urllib_decode as unsafe fn(*mut State) -> i32),
    },
    RegisteredFunction {
        registeredfunction_name: c"parse".as_ptr(),
        registeredfunction_function: Some(urllib_parse as unsafe fn(*mut State) -> i32),
    },
    RegisteredFunction {
        registeredfunction_name: c"get".as_ptr(),
        registeredfunction_function: Some(urllib_get as unsafe fn(*mut State) -> i32),
    },
    RegisteredFunction {
        registeredfunction_name: c"post".as_ptr(),
        registeredfunction_function: Some(urllib_post as unsafe fn(*mut State) -> i32),
    },
    RegisteredFunction {
        registeredfunction_name: c"request".as_ptr(),
        registeredfunction_function: Some(urllib_request as unsafe fn(*mut State) -> i32),
    },
];

pub unsafe fn luaopen_urllib(state: *mut State) -> i32 {
    unsafe {
        (*state).lua_createtable();
        lual_setfuncs(state, URLLIB_FUNCTIONS.as_ptr(), URLLIB_FUNCTIONS.len(), 0);
        1
    }
}
