            ...
            _ => {
                ...
                (((2 as i64 <= base && base <= 36 as i64) as i32 != 0) as i64 != 0
                    || lual_argerror(
                        interpreter,
                        2,
                        make_cstring!("base out of range"),
                    ) != 0) as i32;
...
pub unsafe fn luab_setmetatable(interpreter: *mut Interpreter) -> i32 {
    unsafe {
        ...
        if lual_getmetafield(interpreter, 1, make_cstring!("__metatable"))
            != TagType::Nil
        {
            return lual_error(
                interpreter,
...
pub unsafe fn generic_reader(
    ...
    unsafe {
        lual_checkstack(
            interpreter,
            2,
            make_cstring!("too many nested functions"),
        );
        lua_pushvalue(interpreter, 1);
            ...
            && *lua_tolstring(interpreter, 1, null_mut()) as i32 == CHARACTER_OCTOTHORPE as i32
        {
            ...
            (((1 <= i) as i32 != 0) as i64 != 0
                || lual_argerror(
                    interpreter,
                    1,
                    make_cstring!("index out of range"),
                ) != 0) as i32;
            return n - i as i32;
        };
    }
    ...
    [
        ...
        },
        {
            RegisteredFunction {
                ...
                function: Some(
                    luab_collectgarbage as unsafe fn(*mut Interpreter) -> i32,
                ),
            }
        },
        {
        ...
        },
        {
            RegisteredFunction {
                ...
                function: Some(luab_setmetatable as unsafe fn(*mut Interpreter) -> i32),
            }
        },
        {
            RegisteredFunction {
                name: make_cstring!("tonumber"),
                function: Some(luab_tonumber as unsafe fn(*mut Interpreter) -> i32),
            }
        },
        {
            RegisteredFunction {
                name: make_cstring!("tostring"),
