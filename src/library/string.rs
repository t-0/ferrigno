    ...
    unsafe {
        ...
        for i in 0..n {
            (*interpreter).push_integer(
                *s.offset(posi.wrapping_add(i as usize).wrapping_sub(1 as usize) as isize) as u8
                    as i64,
            );
        }
        return n;
    ...
    unsafe {
        ...
        for i in 1..(1 + n) {
            ...
            (((c <= (127 as i32 * 2 + 1) as usize) as i32 != 0) as i64 != 0
                || lual_argerror(interpreter, i, make_cstring!("value out of range")) != 0)
                as i32;
...
pub unsafe fn str_gsub(interpreter: *mut Interpreter) -> i32 {
    unsafe {
        ...
        (((tr == Some(TagType::Numeric)
            ...
            != 0) as i64
            != 0
            || lual_typeerror(interpreter, 3, make_cstring!("string/function/table")) != 0)
            as i32;
    ...
    unsafe {
        match lua_type(interpreter, arg) {
            ...
            Some(TagType::Numeric) => {
                ...
                (*b).loads.set_length(
                    (((*b).loads.get_length() as usize).wrapping_add(nb as usize) as i32) as usize,
                );
            }
                ...
                if *strfrmt as i32 == CHARACTER_PERCENT as i32 {
                    ...
                    *(b.loads.loads_pointer).offset(fresh177 as isize) = *fresh176;
                    ...
                    match *fresh178 as i32 {
                        ...
                        CHARACTER_LOWER_E | CHARACTER_UPPER_E | CHARACTER_LOWER_G
                        | CHARACTER_UPPER_G => {
                            current_block = 6669252993407410313;
                        }
                        CHARACTER_LOWER_P => {
                            ...
                            if form[2 as usize] as i32 == Character::Null as i32 {
                                ...
                                (((l == strlen(s) as usize) as i32 != 0) as i64 != 0
                                    || lual_argerror(
                                        interpreter,
                                        arg,
                                        make_cstring!("string contains zeros"),
                                    ) != 0) as i32;
...
pub unsafe fn packint(b: *mut Buffer, mut n: usize, islittle: i32, size: i32, is_negative_: i32) {
    unsafe {
        ...
        *buffer.offset((if islittle != 0 { 0 } else { size - 1 }) as isize) =
        ...
        (*b).loads.set_length(
            (((*b).loads.get_length() as usize).wrapping_add(size as usize) as i32) as usize,
        );
    }
}
        ...
        while *fmt as i32 != Character::Null as i32 {
            ...
            match opt as u32 {
                0 => {
                    ...
                    if size < size_of::<i64>() as i32 {
                        ...
                        (((-lim <= n && n < lim) as i32 != 0) as i64 != 0
                            || lual_argerror(interpreter, arg, make_cstring!("integer overflow"))
                                != 0) as i32;
                    }
                ...
                1 => {
                    ...
                    if size < size_of::<i64>() as i32 {
                        ((((n_0 as usize) < (1 as usize) << size * 8) as i32 != 0) as i64 != 0
                            || lual_argerror(interpreter, arg, make_cstring!("unsigned overflow"))
                                != 0) as i32;
                    }
                ...
                2 => {
                    ...
                    b.loads.set_length(
                        ((b.loads.get_length() as usize).wrapping_add(size as usize) as i32)
                            as usize,
                    );
                    current_block_33 = 3222590281903869779;
                }
                ...
                5 => {
                    ...
                    (((length <= size as usize) as i32 != 0) as i64 != 0
                        || lual_argerror(
                            interpreter,
                            arg,
                            make_cstring!("string longer than given size"),
                        ) != 0) as i32;
                ...
                6 => {
                    ...
                    (((size >= size_of::<usize>() as i32 || length < (1 as usize) << size * 8)
                        as i32
                        != 0) as i64
                        != 0
                        || lual_argerror(
                            interpreter,
                            arg,
                            make_cstring!("string length does not fit in given size"),
                        ) != 0) as i32;
                ...
                7 => {
                    ...
                    (((strlen(s_1) as usize == length) as i32 != 0) as i64 != 0
                        || lual_argerror(interpreter, arg, make_cstring!("string contains zeros"))
                            != 0) as i32;
    ...
    unsafe {
        ...
        while *fmt as i32 != Character::Null as i32 {
            ...
            (((opt as u32 != K::String as u32 && opt as u32 != K::ZString as u32) as i32 != 0)
                as i64
                != 0
                || lual_argerror(interpreter, 1, make_cstring!("variable-length format")) != 0)
                as i32;
            size += ntoalign;
            (((totalsize
                <= (if (size_of::<usize>() as usize) < size_of::<i32>() as usize {
                ...
                != 0) as i64
                != 0
                || lual_argerror(interpreter, 1, make_cstring!("format result too large")) != 0)
                as i32;
    ...
    unsafe {
        ...
        (((pos <= ld) as i32 != 0) as i64 != 0
            || lual_argerror(
                interpreter,
                3,
                make_cstring!("initial position out of string"),
            ) != 0) as i32;
        initheader(interpreter, &mut h);
        while *fmt as i32 != Character::Null as i32 {
            let mut size: i32 = 0;
            ...
            ((((ntoalign as usize).wrapping_add(size as usize) <= ld.wrapping_sub(pos)) as i32 != 0)
                as i64
                != 0
                || lual_argerror(interpreter, 2, make_cstring!("data string too short")) != 0)
                as i32;
            ...
            match opt as u32 {
                ...
                6 => {
                    ...
                    (((length <= ld.wrapping_sub(pos).wrapping_sub(size as usize)) as i32 != 0)
                        as i32 as i64
                        != 0
                        || lual_argerror(interpreter, 2, make_cstring!("data string too short"))
                            != 0) as i32;
                    lua_pushlstring(
                        interpreter,
                ...
                7 => {
                    ...
                    (((pos.wrapping_add(length_0) < ld) as i32 != 0) as i64 != 0
                        || lual_argerror(
                            interpreter,
                            2,
                            make_cstring!("unfinished string for format 'zio'"),
                        ) != 0) as i32;
