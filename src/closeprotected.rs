...
impl CloseProtected {
    unsafe fn auxiliary(interpreter: *mut Interpreter, arbitrary_data: *mut libc::c_void) {
        unsafe {
            ...
            luaf_close(
                interpreter,
                (*close_protected).level,
                (*close_protected).status,
                0,
            );
        }
    }
}
pub unsafe fn do_close_protected(
    ...
    unsafe {
        ...
        loop {
            ...
            close_protected.level = ((*interpreter).stack.stkidrel_pointer as *mut i8)
                .offset(level as isize) as *mut TValue;
            close_protected.status = status;
            status = luad_rawrunprotected(
                interpreter,
                Some(
                    CloseProtected::auxiliary
                        as unsafe fn(*mut Interpreter, *mut libc::c_void) -> (),
                ),
