...
pub unsafe fn forlimit(interpreter: *mut Interpreter, initial: i64, lim: *const TValue, p: *mut i64, step: i64) -> i32 {
    unsafe {
        if luav_tointeger(lim, p, if step < 0 { F2I::Ceiling } else { F2I::Floor }) == 0 {
            ...
            if if (*lim).get_tag_variant() == TagVariant::NumericNumber {
...
pub unsafe fn forprep(interpreter: *mut Interpreter, ra: *mut TValue) -> i32 {
    unsafe {
        ...
        if (*pinit).get_tag_variant() == TagVariant::NumericInteger && (*pstep).get_tag_variant() == TagVariant::NumericInteger {
            ...
            if forlimit(interpreter, initial, plimit, &mut limit, step) != 0 {
                ...
                (*plimit).set_tag_variant(TagVariant::NumericInteger);
            }
        } else {
            let mut init_0: f64 = 0.0;
            let mut limit_0: f64 = 0.0;
            let mut step_0: f64 = 0.0;
            if (((if (*plimit).get_tag_variant() == TagVariant::NumericNumber {
            ...
            if (((if (*pstep).get_tag_variant() == TagVariant::NumericNumber {
            ...
            if (((if (*pinit).get_tag_variant() == TagVariant::NumericNumber {
            ...
            if if (0.0) < step_0 { (limit_0 < init_0) as i32 } else { (init_0 < limit_0) as i32 } != 0 {
                ...
                (*plimit).set_tag_variant(TagVariant::NumericNumber);
                (*pstep).value.value_number = step_0;
                (*pstep).set_tag_variant(TagVariant::NumericNumber);
                let io_3: *mut TValue = &mut (*ra);
                (*io_3).value.value_number = init_0;
                (*io_3).set_tag_variant(TagVariant::NumericNumber);
                let io_4: *mut TValue = &mut (*ra.offset(3 as isize));
                (*io_4).value.value_number = init_0;
                (*io_4).set_tag_variant(TagVariant::NumericNumber);
            }
        }
        return 0;
    }
...
pub unsafe fn floatforloop(ra: *mut TValue) -> i32 {
    unsafe {
        let step: f64 = (*ra.offset(2 as isize)).value.value_number;
        let limit: f64 = (*ra.offset(1 as isize)).value.value_number;
        let mut index: f64 = (*ra).value.value_number;
        index = index + step;
