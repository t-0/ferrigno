...
unsafe extern "C" fn luab_cocreate(state: *mut State) -> i32 { unsafe {
...
unsafe extern "C" fn luab_cowrap(state: *mut State) -> i32 { unsafe {
    luab_cocreate(state);
    lua_pushcclosure(
        state,
...
unsafe extern "C" fn luab_coresume(state: *mut State) -> i32 { unsafe {
...
unsafe extern "C" fn luab_costatus(state: *mut State) -> i32 { unsafe {
    let co: *mut State = getco(state);
    lua_pushstring(state, COROUTINE_STATUS_NAMES[auxstatus(state, co) as usize]);
    return 1;
}}
unsafe extern "C" fn luab_yieldable(state: *mut State) -> i32 { unsafe {
...
unsafe extern "C" fn luab_yield(state: *mut State) -> i32 { unsafe {
...
unsafe extern "C" fn auxstatus(state: *mut State, co: *mut State) -> i32 { unsafe {
    if state == co {
        return 0;
    } else {
        match lua_status(co) {
...
pub unsafe extern "C" fn luaopen_coroutine(state: *mut State) -> i32 { unsafe {
    lual_checkversion_(
        state,
        504.0,
