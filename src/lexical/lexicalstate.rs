...
pub unsafe fn codeclosure(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, v: *mut ExpressionDescription) {
    unsafe {
        ...
        init_exp(
            ...
            luak_codeabx(interpreter, lexical_state, function_state, OPCODE_CLOSURE, 0, ((*function_state).count_prototypes - 1) as u32),
        );
...
pub unsafe fn recfield(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, constructor_control: *mut ConstructorControl) {
    unsafe {
        ...
        if (*lexical_state).token.token == Token::Name as i32 {
            codename(interpreter, lexical_state, function_state, &mut key);
        } else {
...
pub unsafe fn constructor(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, t: *mut ExpressionDescription) {
    unsafe {
        ...
        luak_settablesize(interpreter, function_state, program_counter, (*t).value.value_info, constructor_control.count_array, constructor_control.count_table);
    }
}
    ...
    unsafe {
        ...
        if is_method {
            new_localvar(
                interpreter,
                lexical_state,
                luax_newstring(interpreter, lexical_state, c"self".as_ptr(), (size_of::<[i8; 5]>() as usize).wrapping_sub(1 as usize)),
            );
        ...
        check_match(interpreter, lexical_state, function_state, Token::End as i32, Token::Function as i32, line);
...
pub unsafe fn funcargs(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, expression_description: *mut ExpressionDescription) {
    unsafe {
        ...
        match (*lexical_state).token.token {
            ...
            TK_STRING => {
                codestring(&mut args, (*lexical_state).token.semantic_info.value_tstring);
                luax_next(interpreter, lexical_state);
            },
            _ => {
        ...
        let base: i32 = (*expression_description).value.value_info;
        let nparams: i32;
        if args.expression_kind == ExpressionKind::Call || args.expression_kind == ExpressionKind::VariableArguments {
            nparams = -1;
        } else {
    ...
    unsafe {
        ...
        match (*lexical_state).token.token {
            TK_FLOAT => {
                ...
                (*v).value.value_number = (*lexical_state).token.semantic_info.value_number;
            },
            TK_INTEGER => {
                init_exp(v, ExpressionKind::ConstantInteger, 0);
                (*v).value.value_integer = (*lexical_state).token.semantic_info.value_integer;
            },
            TK_STRING => {
                codestring(v, (*lexical_state).token.semantic_info.value_tstring);
            },
            TK_NIL => {
            ...
            TK_DOTS => {
                ...
                if !(*(*function_state).prototype).prototype_is_variable_arguments {
                    luax_syntaxerror(interpreter, lexical_state, c"cannot use '...' outside a vararg function".as_ptr());
                }
                init_exp(v, ExpressionKind::VariableArguments, code_abck(interpreter, lexical_state, function_state, OPCODE_VARARG, 0, 0, 1, 0));
            },
            TK_CHARACTER_BRACE_LEFT => {
...
pub unsafe fn error_expected(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, _function_state: *mut FunctionState, token: i32) -> ! {
    unsafe {
        luax_syntaxerror(
            interpreter,
            lexical_state,
            luao_pushfstring(interpreter, c"%s expected".as_ptr(), luax_token2str(interpreter, lexical_state, token)),
        );
    }
}
...
pub unsafe fn check_readonly(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, expression_description: *mut ExpressionDescription) {
    unsafe {
        ...
        if !variable_name.is_null() {
            let message: *const i8 = luao_pushfstring(interpreter, c"attempt to assign to const variable '%s'".as_ptr(), (*variable_name).get_contents_mut());
            luak_semerror(interpreter, lexical_state, message);
        }
    }
}
    ...
    unsafe {
        ...
        while !lhs_assign.is_null() {
            if (*lhs_assign).expression_kind.is_index() {
                if (*lhs_assign).expression_kind == ExpressionKind::IndexUpValue {
                    if (*v).expression_kind == ExpressionKind::UpValue && (*lhs_assign).value.value_index.valuereference_tag as i32 == (*v).value.value_info {
                        conflict = 1;
                        (*lhs_assign).expression_kind = ExpressionKind::Field;
                        (*lhs_assign).value.value_index.valuereference_tag = extra as u8;
                    }
                } else {
                    if (*v).expression_kind == ExpressionKind::Local && (*lhs_assign).value.value_index.valuereference_tag as i32 == (*v).value.value_variable.valueregister_registerindex as i32 {
                        conflict = 1;
                        (*lhs_assign).value.value_index.valuereference_tag = extra as u8;
                    }
                    if (*lhs_assign).expression_kind == ExpressionKind::Indexed && (*v).expression_kind == ExpressionKind::Local && (*lhs_assign).value.value_index.valuereference_index as i32 == (*v).value.value_variable.valueregister_registerindex as i32 {
                        conflict = 1;
                        (*lhs_assign).value.value_index.valuereference_index = extra as i16;
                    }
                }
            }
        ...
        if conflict != 0 {
            if (*v).expression_kind == ExpressionKind::Local {
                code_abck(interpreter, lexical_state, function_state, OPCODE_MOVE, extra, (*v).value.value_variable.valueregister_registerindex as i32, 0, 0);
            } else {
                code_abck(interpreter, lexical_state, function_state, OPCODE_GET_UPVALUE, extra, (*v).value.value_info, 0, 0);
            }
            luak_reserveregs(interpreter, lexical_state, function_state, 1);
        }
    }
...
pub unsafe fn handle_label_statement(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, name: *mut TString, line: i32) {
    unsafe {
        ...
        while (*lexical_state).token.token == Character::Semicolon as i32 || (*lexical_state).token.token == Token::Dbcolon as i32 {
            parse_statement(interpreter, lexical_state, function_state);
        }
...
pub unsafe fn handle_while_statement(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, line: i32) {
    unsafe {
        ...
        check_match(interpreter, lexical_state, function_state, Token::End as i32, Token::While as i32, line);
        leaveblock(interpreter, lexical_state, function_state);
...
pub unsafe fn handle_repeat_statement(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, line: i32) {
    unsafe {
        ...
        luax_next(interpreter, lexical_state);
        parse_statement_list(interpreter, lexical_state, function_state);
        check_match(interpreter, lexical_state, function_state, Token::Until as i32, Token::Repeat as i32, line);
...
pub unsafe fn handle_forbody(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, base: i32, line: i32, count_variables: i32, isgen: i32) {
    unsafe {
        ...
        if isgen != 0 {
            code_abck(interpreter, lexical_state, function_state, OPCODE_TFORCALL, base, 0, count_variables, 0);
            luak_fixline(interpreter, lexical_state, function_state, line);
        }
...
pub unsafe fn handle_for_statement(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, line: i32) {
    unsafe {
      ...
      match (*lexical_state).token.token {
        ...
        check_match(interpreter, lexical_state, function_state, Token::End as i32, TK_FOR as i32, line);
        leaveblock(interpreter, lexical_state, function_state);
    }
}
pub unsafe fn handle_test_then_block(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, escapelist: *mut i32) {
    unsafe {
        ...
        if (*lexical_state).token.token == Token::Break as i32 {
            let line: i32 = (*lexical_state).line_number;
        ...
        parse_statement_list(interpreter, lexical_state, function_state);
        leaveblock(interpreter, lexical_state, function_state);
        if (*lexical_state).token.token == Token::Else as i32 || (*lexical_state).token.token == Token::Elseif as i32 {
...
pub unsafe fn handle_if_statement(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState, line: i32) {
    unsafe {
        ...
        while (*lexical_state).token.token == Token::Elseif as i32 {
            handle_test_then_block(interpreter, lexical_state, (*lexical_state).lexical_state_function_state, &mut escape_list);
        }
        if testnext(interpreter, lexical_state, function_state, Token::Else as i32) != 0 {
            handle_block(interpreter, lexical_state, (*lexical_state).lexical_state_function_state);
        }
        check_match(interpreter, lexical_state, function_state, Token::End as i32, Token::If as i32, line);
    ...
    unsafe {
        if testnext(interpreter, lexical_state, function_state, Character::AngleLeft as i32) != 0 {
            ...
            if strcmp(attr, c"const".as_ptr()) == 0 {
                ...
                luak_semerror(interpreter, lexical_state, luao_pushfstring(interpreter, c"unknown attribute '%s'".as_ptr(), attr));
            }
        }
        return 0;
    }
...
pub unsafe fn handle_expression_statement(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, function_state: *mut FunctionState) {
    unsafe {
        ...
        if (*lexical_state).token.token == Character::Equal as i32 || (*lexical_state).token.token == Character::Comma as i32 {
            ...
            let inst: *mut u32 = &mut *((*(*function_state).prototype).prototype_code.vectort_pointer).offset(new_lhs_assign.value.value_info as isize) as *mut u32;
        ...
        if block_follow_with_until((*lexical_state).token.token) || (*lexical_state).token.token == Character::Semicolon as i32 {
            ...
            if expression_description.expression_kind == ExpressionKind::Call || expression_description.expression_kind == ExpressionKind::VariableArguments {
                ...
                if expression_description.expression_kind == ExpressionKind::Call && nret == 1 && !(*(*function_state).block_control).is_inside_tbc {
                    *((*(*function_state).prototype).prototype_code.vectort_pointer).offset(expression_description.value.value_info as isize) =
                        *((*(*function_state).prototype).prototype_code.vectort_pointer).offset(expression_description.value.value_info as isize) & !(!(!(0u32) << 7) << 0) | (OPCODE_TAILCALL as u32) << 0 & !(!(0u32) << 7) << 0;
                }
                nret = -1;
            } else if nret == 1 {
    ...
    unsafe {
        ...
        match token {
            TK_NAME | TK_STRING | TK_FLOAT | TK_INTEGER => {
                ...
                return luao_pushfstring(interpreter, c"'%s'".as_ptr(), (*(*lexical_state).buffer).loads.loads_pointer);
            },
...
pub unsafe fn luax_newstring(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, str: *const i8, length: usize) -> *mut TString {
    unsafe {
        ...
        let tvalue: *const TValue = luah_getstr((*lexical_state).table, tstring);
        if (*tvalue).is_tagtype_nil() {
            ...
            luah_finishset(interpreter, (*lexical_state).table, stv, tvalue, stv);
            if (*(*interpreter).global).global_gcdebt > 0 {
                (*interpreter).luac_step();
            }
            (*interpreter).top.stkidrel_pointer = (*interpreter).top.stkidrel_pointer.offset(-1);
        } else {
            tstring = &mut (*((*(tvalue as *mut Node)).key.value.value_object as *mut TString));
        }
        return tstring;
    }
}
...
pub unsafe fn luax_setinput(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, zio: *mut ZIO, source: *mut TString, firstchar: i32) {
    unsafe {
        ...
        (*lexical_state).look_ahead.token = Token::EndOfStream as i32;
...
pub unsafe fn read_numeral(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, semantic_info: *mut Value) -> i32 {
    unsafe {
        ...
        if obj.get_tag_variant() == TagVariant::NumericInteger {
            ...
            (*semantic_info).value_number = obj.value.value_number;
        ...
        loop {
            match Character::from2((*lexical_state).current) {
                None => {
                    ...
                    let message: *const i8 = luao_pushfstring(interpreter, c"unfinished long %s (starting at line %d)".as_ptr(), what, line);
...
pub unsafe fn readutf8esc(interpreter: *mut Interpreter, lexical_state: *mut LexicalState) -> usize {
    unsafe {
        ...
        loop {
            ...
            i += 1;
            esccheck(interpreter, lexical_state, r <= (0x7fffffff as usize >> 4), c"UTF-8 value too large".as_ptr());
...
pub unsafe fn read_string(interpreter: *mut Interpreter, lexical_state: *mut LexicalState, del: i32, semantic_info: *mut Value) {
    unsafe {
        ...
        while (*lexical_state).current != del {
            match Character::from2((*lexical_state).current) {
                None => {
                    lexerror(interpreter, lexical_state, c"unfinished string".as_ptr(), Token::EndOfStream as i32);
                },
                Some(Character::LineFeed) | Some(Character::CarriageReturn) => {
                ...
                Some(Character::Backslash) => {
                    ...
                    match Character::from((*lexical_state).current) {
                        ...
                        _ => {
                            esccheck(interpreter, lexical_state, Character::from((*lexical_state).current).is_digit_decimal(), c"invalid escape sequence".as_ptr());
            ...
            match Character::from2((*lexical_state).current) {
                ...
                Some(Character::BracketLeft) => {
                    ...
                    if sep_0 >= 2 as usize {
                        ...
                        lexerror(interpreter, lexical_state, c"invalid long string delimiter".as_ptr(), Token::String as i32);
                    }
                ...
                _ => {
                    if Character::from((*lexical_state).current).is_identifier() {
                        ...
                        if (*tstring).get_tag_variant() == TagVariant::StringShort && (*tstring).tstring_extra as i32 > 0 {
...
pub unsafe fn luax_next(interpreter: *mut Interpreter, lexical_state: *mut LexicalState) {
    unsafe {
        ...
        if (*lexical_state).look_ahead.token != Token::EndOfStream as i32 {
    ...
    unsafe {
        ...
        match (*lexical_state).token.token {
            ...
            TK_DO => {
                luax_next(interpreter, lexical_state);
                handle_block(interpreter, lexical_state, (*lexical_state).lexical_state_function_state);
                check_match(interpreter, lexical_state, function_state, Token::End as i32, Token::Do as i32, line);
            },
            TK_FOR => {
