...
unsafe extern "C" fn laction(mut i: i32) {
    let mut flag: i32 = (1i32) << 0i32 | (1i32) << 1i32 | (1i32) << 2i32 | (1i32) << 3i32;
    setsignal(i, None);
    lua_sethook(
        globalL,
...
unsafe extern "C" fn report(mut L: *mut lua_State, mut status: i32) -> i32 {
    if status != 0i32 {
        let mut msg: *const libc::c_char = lua_tolstring(L, -(1i32), 0 as *mut u64);
        if msg.is_null() {
...
unsafe extern "C" fn msghandler(mut L: *mut lua_State) -> i32 {
    let mut msg: *const libc::c_char = lua_tolstring(L, 1i32, 0 as *mut u64);
    if msg.is_null() {
        if luaL_callmeta(L, 1i32, b"__tostring\0" as *const u8 as *const libc::c_char) != 0
...
unsafe extern "C" fn docall(mut L: *mut lua_State, mut narg: i32, mut nres: i32) -> i32 {
    ...
    globalL = L;
    setsignal(2i32, Some(laction as unsafe extern "C" fn(i32) -> ()));
...
unsafe extern "C" fn print_version() {
    fwrite(
        b"Lua 5.4.8  Copyright (C) 1994-2025 Lua.org, PUC-Rio\0" as *const u8 as *const libc::c_char
...
unsafe extern "C" fn createargtable(
    ...
    while i < argc {
        ...
        lua_rawseti(L, -(2i32), (i - script) as i64);
        i += 1;
    }
    lua_setglobal(L, b"arg\0" as *const u8 as *const libc::c_char);
}
unsafe extern "C" fn dochunk(mut L: *mut lua_State, mut status: i32) -> i32 {
    if status == 0i32 {
        status = docall(L, 0i32, 0i32);
    }
    return report(L, status);
}
unsafe extern "C" fn dofile(mut L: *mut lua_State, mut name: *const libc::c_char) -> i32 {
...
unsafe extern "C" fn dolibrary(mut L: *mut lua_State, mut globname: *mut libc::c_char) -> i32 {
    let mut status: i32 = 0;
...
unsafe extern "C" fn pushargs(mut L: *mut lua_State) -> i32 {
    ...
    if lua_getglobal(L, b"arg\0" as *const u8 as *const libc::c_char) != 5i32 {
        luaL_error(
            L,
            b"'arg' is not a table\0" as *const u8 as *const libc::c_char,
        );
    }
...
unsafe extern "C" fn handle_script(mut L: *mut lua_State, mut argv: *mut *mut libc::c_char) -> i32 {
    let mut status: i32 = 0;
...
unsafe extern "C" fn collectargs(mut argv: *mut *mut libc::c_char, mut first: *mut i32) -> i32 {
    let mut args: i32 = 0i32;
    let mut i: i32 = 0;
    if !(*argv.offset(0i32 as isize)).is_null() {
        if *(*argv.offset(0i32 as isize)).offset(0i32 as isize) != 0 {
    ...
    while !(*argv.offset(i as isize)).is_null() {
        *first = i;
        if *(*argv.offset(i as isize)).offset(0i32 as isize) as i32 != '-' as i32 {
            return args;
        }
        let mut current_block_31: u64;
        match *(*argv.offset(i as isize)).offset(1i32 as isize) as i32 {
            45 => {
                if *(*argv.offset(i as isize)).offset(2i32 as isize) as i32 != '\0' as i32 {
                    return 1i32;
                }
                *first = i + 1i32;
                return args;
            ...
            8308901264692237116 => {
                if *(*argv.offset(i as isize)).offset(2i32 as isize) as i32 == '\0' as i32 {
                    ...
                    if (*argv.offset(i as isize)).is_null()
                        || *(*argv.offset(i as isize)).offset(0i32 as isize) as i32 == '-' as i32
                    {
                        return 1i32;
                    }
                }
...
unsafe extern "C" fn runargs(
    ...
    i = 1i32;
    while i < n {
        let mut option: i32 = *(*argv.offset(i as isize)).offset(1i32 as isize) as i32;
        match option {
            101 | 108 => {
                let mut status: i32 = 0;
                let mut extra: *mut libc::c_char = (*argv.offset(i as isize)).offset(2i32 as isize);
            ...
            87 => {
                lua_warning(L, b"@on\0" as *const u8 as *const libc::c_char, 0i32);
            }
            _ => {}
        }
        i += 1;
...
unsafe extern "C" fn get_prompt(mut L: *mut lua_State, mut firstline: i32) -> *const libc::c_char {
    if lua_getglobal(
        L,
        if firstline != 0 {
    ...
    ) == 0i32
    {
        ...
        let mut p: *const libc::c_char = luaL_tolstring(L, -(1i32), 0 as *mut u64);
        lua_rotate(L, -(2i32), -(1i32));
        lua_settop(L, -(1i32) - 1i32);
        return p;
    };
}
unsafe extern "C" fn incomplete(mut L: *mut lua_State, mut status: i32) -> i32 {
    if status == 3i32 {
        let mut lmsg: u64 = 0;
...
unsafe extern "C" fn pushline(mut L: *mut lua_State, mut firstline: i32) -> i32 {
    ...
    fputs(prmt, stdout);
    fflush(stdout);
    let mut readstatus: i32 =
        (fgets(b, 512i32, stdin) != 0 as *mut libc::c_void as *mut libc::c_char) as i32;
    lua_settop(L, -(1i32) - 1i32);
    if readstatus == 0i32 {
        return 0i32;
    }
    ...
    if l > 0i32 as libc::c_ulong
        && *b.offset(l.wrapping_sub(1i32 as libc::c_ulong) as isize) as i32 == '\n' as i32
    {
        l = l.wrapping_sub(1);
        *b.offset(l as isize) = '\0' as i32 as libc::c_char;
    }
    if firstline != 0 && *b.offset(0i32 as isize) as i32 == '=' as i32 {
        lua_pushfstring(
            L,
...
unsafe extern "C" fn addreturn(mut L: *mut lua_State) -> i32 {
    let mut line: *const libc::c_char = lua_tolstring(L, -(1i32), 0 as *mut u64);
    let mut retline: *const libc::c_char =
        lua_pushfstring(L, b"return %s;\0" as *const u8 as *const libc::c_char, line);
...
unsafe extern "C" fn pmain(mut L: *mut lua_State) -> i32 {
    let mut argc: i32 = lua_tointegerx(L, 1i32, 0 as *mut i32) as i32;
    ...
    if args & 2i32 != 0 {
        doREPL(L);
    } else if script < 1i32 && args & (8i32 | 4i32) == 0 {
        if isatty(0i32) != 0 {
            print_version();
            doREPL(L);
        } else {
...
pub unsafe fn main_0(mut argc: i32, mut argv: *mut *mut libc::c_char) -> i32 {
    let mut status: i32 = 0;
    ...
    if L.is_null() {
        l_message(
            *argv.offset(0i32 as isize),
            b"cannot create state: not enough memory\0" as *const u8 as *const libc::c_char,
        );
        return 1i32;
    }
    lua_gc(L, 0i32);
