...
pub unsafe extern "C" fn lua_rawget(state: *mut State, index: i32) -> i32 {
    unsafe {
        ...
        let value: *const TValue =
            luah_get(table, &mut (*(*state).top.p.offset(-(1 as isize))).value);
        (*state).top.p = (*state).top.p.offset(-1);
...
pub unsafe extern "C" fn lua_setmetatable(state: *mut State, objindex: i32) -> i32 {
    unsafe {
        ...
        if get_tag_type((*(*state).top.p.offset(-(1 as isize))).value.get_tag()) == TAG_TYPE_NIL {
            ...
            mt = &mut (*((*(*state).top.p.offset(-(1 as isize))).value.value.object
                as *mut Table))
        }
    ...
    unsafe {
        ...
        if k.is_none() || !((*state).count_c_calls & 0xffff0000 as u32 == 0) {
            ...
            (*call_info).u2.funcidx =
                (calls.function as *mut i8).offset_from((*state).stack.p as *mut i8) as i64 as i32;
...
pub unsafe extern "C" fn lua_load(
    ...
    unsafe {
        ...
        if status == 0 {
            let f: *mut LClosure =
                &mut (*((*(*state).top.p.offset(-(1 as isize))).value.value.object
                    as *mut LClosure));
            if (*f).count_upvalues as i32 >= 1 {
                ...
                if (*gt).is_collectable() {
                    if (**((*f).upvalues).as_mut_ptr().offset(0 as isize)).get_marked() & 1 << 5
                        != 0
...
pub unsafe extern "C" fn lua_dump(
    ...
    unsafe {
        ...
        if (*o).get_tag_variant() == TAG_VARIANT_CLOSURE_L {
            status = luau_dump(
                state,
                (*((*o).value.object as *mut LClosure)).p,
                writer_0,
                data,
                is_strip,
            );
...
pub unsafe extern "C" fn lua_status(state: *mut State) -> i32 {
    unsafe {
...
pub unsafe extern "C" fn lua_setwarnf(state: *mut State, f: WarnFunction, arbitrary_data: *mut libc::c_void) {
    unsafe {
        (*(*state).global).ud_warn = arbitrary_data;
...
pub unsafe extern "C" fn getupvalref(
    ...
    unsafe {
        ...
        if 1 <= n && n <= (*(*f).p).size_upvalues {
            return &mut *((*f).upvalues).as_mut_ptr().offset((n - 1) as isize)
                as *mut *mut UpValue;
        } else {
...
pub unsafe extern "C" fn luae_freethread(state: *mut State, other_state: *mut State) {
    unsafe {
        ...
        (*state).free_memory(l as *mut libc::c_void, ::core::mem::size_of::<LX>() as u64);
    }
}
...
pub unsafe extern "C" fn lua_newstate() -> *mut State {
    unsafe {
        ...
        (*g).allgc = &mut (*(state as *mut Object));
...
pub unsafe extern "C" fn findsetreg(p: *const Prototype, mut lastpc: i32, reg: i32) -> i32 {
    unsafe {
        ...
        if OPMODES
            [(*((*p).code).offset(lastpc as isize) >> 0 & !(!(0) << 7) << 0) as u32 as usize]
            as i32
            & 1 << 7
            != 0
        {
...
pub unsafe extern "C" fn luag_tracecall(state: *mut State) -> i32 {
    unsafe {
        let call_info: *mut CallInfo = (*state).call_info;
        let p: *mut Prototype = (*((*(*call_info).function.p).value.value.object as *mut LClosure))
            .p;
        ::core::ptr::write_volatile(&mut (*call_info).u.l.trap as *mut i32, 1);
...
pub unsafe extern "C" fn luag_traceexec(state: *mut State, mut program_counter: *const u32) -> i32 {
    unsafe {
        ...
        let p: *const Prototype = (*((*(*call_info).function.p).value.value.object
            as *mut LClosure))
            .p;
    ...
    unsafe {
        match op {
            7 | 8 | 9 | 10 | 11 | 13 => {
                let mut i1: i64 = 0;
                let mut i2: i64 = 0;
                if (if (((*p1).get_tag() == TAG_VARIANT_NUMERIC_INTEGER) as i32 != 0) as i32 as i64
                    != 0
                {
                    i1 = (*p1).value.i;
                    1
    ...
    unsafe {
                ...
                || *s.offset(1 as isize) as i32 == 'X' as i32)
        {
            ...
            while CHARACTER_TYPE[(*s as u8 as i32 + 1) as usize] as i32 & 1 << 1 != 0 {
                ...
                if a >= (0x7FFFFFFFFFFFFFFF as i64 / 10 as i32 as i64) as u64
                    && (a > (0x7FFFFFFFFFFFFFFF as i64 / 10 as i32 as i64) as u64
                        || d > (0x7FFFFFFFFFFFFFFF as i64 % 10 as i32 as i64) as i32
...
pub unsafe extern "C" fn traverse_state(g: *mut Global, state: *mut State) -> i32 {
    unsafe {
        ...
        return 1 + ((*state).stack_last.p).offset_from((*state).stack.p) as i64 as i32;
    }
}
...
pub unsafe extern "C" fn luaf_newlclosure(state: *mut State, mut nupvals: i32) -> *mut LClosure {
    unsafe {
        let o: *mut Object = luac_newobj(
            state,
            TAG_VARIANT_CLOSURE_L,
            (32 as u64 as i32 + ::core::mem::size_of::<*mut TValue>() as u64 as i32 * nupvals)
                as u64,
        );
...
pub unsafe extern "C" fn luas_newlstr(state: *mut State, str: *const i8, l: u64) -> *mut TString {
    unsafe {
        if l <= STRING_SHORT_MAX as i32 as u64 {
...
pub unsafe extern "C" fn load_int(load_state: *mut LoadState) -> i32 {
    unsafe {
        return load_unsigned(load_state, 0x7FFFFFFF as i32 as u64) as i32;
    }
}
...
pub unsafe extern "C" fn dump_block(dump_state: *mut DumpState, b: *const libc::c_void, size: u64) {
    unsafe {
...
pub unsafe extern "C" fn dump_byte(dump_state: *mut DumpState, y: u8) {
    unsafe {
        let mut x: u8 = y;
        dump_block(
            dump_state,
...
pub unsafe extern "C" fn dump_size(dump_state: *mut DumpState, mut x: u64) {
    unsafe {
...
pub unsafe extern "C" fn dump_int(dump_state: *mut DumpState, x: i32) {
    unsafe {
        dump_size(dump_state, x as u64);
    }
}
pub unsafe extern "C" fn dump_number(dump_state: *mut DumpState, mut x: f64) {
    unsafe {
        dump_block(
            dump_state,
...
pub unsafe extern "C" fn dump_integer(dump_state: *mut DumpState, mut x: i64) {
    unsafe {
        dump_block(
            dump_state,
...
pub unsafe extern "C" fn dump_string(dump_state: *mut DumpState, s: *const TString) {
    unsafe {
        if s.is_null() {
...
pub unsafe extern "C" fn dump_code(dump_state: *mut DumpState, f: *const Prototype) {
    unsafe {
...
pub unsafe extern "C" fn dump_constants(dump_state: *mut DumpState, f: *const Prototype) {
    unsafe {
        let mut i: i32;
        let n: i32 = (*f).size_k;
        ...
        while i < n {
            ...
            match tag {
                ...
                4 | 20 => {
                    dump_string(dump_state, &mut (*((*o).value.object as *mut TString)));
                }
                _ => {}
            }
            i += 1;
...
pub unsafe extern "C" fn dump_prototypes(dump_state: *mut DumpState, f: *const Prototype) {
    unsafe {
        let mut i: i32;
        let n: i32 = (*f).size_p;
        dump_int(dump_state, n);
...
pub unsafe extern "C" fn dump_upvalues(dump_state: *mut DumpState, f: *const Prototype) {
    unsafe {
        let mut i: i32;
...
pub unsafe extern "C" fn dump_debug(dump_state: *mut DumpState, f: *const Prototype) {
    unsafe {
        let mut i: i32;
        let mut n: i32;
...
pub unsafe extern "C" fn dump_header(dump_state: *mut DumpState) {
    unsafe {
        dump_block(
            dump_state,
        ...
        match (*lexical_state).t.token {
            CHARACTER_PARENTHESIS_LEFT => {
                ...
                if (*lexical_state).t.token == ')' as i32 {
                    ...
                    if args.k as u32 == VCALL as i32 as u32
                        || args.k as u32 == VVARARG as i32 as u32
                    {
...
pub unsafe extern "C" fn check_conflict(
    ...
    unsafe {
        ...
        while !lh.is_null() {
            if VINDEXED as i32 as u32 <= (*lh).v.k as u32
                && (*lh).v.k as u32 <= VINDEXSTR as i32 as u32
            {
                if (*lh).v.k as u32 == VINDEXUP as i32 as u32 {
                    if (*v).k as u32 == VUPVAL as i32 as u32
...
pub unsafe extern "C" fn restassign(
    ...
    unsafe {
        ...
        if !(VLOCAL as i32 as u32 <= (*lh).v.k as u32
            && (*lh).v.k as u32 <= VINDEXSTR as i32 as u32)
        {
        ...
        if testnext(lexical_state, ',' as i32) != 0 {
            ...
            if !(VINDEXED as i32 as u32 <= nv.v.k as u32
                && nv.v.k as u32 <= VINDEXSTR as i32 as u32)
            {
...
pub unsafe extern "C" fn readutf8esc(lexical_state: *mut LexicalState) -> u64 {
    unsafe {
        ...
        (*(*lexical_state).buffer).length =
            ((*(*lexical_state).buffer).length as u64).wrapping_sub(i as u64) as u64 as u64;
        return r;
    }
}
...
pub unsafe extern "C" fn hashint(t: *const Table, i: i64) -> *mut Node {
    unsafe {
        let ui: u64 = i as u64;
        if ui <= 0x7FFFFFFF as i32 as u32 as u64 {
            return &mut *((*t).node)
...
pub unsafe extern "C" fn l_hashfloat(mut n: f64) -> i32 {
        ...
        })
    {
        ...
        return (if u <= 0x7FFFFFFF as i32 as u32 { u } else { !u }) as i32;
    };
}
...
pub unsafe extern "C" fn luah_getint(table: *mut Table, key: i64) -> *const TValue {
    unsafe {
        ...
        if (key as u64).wrapping_sub(1 as u32 as u64) < array_limit {
            ...
            && (key as u64).wrapping_sub(1 as u32 as u64)
                & !array_limit.wrapping_sub(1 as u32 as u64)
                < array_limit
        {
...
pub unsafe extern "C" fn luah_getshortstr(table: *mut Table, key: *mut TString) -> *const TValue {
    unsafe {
        let mut node: *mut Node = &mut *((*table).node).offset(
            ((*key).hash & ((1 << (*table).log_size_node as i32) - 1) as u32) as i32 as isize,
        ) as *mut Node;
        loop {
...
pub unsafe extern "C" fn getjumpcontrol(fs: *mut FunctionState, program_counter: i32) -> *mut u32 {
    unsafe {
        ...
        if program_counter >= 1
            && OPMODES[(*pi.offset(-(1 as isize)) >> 0 & !(!(0u32) << 7) << 0) as u32 as usize]
                as i32
                & 1 << 4
                != 0
        {
    ...
    unsafe {
        ...
        if reg != (1 << 8) - 1 && reg != (*i >> 0 + 7 + 8 + 1 & !(!(0u32) << 8) << 0) as i32 {
            ...
            *i = (OP_TEST as i32 as u32) << 0
                | ((*i >> 0 + 7 + 8 + 1 & !(!(0u32) << 8) << 0) as i32 as u32) << 0 + 7
                | (0u32) << 0 + 7 + 8 + 1
                | (0u32) << 0 + 7 + 8 + 1 + 8
                | ((*i >> 0 + 7 + 8 & !(!(0u32) << 1) << 0) as i32 as u32) << 0 + 7 + 8;
        }
        return 1;
    }
}
...
pub unsafe extern "C" fn codeextraarg(fs: *mut FunctionState, a: i32) -> i32 {
    unsafe {
        return luak_code(fs, (OP_EXTRAARG as i32 as u32) << 0 | (a as u32) << 0 + 7);
    }
}
...
pub unsafe extern "C" fn fits_c(i: i64) -> i32 {
    return ((i as u64).wrapping_add(((1 << 8) - 1 >> 1) as u64) <= ((1 << 8) - 1) as u32 as u64)
        as i32;
}
...
pub unsafe extern "C" fn luak_settablesize(
    ...
    unsafe {
        ...
        *inst = (OP_NEWTABLE as i32 as u32) << 0
            ...
            | (rc as u32) << 0 + 7 + 8 + 1 + 8
            | (k as u32) << 0 + 7 + 8;
        *inst.offset(1 as isize) = (OP_EXTRAARG as i32 as u32) << 0 | (extra as u32) << 0 + 7;
    }
}
...
pub unsafe extern "C" fn finaltarget(code: *mut u32, mut i: i32) -> i32 {
    unsafe {
        ...
        while count < 100 as i32 {
            ...
            if (program_counter >> 0 & !(!(0u32) << 7) << 0) as u32 as u32 != OP_JMP as i32 as u32 {
                break;
            }
...
pub unsafe extern "C" fn luav_finishop(state: *mut State) {
    unsafe {
        ...
        match op as u32 {
            ...
            70 => {
                let ra: StkId = base.offset((inst >> 0 + 7 & !(!(0u32) << 8) << 0) as i32 as isize);
                    ...
                    match (i >> 0 & !(!(0u32) << 7) << 0) as u32 as u32 {
                        0 => {
                            ...
                            let io2: *const TValue = &mut (*base.offset(
                                (i >> 0 + 7 + 8 + 1 & !(!(0u32) << 8) << 0) as i32 as isize,
                            ))
                            .value;
                            (*io1).value = (*io2).value;
                            (*io1).set_tag((*io2).get_tag());
                        ...
                        3 => {
                            ...
                            let rb: *mut TValue = k.offset(
                                (i >> 0 + 7 + 8 & !(!(0u32) << 8 + 8 + 1) << 0) as i32 as isize,
                            );
                        ...
                        11 => {
                            ...
                            let rc: *mut TValue = k.offset(
                                (i >> 0 + 7 + 8 + 1 + 8 & !(!(0u32) << 8) << 0) as i32 as isize,
                            );
                        ...
                        15 => {
                            ...
                            let upval_0: *mut TValue = (**((*cl).upvalues)
                                .as_mut_ptr()
                                .offset((i >> 0 + 7 & !(!(0u32) << 8) << 0) as i32 as isize))
                            .v
                            .p;
                        ...
                        46 => {
                            ...
                            .value;
                            let tm: u32 =
                                (i >> 0 + 7 + 8 + 1 + 8 & !(!(0u32) << 8) << 0) as i32 as u32;
                            let result: StkId =
                                base.offset((pi >> 0 + 7 & !(!(0u32) << 8) << 0) as i32 as isize);
                            (*call_info).u.l.saved_program_counter = program_counter;
                            (*state).top.p = (*call_info).top.p;
                        ...
                        47 => {
                            ...
                            let result_0: StkId =
                                base.offset((pi_0 >> 0 + 7 & !(!(0u32) << 8) << 0) as i32 as isize);
                            (*call_info).u.l.saved_program_counter = program_counter;
                            (*state).top.p = (*call_info).top.p;
                            luat_trybinitm(
                                state,
                        ...
                        48 => {
                            ...
                            let result_1: StkId =
                                base.offset((pi_1 >> 0 + 7 & !(!(0u32) << 8) << 0) as i32 as isize);
                            (*call_info).u.l.saved_program_counter = program_counter;
                            (*state).top.p = (*call_info).top.p;
                            luat_trybinassoctm(
                                state,
...
pub unsafe extern "C" fn pushglobalfuncname(state: *mut State, ar: *mut Debug) -> i32 {
    unsafe {
...
pub unsafe extern "C" fn lual_testudata(
    ...
    unsafe {
        ...
        if !p.is_null() {
            if (*state).lua_getmetatable(arbitrary_data) != 0 {
                lua_getfield(state, -(1000000 as i32) - 1000 as i32, tname);
...
pub unsafe extern "C" fn lual_getmetafield(state: *mut State, obj: i32, event: *const i8) -> i32 {
    unsafe {
        if (*state).lua_getmetatable(obj) == 0 {
            return 0;
        } else {
            let tag: i32;
...
pub unsafe extern "C" fn checktab(state: *mut State, arg: i32, what: i32) {
    unsafe {
        if lua_type(state, arg) != Some(TAG_TYPE_TABLE) {
            let mut n: i32 = 1;
            if (*state).lua_getmetatable(arg) != 0
                && (what & 1 == 0 || {
                    n += 1;
    ...
    unsafe {
        ...
        match (*state).get_top() {
            ...
            3 => {
                ...
                ((((pos as u64).wrapping_sub(1 as u32 as u64) < e as u64) as i32 != 0) as i32
                    as i64
                    != 0
                    || lual_argerror(
                        state,
...
pub unsafe extern "C" fn tremove(state: *mut State) -> i32 {
    unsafe {
        ...
        if pos != size {
            ((((pos as u64).wrapping_sub(1 as u32 as u64) <= size as u64) as i32 != 0) as i32
                as i64
                != 0
                || lual_argerror(
                    state,
...
pub unsafe extern "C" fn tmove(state: *mut State) -> i32 {
    unsafe {
        ...
        if e >= f {
            let n: i64;
            let mut i: i64;
            (((f > 0 || e < 0x7FFFFFFFFFFFFFFF as i64 + f) as i32 != 0) as i32 as i64 != 0
                || lual_argerror(
                    state,
                    3,
            ...
            n = e - f + 1;
            (((t <= 0x7FFFFFFFFFFFFFFF as i64 - n + 1) as i32 != 0) as i32 as i64 != 0
                || lual_argerror(
                    state,
                    4,
...
pub unsafe extern "C" fn tunpack(state: *mut State) -> i32 {
    unsafe {
        ...
        if ((n >= 0x7FFFFFFF as i32 as u32 as u64 || {
            n = n.wrapping_add(1);
...
pub unsafe extern "C" fn read_number(state: *mut State, f: *mut FILE) -> i32 {
    unsafe {
        ...
        loop {
            rn.c = getc_unlocked(rn.f);
            if !(*(*__ctype_b_loc()).offset(rn.c as isize) as i32 & _ISSPACE as i32 as u16 as i32
                != 0)
            {
                break;
            }
...
pub unsafe extern "C" fn getfield(state: *mut State, key: *const i8, d: i32, delta: i32) -> i32 {
    unsafe {
        ...
        if !is_number {
            ...
            if if res >= 0 {
                (res - delta as i64 <= 0x7FFFFFFF as i32 as i64) as i32
            } else {
...
pub unsafe extern "C" fn l_checktime(state: *mut State, arg: i32) -> i64 {
    unsafe {
        ...
        (((t as i64 == t) as i32 != 0) as i32 as i64 != 0
            || lual_argerror(
                state,
                arg,
...
pub unsafe extern "C" fn str_byte(state: *mut State) -> i32 {
    unsafe {
        ...
        if ((pose.wrapping_sub(posi) >= 0x7FFFFFFF as i32 as u64) as i32 != 0) as i32 as i64 != 0 {
...
pub unsafe extern "C" fn match_class(c: i32, cl: i32) -> i32 {
    unsafe {
        ...
        match tolower(cl) {
            ...
            99 => {
                res =
                    *(*__ctype_b_loc()).offset(c as isize) as i32 & _ISCONTROL as i32 as u16 as i32;
            }
            100 => {
            ...
            112 => {
                res = *(*__ctype_b_loc()).offset(c as isize) as i32
                    & _ISPUNCTUATION as i32 as u16 as i32;
            }
            115 => {
            ...
            120 => {
                res =
                    *(*__ctype_b_loc()).offset(c as isize) as i32 & _ISXDIGIT as i32 as u16 as i32;
            }
            122 => {
                res = (c == 0) as i32;
            }
        ...
        return if *(*__ctype_b_loc()).offset(cl as isize) as i32 & _ISLOWER as i32 as u16 as i32
            != 0
        {
            res
        } else {
    ...
    unsafe {
        ...
        if find != 0 && (lua_toboolean(state, 4) != 0 || nospecials(p, lp) != 0) {
            ...
            if !s2.is_null() {
                ...
                (*state).push_integer((s2.offset_from(s) as i64 as u64).wrapping_add(lp) as i64);
                return 2;
            }
        } else {
...
pub unsafe extern "C" fn str_gsub(state: *mut State) -> i32 {
    unsafe {
        ...
        let mut b = Buffer::new();
        (((tr == Some(TAG_TYPE_NUMERIC) || tr == Some(TAG_TYPE_STRING) || tr == Some(TAG_TYPE_CLOSURE) || tr == Some(TAG_TYPE_TABLE)) as i32 != 0) as i32 as i64 != 0
            || lual_typeerror(
                state,
                3,
        ...
        while *fmt as i32 != '\0' as i32 {
            ...
            match opt as u32 {
                ...
                1 => {
                    ...
                    if size < ::core::mem::size_of::<i64>() as u64 as i32 {
                        ((((n_0 as u64) < (1 as u64) << size * 8) as i32 != 0) as i32 as i64 != 0
                            || lual_argerror(
                                state,
                                arg,
...
pub unsafe extern "C" fn str_packsize(state: *mut State) -> i32 {
    unsafe {
        ...
        while *fmt as i32 != '\0' as i32 {
            ...
            let mut ntoalign: i32 = 0;
            let opt: K = getdetails(&mut h, totalsize, &mut fmt, &mut size, &mut ntoalign);
            (((opt as u32 != K::String as i32 as u32 && opt as u32 != K::ZString as i32 as u32)
                as i32
                != 0) as i32 as i64
                != 0
                || lual_argerror(
...
pub unsafe extern "C" fn codepoint(state: *mut State) -> i32 {
    unsafe {
        ...
        if pose - posi >= 0x7FFFFFFF as i32 as i64 {
            return lual_error(state, b"string slice too long\0" as *const u8 as *const i8);
        }
...
pub unsafe extern "C" fn checkstack(state: *mut State, other_state: *mut State, n: i32) {
    unsafe {
        if ((state != other_state && lua_checkstack(other_state, n) == 0) as i32 != 0) as i32 as i64
            != 0
        {
            lual_error(state, b"stack overflow\0" as *const u8 as *const i8);
        }
