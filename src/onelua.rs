...
pub unsafe extern "C" fn luad_reallocstack(
    ...
    while i < new_size + 5 {
        (*newstack.offset(i as isize)).val.tag = (0 | 0 << 4) as u8;
        i += 1;
    }
    return 1;
}}
...
pub unsafe extern "C" fn tryfunctm(state: *mut State, mut function: StkId) -> StkId { unsafe {
    ...
    if (((*tm).tag as i32 & 0xf as i32 == 0) as i32 != 0) as i32 as i64 != 0 {
        luag_callerror(state, &mut (*function).val);
    }
    p = (*state).top.p;
    while p > function {
        let io1: *mut TValue = &mut (*p).val;
        let io2: *const TValue = &mut (*p.offset(-(1 as isize))).val;
        (*io1).value = (*io2).value;
        (*io1).tag = (*io2).tag;
        p = p.offset(-1);
    }
...
pub unsafe extern "C" fn moveresults(
    ...
    while i < wanted {
        (*res.offset(i as isize)).val.tag = (0 | 0 << 4) as u8;
        i += 1;
    }
...
pub unsafe extern "C" fn luad_pretailcall(
    ...
    loop {
        match (*function).val.tag as i32 & 0x3f as i32 {
            38 => {
                return precallc(
                    state,
                    function,
                    -1,
                    (*((*function).val.value.gc as *mut GCUnion)).ccl.f,
                );
            }
            22 => return precallc(state, function, -1, (*function).val.value.f),
            6 => {
...
pub unsafe extern "C" fn luad_precall(
    ...
    loop {
        match (*function).val.tag as i32 & 0x3f as i32 {
            ...
            22 => {
                precallc(state, function, count_results, (*function).val.value.f);
                return std::ptr::null_mut();
            }
            6 => {
                let call_info;
...
pub unsafe extern "C" fn resume_error(
    ...
    let io: *mut TValue = &mut (*(*state).top.p).val;
    let x_: *mut TString = luas_new(state, msg);
    (*io).value.gc = &mut (*(x_ as *mut GCUnion)).gc;
    (*io).tag = ((*x_).tag as i32 | 1 << 6) as u8;
    (*state).top.p = ((*state).top.p).offset(1);
    (*state).top.p;
...
pub unsafe extern "C" fn index2value(state: *mut State, mut index: i32) -> *mut TValue { unsafe {
    ...
    if index > 0 {
        ...
        return &mut (*((*state).top.p).offset(index as isize)).val;
    } else if index == -(1000000 as i32) - 1000 as i32 {
...
pub unsafe extern "C" fn lua_xmove(from: *mut State, to: *mut State, n: i32) { unsafe {
    ...
    i = 0;
    while i < n {
        let io1: *mut TValue = &mut (*(*to).top.p).val;
        let io2: *const TValue = &mut (*((*from).top.p).offset(i as isize)).val;
        (*io1).value = (*io2).value;
        (*io1).tag = (*io2).tag;
...
pub unsafe extern "C" fn lua_rotate(state: *mut State, index: i32, n: i32) { unsafe {
    let t: StkId = ((*state).top.p).offset(-(1 as isize));
    let p: StkId = index2stack(state, index);
    let m: StkId = if n >= 0 {
        t.offset(-(n as isize))
    } else {
    ...
    reverse(state, p, m);
    reverse(state, m.offset(1 as isize), t);
    reverse(state, p, t);
}}
#[unsafe(no_mangle)]
    ...
    if toidx < -(1000000 as i32) - 1000 as i32 {
        if (*fr).tag as i32 & 1 << 6 != 0 {
            if (*((*(*(*state).call_info).function.p).val.value.gc as *mut GCUnion))
                ...
                & 1 << 5
                != 0
                && (*(*fr).value.gc).marked as i32 & (1 << 3 | 1 << 4) != 0
            {
                luac_barrier_(
                    state,
...
pub unsafe extern "C" fn lua_type(state: *mut State, index: i32) -> i32 { unsafe {
    ...
    return if !((*o).tag as i32 & 0xf as i32 == 0)
        || o != &mut (*(*state).global).nilvalue as *mut TValue as *const TValue
    {
        (*o).tag as i32 & 0xf as i32
    } else {
        -1
    };
}}
...
pub unsafe extern "C" fn lua_isstring(state: *mut State, index: i32) -> bool { unsafe {
    let o: *const TValue = index2value(state, index);
    return (*o).tag as i32 & 0xf as i32 == 4 || (*o).tag as i32 & 0xf as i32 == 3;
}}
#[unsafe(no_mangle)]
pub unsafe extern "C" fn lua_rawequal(
    ...
    return if (!((*o1).tag as i32 & 0xf as i32 == 0)
        || o1 != &mut (*(*state).global).nilvalue as *mut TValue as *const TValue)
        && (!((*o2).tag as i32 & 0xf as i32 == 0)
...
pub unsafe extern "C" fn lua_arith(state: *mut State, op: i32) { unsafe {
    ...
    luao_arith(
        state,
        op,
        &mut (*((*state).top.p).offset(-(2 as isize))).val,
...
pub unsafe extern "C" fn lua_stringtonumber(state: *mut State, s: *const i8) -> u64 { unsafe {
    let size: u64 = luao_str2num(s, &mut (*(*state).top.p).val);
    if size != 0u64 {
        (*state).top.p = ((*state).top.p).offset(1);
        (*state).top.p;
    }
...
pub unsafe extern "C" fn lua_toboolean(state: *mut State, index: i32) -> i32 { unsafe {
    let o: *const TValue = index2value(state, index);
    return !((*o).tag as i32 == 1 | 0 << 4 || (*o).tag as i32 & 0xf as i32 == 0)
        as i32;
}}
#[unsafe(no_mangle)]
pub unsafe extern "C" fn lua_tolstring(
    ...
    if !length.is_null() {
        *length = if (*((*o).value.gc as *mut GCUnion)).ts.short_length as i32 != 0xff as i32 {
            (*((*o).value.gc as *mut GCUnion)).ts.short_length as u64
        } else {
            (*((*o).value.gc as *mut GCUnion)).ts.u.long_length
        };
    }
    return ((*((*o).value.gc as *mut GCUnion)).ts.contents).as_mut_ptr();
}}
#[unsafe(no_mangle)]
pub unsafe extern "C" fn lua_rawlen(state: *mut State, index: i32) -> u64 { unsafe {
    let o: *const TValue = index2value(state, index);
    match (*o).tag as i32 & 0x3f as i32 {
...
pub unsafe extern "C" fn lua_tocfunction(state: *mut State, index: i32) -> CFunction { unsafe {
    let o: *const TValue = index2value(state, index);
    if (*o).tag as i32 == 6 | 1 << 4 {
        return (*o).value.f;
    } else if (*o).tag as i32 == 6 | (2) << 4 | 1 << 6 {
                ...
                (if (*((*o).value.gc as *mut GCUnion)).u.nuvalue as i32 == 0 {
                    ...
                    (40 as u64).wrapping_add(
                        (::core::mem::size_of::<UValue>() as u64)
                            .wrapping_mul((*((*o).value.gc as *mut GCUnion)).u.nuvalue as u64),
                    )
                }) as isize,
            ) as *mut libc::c_void;
        }
...
pub unsafe extern "C" fn lua_tothread(state: *mut State, index: i32) -> *mut State { unsafe {
    let o: *const TValue = index2value(state, index);
    return if !((*o).tag as i32 == 8 | 0 << 4 | 1 << 6) {
        std::ptr::null_mut()
    } else {
...
pub unsafe extern "C" fn lua_pushcclosure(state: *mut State, fn_0: CFunction, mut n: i32) { unsafe {
    if n == 0 {
        ...
        loop {
            ...
            let io2: *const TValue = &mut (*((*state).top.p).offset(n as isize)).val;
            (*io1).value = (*io2).value;
            (*io1).tag = (*io2).tag;
        }
...
pub unsafe extern "C" fn auxgetstr(
    ...
    if if !((*t).tag as i32 == 5 | 0 << 4 | 1 << 6) {
        slot = std::ptr::null();
        0
    } else {
        slot = luah_getstr(&mut (*((*t).value.gc as *mut GCUnion)).h, str);
        !((*slot).tag as i32 & 0xf as i32 == 0) as i32
    } != 0
    {
        let io1: *mut TValue = &mut (*(*state).top.p).val;
    ...
    return (*((*state).top.p).offset(-(1 as isize))).val.tag as i32 & 0xf as i32;
}}
#[unsafe(no_mangle)]
pub unsafe extern "C" fn lua_getglobal(state: *mut State, name: *const i8) -> i32 { unsafe {
    let global_table: *const TValue = &mut *((*((*(*state).global).l_registry.value.gc as *mut GCUnion))
        .h
        .array)
...
pub unsafe extern "C" fn lua_gettable(state: *mut State, index: i32) -> i32 { unsafe {
    ...
    if if !((*t).tag as i32 == 5 | 0 << 4 | 1 << 6) {
        ...
        slot = luah_get(
            &mut (*((*t).value.gc as *mut GCUnion)).h,
            &mut (*((*state).top.p).offset(-(1 as isize))).val,
        );
        !((*slot).tag as i32 & 0xf as i32 == 0) as i32
    } != 0
    {
        let io1: *mut TValue = &mut (*((*state).top.p).offset(-(1 as isize))).val;
        let io2: *const TValue = slot;
        (*io1).value = (*io2).value;
        (*io1).tag = (*io2).tag;
    } else {
...
pub unsafe extern "C" fn lua_geti(state: *mut State, index: i32, n: i64) -> i32 { unsafe {
    ...
    if if !((*t).tag as i32 == 5 | 0 << 4 | 1 << 6) {
        ...
        slot = if (n as u64).wrapping_sub(1 as u32 as u64)
            < (*((*t).value.gc as *mut GCUnion)).h.alimit as u64
        {
            &mut *((*((*t).value.gc as *mut GCUnion)).h.array).offset((n - 1) as isize)
                as *mut TValue as *const TValue
        } else {
            luah_getint(&mut (*((*t).value.gc as *mut GCUnion)).h, n)
        };
        !((*slot).tag as i32 & 0xf as i32 == 0) as i32
    } != 0
    {
...
pub unsafe extern "C" fn finishrawget(state: *mut State, val: *const TValue) -> i32 { unsafe {
    if (*val).tag as i32 & 0xf as i32 == 0 {
        (*(*state).top.p).val.tag = (0 | 0 << 4) as u8;
    } else {
        let io1: *mut TValue = &mut (*(*state).top.p).val;
...
pub unsafe extern "C" fn gettable(state: *mut State, index: i32) -> *mut Table { unsafe {
    let t: *mut TValue = index2value(state, index);
    return &mut (*((*t).value.gc as *mut GCUnion)).h;
}}
#[unsafe(no_mangle)]
pub unsafe extern "C" fn lua_rawget(state: *mut State, index: i32) -> i32 { unsafe {
    let t: *mut Table = gettable(state, index);
    let val: *const TValue = luah_get(t, &mut (*((*state).top.p).offset(-(1 as isize))).val);
    (*state).top.p = ((*state).top.p).offset(-1);
    (*state).top.p;
    return finishrawget(state, val);
}}
#[unsafe(no_mangle)]
pub unsafe extern "C" fn lua_rawgeti(state: *mut State, index: i32, n: i64) -> i32 { unsafe {
    let t: *mut Table = gettable(state, index);
    return finishrawget(state, luah_getint(t, n));
}}
#[unsafe(no_mangle)]
...
pub unsafe extern "C" fn lua_createtable(state: *mut State, narray: i32, nrec: i32) { unsafe {
    ...
    let x_: *mut Table = t;
    (*io).value.gc = &mut (*(x_ as *mut GCUnion)).gc;
    (*io).tag = (5 | 0 << 4 | 1 << 6) as u8;
    (*state).top.p = ((*state).top.p).offset(1);
    (*state).top.p;
    if narray > 0 || nrec > 0 {
        luah_resize(state, t, narray as u32, nrec as u32);
    }
    if (*(*state).global).gc_debt > 0 {
        luac_step(state);
    }
...
pub unsafe extern "C" fn lua_getmetatable(state: *mut State, objindex: i32) -> i32 { unsafe {
    ...
    let mt: *mut Table;
    match (*obj).tag as i32 & 0xf as i32 {
        5 => {
            mt = (*((*obj).value.gc as *mut GCUnion)).h.metatable;
        }
        7 => {
            mt = (*((*obj).value.gc as *mut GCUnion)).u.metatable;
        }
        _ => {
            mt = (*(*state).global).mt[((*obj).tag as i32 & 0xf as i32) as usize];
        }
    }
    let res: i32 = if mt.is_null() {
        0
    } else {
        let io: *mut TValue = &mut (*(*state).top.p).val;
        let x_: *mut Table = mt;
...
pub unsafe extern "C" fn lua_getiuservalue(state: *mut State, index: i32, n: i32) -> i32 { unsafe {
    ...
    if n <= 0 || n > (*((*o).value.gc as *mut GCUnion)).u.nuvalue as i32 {
        ...
        let io1: *mut TValue = &mut (*(*state).top.p).val;
        let io2: *const TValue = &mut (*((*((*o).value.gc as *mut GCUnion)).u.uv)
            .as_mut_ptr()
            .offset((n - 1) as isize))
        .uv;
        (*io1).value = (*io2).value;
        (*io1).tag = (*io2).tag;
        t = (*(*state).top.p).val.tag as i32 & 0xf as i32;
    }
    (*state).top.p = ((*state).top.p).offset(1);
    (*state).top.p;
    return t;
...
pub unsafe extern "C" fn auxsetstr(state: *mut State, t: *const TValue, k: *const i8) { unsafe {
    ...
    } != 0
    {
        ...
        (*io1).value = (*io2).value;
        (*io1).tag = (*io2).tag;
        if (*((*state).top.p).offset(-(1 as isize))).val.tag as i32 & 1 << 6 != 0 {
            if (*(*t).value.gc).marked as i32 & 1 << 5 != 0
                && (*(*((*state).top.p).offset(-(1 as isize))).val.value.gc).marked as i32
                    & (1 << 3 | 1 << 4)
                    != 0
            {
                luac_barrierback_(state, (*t).value.gc);
            } else {
            };
        } else {
        };
...
pub unsafe extern "C" fn aux_rawset(
    ...
    if (*((*state).top.p).offset(-(1 as isize))).val.tag as i32 & 1 << 6 != 0 {
        if (*(t as *mut GCUnion)).gc.marked as i32 & 1 << 5 != 0
            && (*(*((*state).top.p).offset(-(1 as isize))).val.value.gc).marked as i32
                & (1 << 3 | 1 << 4)
                != 0
        {
            luac_barrierback_(state, &mut (*(t as *mut GCUnion)).gc);
        } else {
        };
    } else {
    };
...
pub unsafe extern "C" fn lua_setmetatable(state: *mut State, objindex: i32) -> i32 { unsafe {
    ...
    if (*((*state).top.p).offset(-(1 as isize))).val.tag as i32 & 0xf as i32 == 0 {
        mt = std::ptr::null_mut();
    } else {
        mt = &mut (*((*((*state).top.p).offset(-(1 as isize))).val.value.gc as *mut GCUnion)).h;
    }
    match (*obj).tag as i32 & 0xf as i32 {
        5 => {
            let ref mut fresh6 = (*((*obj).value.gc as *mut GCUnion)).h.metatable;
            *fresh6 = mt;
            if !mt.is_null() {
                if (*(*obj).value.gc).marked as i32 & 1 << 5 != 0
                    && (*mt).marked as i32 & (1 << 3 | 1 << 4) != 0
                {
                    luac_barrier_(
                        state,
                        &mut (*((*obj).value.gc as *mut GCUnion)).gc,
                        &mut (*(mt as *mut GCUnion)).gc,
                    );
                } else {
                };
                luac_checkfinalizer(state, (*obj).value.gc, mt);
            }
        }
        7 => {
            let ref mut fresh7 = (*((*obj).value.gc as *mut GCUnion)).u.metatable;
            *fresh7 = mt;
            if !mt.is_null() {
                if (*((*obj).value.gc as *mut GCUnion)).u.marked as i32 & 1 << 5 != 0
                    && (*mt).marked as i32 & (1 << 3 | 1 << 4) != 0
                {
                    luac_barrier_(
                        state,
        ...
        _ => {
            (*(*state).global).mt[((*obj).tag as i32 & 0xf as i32) as usize] = mt;
        }
    }
    (*state).top.p = ((*state).top.p).offset(-1);
    (*state).top.p;
...
pub unsafe extern "C" fn lua_setiuservalue(state: *mut State, index: i32, n: i32) -> i32 { unsafe {
    let res: i32;
    let o: *mut TValue = index2value(state, index);
    if !((n as u32).wrapping_sub(1 as u32) < (*((*o).value.gc as *mut GCUnion)).u.nuvalue as u32) {
        res = 0;
    } else {
        let io1: *mut TValue = &mut (*((*((*o).value.gc as *mut GCUnion)).u.uv)
            .as_mut_ptr()
            .offset((n - 1) as isize))
        .uv;
        let io2: *const TValue = &mut (*((*state).top.p).offset(-(1 as isize))).val;
        ...
        (*io1).tag = (*io2).tag;
        if (*((*state).top.p).offset(-(1 as isize))).val.tag as i32 & 1 << 6 != 0 {
            if (*(*o).value.gc).marked as i32 & 1 << 5 != 0
                && (*(*((*state).top.p).offset(-(1 as isize))).val.value.gc).marked as i32
                    & (1 << 3 | 1 << 4)
                    != 0
            {
                luac_barrierback_(state, (*o).value.gc);
            } else {
            };
        } else {
        };
...
pub unsafe extern "C" fn lua_load(
    ...
    if status == 0 {
        let f: *mut LClosure =
            &mut (*((*((*state).top.p).offset(-(1 as isize))).val.value.gc as *mut GCUnion)).lcl;
        if (*f).count_upvalues as i32 >= 1 {
            let gt: *const TValue =
                &mut *((*((*(*state).global).l_registry.value.gc as *mut GCUnion))
                    .h
                    .array)
                    .offset((2 - 1) as isize) as *mut TValue;
            ...
            (*io1).value = (*io2).value;
            (*io1).tag = (*io2).tag;
            if (*gt).tag as i32 & 1 << 6 != 0 {
                if (**((*f).upvalues).as_mut_ptr().offset(0 as isize)).marked as i32
                    & 1 << 5
                    != 0
                    && (*(*gt).value.gc).marked as i32 & (1 << 3 | 1 << 4) != 0
                {
                    luac_barrier_(
                        state,
                        &mut (*(*((*f).upvalues).as_mut_ptr().offset(0 as isize)
                            as *mut GCUnion))
                            .gc,
                        &mut (*((*gt).value.gc as *mut GCUnion)).gc,
                    );
                } else {
                };
            } else {
...
pub unsafe extern "C" fn lua_error(state: *mut State) -> i32 { unsafe {
    ...
    if (*errobj).tag as i32 == 4 | 0 << 4 | 1 << 6
...
pub unsafe extern "C" fn lua_next(state: *mut State, index: i32) -> i32 { unsafe {
    let t: *mut Table = gettable(state, index);
    let more: i32 = luah_next(state, t, ((*state).top.p).offset(-(1 as isize)));
    if more != 0 {
        (*state).top.p = ((*state).top.p).offset(1);
    } else {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn lua_warning(state: *mut State, msg: *const i8, tocont: i32) { unsafe {
    luae_warning(state, msg, tocont);
}}
#[unsafe(no_mangle)]
...
pub unsafe extern "C" fn aux_upvalue(
    ...
    match (*fi).tag as i32 & 0x3f as i32 {
        ...
        6 => {
            let f_0: *mut LClosure = &mut (*((*fi).value.gc as *mut GCUnion)).lcl;
            let p: *mut Prototype = (*f_0).p;
            if !((n as u32).wrapping_sub(1 as u32) < (*p).size_upvalues as u32) {
                return std::ptr::null();
            }
            *val = (**((*f_0).upvalues).as_mut_ptr().offset((n - 1) as isize))
                .v
                .p;
            if !owner.is_null() {
...
pub unsafe extern "C" fn lua_getupvalue(
    ...
    let mut val: *mut TValue = std::ptr::null_mut();
...
pub unsafe extern "C" fn lua_setupvalue(
    ...
    let name: *const i8 = aux_upvalue(fi, n, &mut val, &mut owner);
    if !name.is_null() {
        (*state).top.p = ((*state).top.p).offset(-1);
        (*state).top.p;
        ...
        (*io1).value = (*io2).value;
        (*io1).tag = (*io2).tag;
        if (*val).tag as i32 & 1 << 6 != 0 {
            if (*owner).marked as i32 & 1 << 5 != 0
                && (*(*val).value.gc).marked as i32 & (1 << 3 | 1 << 4) != 0
            {
                luac_barrier_(
                    state,
                    &mut (*(owner as *mut GCUnion)).gc,
                    &mut (*((*val).value.gc as *mut GCUnion)).gc,
                );
            } else {
            };
        } else {
...
pub unsafe extern "C" fn getupvalref(
    ...
    let f: *mut LClosure = &mut (*((*fi).value.gc as *mut GCUnion)).lcl;
    if !pf.is_null() {
        *pf = f;
    }
...
pub unsafe extern "C" fn lua_upvaluejoin(
    ...
    *up1 = *up2;
    if (*f1).marked as i32 & 1 << 5 != 0
        && (**up1).marked as i32 & (1 << 3 | 1 << 4) != 0
    {
        luac_barrier_(
            state,
...
pub unsafe extern "C" fn lua_newthread(state: *mut State) -> *mut State { unsafe {
    ...
    let o: *mut Object = luac_newobjdt(state, 8, ::core::mem::size_of::<LX>() as u64, 8 as u64);
...
pub unsafe extern "C" fn luae_warning(state: *mut State, msg: *const i8, tocont: i32) { unsafe {
    let wf: WarnFunction = (*(*state).global).warnf;
    if wf.is_some() {
        wf.expect("non-null function pointer")((*(*state).global).ud_warn, msg, tocont);
    }
}}
pub unsafe extern "C" fn luae_warnerror(state: *mut State, where_0: *const i8) { unsafe {
    let errobj: *mut TValue = &mut (*((*state).top.p).offset(-(1 as isize))).val;
    let msg: *const i8 = if (*errobj).tag as i32 & 0xf as i32 == 4 {
        ((*((*errobj).value.gc as *mut GCUnion)).ts.contents).as_mut_ptr() as *const i8
    } else {
...
pub unsafe extern "C" fn lua_getlocal(
    ...
    let name;
    if ar.is_null() {
        if !((*((*state).top.p).offset(-(1 as isize))).val.tag as i32
            == 6 | 0 << 4 | 1 << 6)
        {
            name = std::ptr::null();
        } else {
            name = luaf_getlocalname(
                (*((*((*state).top.p).offset(-(1 as isize))).val.value.gc as *mut GCUnion))
                    .lcl
                    .p,
                n,
                0,
        ...
        if !name.is_null() {
            let io1: *mut TValue = &mut (*(*state).top.p).val;
            let io2: *const TValue = &mut (*pos).val;
            (*io1).value = (*io2).value;
            (*io1).tag = (*io2).tag;
            (*state).top.p = ((*state).top.p).offset(1);
            (*state).top.p;
...
pub unsafe extern "C" fn lua_setlocal(
    ...
    if !name.is_null() {
        let io1: *mut TValue = &mut (*pos).val;
        let io2: *const TValue = &mut (*((*state).top.p).offset(-(1 as isize))).val;
        (*io1).value = (*io2).value;
        (*io1).tag = (*io2).tag;
        (*state).top.p = ((*state).top.p).offset(-1);
...
pub unsafe extern "C" fn funcinfo(ar: *mut Debug, cl: *mut UClosure) { unsafe {
    if !(!cl.is_null() && (*cl).c.tag as i32 == 6 | 0 << 4) {
...
pub unsafe extern "C" fn collectvalidlines(state: *mut State, f: *mut UClosure) { unsafe {
    if !(!f.is_null() && (*f).c.tag as i32 == 6 | 0 << 4) {
        ...
        let t: *mut Table = luah_new(state);
        let io: *mut TValue = &mut (*(*state).top.p).val;
        let x_: *mut Table = t;
        (*io).value.gc = &mut (*(x_ as *mut GCUnion)).gc;
        (*io).tag = (5 | 0 << 4 | 1 << 6) as u8;
        ...
        if !((*p).line_info).is_null() {
            ...
            while i < (*p).size_line_info {
                ...
                luah_setint(state, t, currentline as i64, &mut v);
                i += 1;
            }
        }
    };
...
pub unsafe extern "C" fn kname(
    ...
    if (*kvalue).tag as i32 & 0xf as i32 == 4 {
        *name = ((*((*kvalue).value.gc as *mut GCUnion)).ts.contents).as_mut_ptr();
...
pub unsafe extern "C" fn luag_concaterror(
    ...
    if (*p1).tag as i32 & 0xf as i32 == 4 || (*p1).tag as i32 & 0xf as i32 == 3 {
        p1 = p2;
    }
...
pub unsafe extern "C" fn luag_opinterror(
    ...
    if !((*p1).tag as i32 & 0xf as i32 == 3) {
        p2 = p1;
    }
    luag_typeerror(state, p2, msg);
}}
...
pub unsafe extern "C" fn luag_errormsg(state: *mut State) -> ! { unsafe {
    if (*state).error_function != 0 {
        ...
        (*io1).value = (*io2).value;
        (*io1).tag = (*io2).tag;
        let io1_0: *mut TValue = &mut (*((*state).top.p).offset(-(1 as isize))).val;
        let io2_0: *const TValue = &mut (*error_function).val;
        (*io1_0).value = (*io2_0).value;
        (*io1_0).tag = (*io2_0).tag;
        (*state).top.p = ((*state).top.p).offset(1);
        (*state).top.p;
...
pub unsafe extern "C" fn tryagain(
    ...
    let g: *mut Global = (*state).global;
    if (*g).nilvalue.tag as i32 & 0xf as i32 == 0 && (*g).gcstopem == 0 {
        luac_fullgc(state, 1);
...
pub unsafe extern "C" fn luam_malloc_(
    ...
    if size == 0u64 {
        ...
        if ((new_block == std::ptr::null_mut()) as i32 != 0) as i32 as i64 != 0 {
            new_block = tryagain(state, std::ptr::null_mut(), tag as u64, size);
            if new_block.is_null() {
                luad_throw(state, 4);
            }
        }
...
pub unsafe extern "C" fn luao_rawarith(
    ...
    match op {
        7 | 8 | 9 | 10 | 11 | 13 => {
            let mut i1: i64 = 0;
            let mut i2: i64 = 0;
            if (if (((*p1).tag as i32 == 3 | 0 << 4) as i32 != 0) as i32 as i64 != 0 {
                i1 = (*p1).value.i;
                1
            } else {
                luav_tointegerns(p1, &mut i1, F2I::Equal)
            }) != 0
                && (if (((*p2).tag as i32 == 3 | 0 << 4) as i32 != 0) as i32 as i64
                    != 0
                {
                    i2 = (*p2).value.i;
                    1
        ...
        5 | 4 => {
            let mut n1: f64 = 0.0;
            let mut n2: f64 = 0.0;
            if (if (*p1).tag as i32 == 3 | 1 << 4 {
                n1 = (*p1).value.n;
                1
            } else {
                if (*p1).tag as i32 == 3 | 0 << 4 {
                ...
                && (if (*p2).tag as i32 == 3 | 1 << 4 {
                    n2 = (*p2).value.n;
                    1
                } else {
                    if (*p2).tag as i32 == 3 | 0 << 4 {
        ...
        _ => {
                ...
                && (*p2).tag as i32 == 3 | 0 << 4
            {
                ...
                return 1;
            } else if (if (*p1).tag as i32 == 3 | 1 << 4 {
                n1_0 = (*p1).value.n;
                1
            } else {
                if (*p1).tag as i32 == 3 | 0 << 4 {
...
pub unsafe extern "C" fn luao_arith(
    ...
    if luao_rawarith(state, op, p1, p2, &mut (*res).val) == 0 {
        ...
        *buffer.offset((8 - 1) as isize) = x as i8;
        ...
        loop {
            ...
            *buffer.offset((8 - fresh9) as isize) =
                (0x80 as i32 as u64 | x & 0x3f as i32 as u64) as i8;
            x >>= 6;
            mfb >>= 1;
            if !(x > mfb as u64) {
                break;
...
pub unsafe extern "C" fn luat_objtypename(
    ...
    if (*o).tag as i32 == 5 | 0 << 4 | 1 << 6 && {
        mt = (*((*o).value.gc as *mut GCUnion)).h.metatable;
        !mt.is_null()
    } || (*o).tag as i32 == 7 | 0 << 4 | 1 << 6 && {
        mt = (*((*o).value.gc as *mut GCUnion)).u.metatable;
        !mt.is_null()
    } {
        let name: *const TValue =
            luah_getshortstr(mt, luas_new(state, b"__name\0" as *const u8 as *const i8));
        if (*name).tag as i32 & 0xf as i32 == 4 {
            return ((*((*name).value.gc as *mut GCUnion)).ts.contents).as_mut_ptr();
        }
    }
    return TYPE_NAMES[(((*o).tag as i32 & 0xf as i32) + 1) as usize];
}}
pub unsafe extern "C" fn luat_calltm(
    ...
    let io1: *mut TValue = &mut (*function).val;
    let io2: *const TValue = f;
    (*io1).value = (*io2).value;
    (*io1).tag = (*io2).tag;
    let io1_0: *mut TValue = &mut (*function.offset(1 as isize)).val;
    let io2_0: *const TValue = p1;
    (*io1_0).value = (*io2_0).value;
    (*io1_0).tag = (*io2_0).tag;
    let io1_1: *mut TValue = &mut (*function.offset(2 as isize)).val;
    let io2_1: *const TValue = p2;
    (*io1_1).value = (*io2_1).value;
    (*io1_1).tag = (*io2_1).tag;
    let io1_2: *mut TValue = &mut (*function.offset(3 as isize)).val;
...
pub unsafe extern "C" fn luat_calltmres(
    ...
    let io1_2: *mut TValue = &mut (*res).val;
    (*state).top.p = ((*state).top.p).offset(-1);
    let io2_2: *const TValue = &mut (*(*state).top.p).val;
    (*io1_2).value = (*io2_2).value;
    (*io1_2).tag = (*io2_2).tag;
}}
...
pub unsafe extern "C" fn luat_trybintm(
    ...
    if ((callbintm(state, p1, p2, res, event) == 0) as i32 != 0) as i32 as i64 != 0 {
        match event as u32 {
            13 | 14 | 15 | 16 | 17 | 19 => {
                if (*p1).tag as i32 & 0xf as i32 == 3 && (*p2).tag as i32 & 0xf as i32 == 3 {
...
pub unsafe extern "C" fn luat_tryconcattm(state: *mut State) { unsafe {
    ...
    if ((callbintm(
        state,
        &mut (*top.offset(-(2 as isize))).val,
        &mut (*top.offset(-(1 as isize))).val,
...
pub unsafe extern "C" fn luat_callordertm(
    ...
    if callbintm(state, p1, p2, (*state).top.p, event) != 0 {
        return !((*(*state).top.p).val.tag as i32 == 1 | 0 << 4
            || (*(*state).top.p).val.tag as i32 & 0xf as i32 == 0) as i32;
    }
...
pub unsafe extern "C" fn luat_getvarargs(
    ...
    while i < wanted {
        (*where_0.offset(i as isize)).val.tag = (0 | 0 << 4) as u8;
        i += 1;
    }
}}
...
pub unsafe extern "C" fn linkgclist_(
    ...
    *list = o;
    (*o).marked = ((*o).marked as i32
        & !(1 << 5 | (1 << 3 | 1 << 4)) as u8 as i32)
        as u8;
}}
pub unsafe extern "C" fn clearkey(n: *mut Node) { unsafe {
    if (*n).u.key_tag as i32 & 1 << 6 != 0 {
...
pub unsafe extern "C" fn iscleared(g: *mut Global, o: *const Object) -> i32 { unsafe {
    if o.is_null() {
        return 0;
    } else if (*o).tag as i32 & 0xf as i32 == 4 {
        if (*o).marked as i32 & (1 << 3 | 1 << 4) != 0 {
            reallymarkobject(g, &mut (*(o as *mut GCUnion)).gc);
        }
        return 0;
    } else {
        return (*o).marked as i32 & (1 << 3 | 1 << 4);
    };
}}
pub unsafe extern "C" fn luac_barrier_(
    ...
    if (*g).gcstate as i32 <= 2 {
        ...
        if (*o).marked as i32 & 7 > 1 {
            (*v).marked = ((*v).marked as i32 & !(7) | 2) as u8;
        }
...
pub unsafe extern "C" fn luac_barrierback_(state: *mut State, o: *mut Object) { unsafe {
    ...
    if (*o).marked as i32 & 7 > 1 {
        (*o).marked = ((*o).marked as i32 & !(7) | 5) as u8;
    }
}}
pub unsafe extern "C" fn luac_fix(state: *mut State, o: *mut Object) { unsafe {
    ...
    (*o).marked = ((*o).marked as i32
        & !(1 << 5 | (1 << 3 | 1 << 4)) as u8 as i32)
        as u8;
    (*o).marked = ((*o).marked as i32 & !(7) | 4) as u8;
    (*g).allgc = (*o).next;
    (*o).next = (*g).fixedgc;
    (*g).fixedgc = o;
}}
pub unsafe extern "C" fn luac_newobjdt(
    ...
    let g: *mut Global = (*state).global;
    let p: *mut i8 = luam_malloc_(state, size, tag & 0xf as i32) as *mut i8;
    ...
    match (*o).tag as i32 {
        ...
        9 => {
            ...
            if (*uv).v.p != &mut (*uv).u.value as *mut TValue {
                ...
                (*uv).marked = ((*uv).marked as i32 & !(1 << 3 | 1 << 4)
                    | 1 << 5) as u8;
            }
            if (*(*uv).v.p).tag as i32 & 1 << 6 != 0
                && (*(*(*uv).v.p).value.gc).marked as i32 & (1 << 3 | 1 << 4) != 0
            {
        ...
        7 => {
            ...
            if (*u).nuvalue as i32 == 0 {
                if !((*u).metatable).is_null() {
                    if (*(*u).metatable).marked as i32 & (1 << 3 | 1 << 4) != 0 {
                        reallymarkobject(g, &mut (*((*u).metatable as *mut GCUnion)).gc);
                    }
                }
                (*u).marked = ((*u).marked as i32 & !(1 << 3 | 1 << 4)
                    | 1 << 5) as u8;
                current_block_18 = 18317007320854588510;
            } else {
...
pub unsafe extern "C" fn markmt(g: *mut Global) { unsafe {
    ...
    while i < 9 as i32 {
        if !((*g).mt[i as usize]).is_null() {
            if (*(*g).mt[i as usize]).marked as i32 & (1 << 3 | 1 << 4) != 0 {
                reallymarkobject(
                    g,
...
pub unsafe extern "C" fn restartcollection(g: *mut Global) { unsafe {
    cleargraylists(g);
    if (*(*g).mainthread).marked as i32 & (1 << 3 | 1 << 4) != 0 {
        reallymarkobject(g, &mut (*((*g).mainthread as *mut GCUnion)).gc);
    }
    if (*g).l_registry.tag as i32 & 1 << 6 != 0
        && (*(*g).l_registry.value.gc).marked as i32 & (1 << 3 | 1 << 4) != 0
    {
...
pub unsafe extern "C" fn genlink(g: *mut Global, o: *mut Object) { unsafe {
    if (*o).marked as i32 & 7 == 5 {
        linkgclist_(
            &mut (*(o as *mut GCUnion)).gc,
            getgclist(o),
            &mut (*g).grayagain,
        );
    } else if (*o).marked as i32 & 7 == 6 {
        (*o).marked = ((*o).marked as i32 ^ (6 ^ 4)) as u8;
    }
}}
pub unsafe extern "C" fn traverseweakvalue(g: *mut Global, h: *mut Table) { unsafe {
    let limit: *mut Node =
    &mut *((*h).node).offset((1 << (*h).lsizenode as i32) as u64 as isize) as *mut Node;
    let mut hasclears: i32 = ((*h).alimit > 0u32) as i32;
    let mut n: *mut Node = &mut *((*h).node).offset(0 as isize) as *mut Node;
    while n < limit {
        if (*n).i_value.tag as i32 & 0xf as i32 == 0 {
            clearkey(n);
        } else {
            if (*n).u.key_tag as i32 & 1 << 6 != 0
                && (*(*n).u.key_value.gc).marked as i32 & (1 << 3 | 1 << 4) != 0
            {
...
pub unsafe extern "C" fn traverseephemeron(
    ...
    let new_size: u32 = (1 << (*h).lsizenode as i32) as u32;
    let mut i: u32 = 0;
    while i < asize {
        if (*((*h).array).offset(i as isize)).tag as i32 & 1 << 6 != 0
            && (*(*((*h).array).offset(i as isize)).value.gc).marked as i32
                & (1 << 3 | 1 << 4)
                != 0
        {
            marked = 1;
            reallymarkobject(g, (*((*h).array).offset(i as isize)).value.gc);
        }
        i = i.wrapping_add(1);
    }
    i = 0u32;
    while i < new_size {
        ...
        ) != 0
        {
            hasclears = 1;
            if (*n).i_value.tag as i32 & 1 << 6 != 0
                && (*(*n).i_value.value.gc).marked as i32 & (1 << 3 | 1 << 4) != 0
            {
                hasww = 1;
            }
        } else if (*n).i_value.tag as i32 & 1 << 6 != 0
            && (*(*n).i_value.value.gc).marked as i32 & (1 << 3 | 1 << 4) != 0
        {
            marked = 1;
...
pub unsafe extern "C" fn traversetable(g: *mut Global, h: *mut Table) -> u64 { unsafe {
    ...
    if !((*h).metatable).is_null() {
        if (*(*h).metatable).marked as i32 & (1 << 3 | 1 << 4) != 0 {
            reallymarkobject(g, &mut (*((*h).metatable as *mut GCUnion)).gc);
        }
    }
    if !mode.is_null() && (*mode).tag as i32 == 4 | 0 << 4 | 1 << 6 && {
    ...
    return (1 as u32).wrapping_add((*h).alimit).wrapping_add(
        (2
            * (if ((*h).last_free).is_null() {
                0
            } else {
                1 << (*h).lsizenode as i32
            })) as u32,
    ) as u64;
}}
pub unsafe extern "C" fn traverseudata(g: *mut Global, u: *mut Udata) -> i32 { unsafe {
    ...
    while i < (*u).nuvalue as i32 {
        if (*((*u).uv).as_mut_ptr().offset(i as isize)).uv.tag as i32 & 1 << 6 != 0
            && (*(*((*u).uv).as_mut_ptr().offset(i as isize)).uv.value.gc).marked as i32
                & (1 << 3 | 1 << 4)
                != 0
        {
            reallymarkobject(g, (*((*u).uv).as_mut_ptr().offset(i as isize)).uv.value.gc);
        }
        i += 1;
    }
...
pub unsafe extern "C" fn traverseproto(g: *mut Global, f: *mut Prototype) -> i32 { unsafe {
    let mut i: i32;
    if !((*f).source).is_null() {
        if (*(*f).source).marked as i32 & (1 << 3 | 1 << 4) != 0 {
            reallymarkobject(g, &mut (*((*f).source as *mut GCUnion)).gc);
        }
    }
    i = 0;
    while i < (*f).size_k {
        if (*((*f).k).offset(i as isize)).tag as i32 & 1 << 6 != 0
            && (*(*((*f).k).offset(i as isize)).value.gc).marked as i32
                & (1 << 3 | 1 << 4)
                != 0
        {
            reallymarkobject(g, (*((*f).k).offset(i as isize)).value.gc);
        }
        i += 1;
    }
    i = 0;
    ...
    while i < (*f).size_p {
        if !(*((*f).p).offset(i as isize)).is_null() {
            if (**((*f).p).offset(i as isize)).marked as i32 & (1 << 3 | 1 << 4)
                != 0
            {
    ...
    while i < (*f).size_local_variables {
        if !((*((*f).local_variables).offset(i as isize)).varname).is_null() {
            if (*(*((*f).local_variables).offset(i as isize)).varname).marked as i32
                & (1 << 3 | 1 << 4)
                != 0
            {
                reallymarkobject(
...
pub unsafe extern "C" fn traversecclosure(g: *mut Global, cl: *mut CClosure) -> i32 { unsafe {
    ...
    while i < (*cl).count_upvalues as i32 {
        if (*((*cl).upvalue).as_mut_ptr().offset(i as isize)).tag as i32 & 1 << 6 != 0
            && (*(*((*cl).upvalue).as_mut_ptr().offset(i as isize)).value.gc).marked as i32
                & (1 << 3 | 1 << 4)
                != 0
        {
            reallymarkobject(
                g,
                (*((*cl).upvalue).as_mut_ptr().offset(i as isize)).value.gc,
            );
        }
        i += 1;
    }
...
pub unsafe extern "C" fn traverselclosure(g: *mut Global, cl: *mut LClosure) -> i32 { unsafe {
    let mut i: i32;
    if !((*cl).p).is_null() {
        if (*(*cl).p).marked as i32 & (1 << 3 | 1 << 4) != 0 {
...
pub unsafe extern "C" fn traversethread(g: *mut Global, th: *mut State) -> i32 { unsafe {
    ...
    if (*th).marked as i32 & 7 > 1 || (*g).gcstate as i32 == 0 {
        linkgclist_(
    ...
    while o < (*th).top.p {
        if (*o).val.tag as i32 & 1 << 6 != 0
        && (*(*o).val.value.gc).marked as i32 & (1 << 3 | 1 << 4) != 0
        {
...
pub unsafe extern "C" fn convergeephemerons(g: *mut Global) { unsafe {
    ...
    loop {
        ...
        loop {
            ...
            next = (*h).gc_list;
            (*h).marked = ((*h).marked as i32 | 1 << 5) as u8;
...
pub unsafe extern "C" fn clearbykeys(g: *mut Global, mut l: *mut Object) { unsafe {
    while !l.is_null() {
        ...
        let limit: *mut Node = &mut *((*h).node)
            .offset((1 << (*h).lsizenode as i32) as u64 as isize)
            as *mut Node;
        let mut n: *mut Node = &mut *((*h).node).offset(0 as isize) as *mut Node;
        while n < limit {
            if iscleared(
...
pub unsafe extern "C" fn gctm_function(state: *mut State) { unsafe {
    ...
    (*io).value.gc = i_g;
    (*io).tag = ((*i_g).tag as i32 | 1 << 6) as u8;
    ...
    if !((*tm).tag as i32 & 0xf as i32 == 0) {
        ...
        (*state).top.p = ((*state).top.p).offset(1);
        let io1: *mut TValue = &mut (*fresh15).val;
        let io2: *const TValue = tm;
        (*io1).value = (*io2).value;
        (*io1).tag = (*io2).tag;
        let fresh16 = (*state).top.p;
        (*state).top.p = ((*state).top.p).offset(1);
        let io1_0: *mut TValue = &mut (*fresh16).val;
...
pub unsafe extern "C" fn separatetobefnz(g: *mut Global, all: i32) { unsafe {
    ...
    loop {
        ...
        if !((*curr).marked as i32 & (1 << 3 | 1 << 4) != 0 || all != 0) {
            p = &mut (*curr).next;
        } else {
            if curr == (*g).finobjsur {
...
pub unsafe extern "C" fn luac_checkfinalizer(
        ...
        || (*g).gcstp as i32 & 4 != 0
    {
        ...
        *p = (*o).next;
        (*o).next = (*g).finobj;
        (*g).finobj = o;
        (*o).marked = ((*o).marked as i32 | 1 << 6) as u8;
    };
}}
...
pub unsafe extern "C" fn sweep2old(state: *mut State, mut p: *mut *mut Object) { unsafe {
    ...
    loop {
        ...
        if (*curr).marked as i32 & (1 << 3 | 1 << 4) != 0 {
            *p = (*curr).next;
            freeobj(state, curr);
        } else {
            (*curr).marked = ((*curr).marked as i32 & !(7) | 4) as u8;
            if (*curr).tag as i32 == 8 | 0 << 4 {
            ...
            } else if (*curr).tag as i32 == 9 as i32 | 0 << 4
            ...
            *p = (*curr).next;
            ...
            if (*curr).marked as i32 & 7 == 0 {
                ...
                (*curr).marked = ((*curr).marked as i32 & !(7)
                    ...
                    as u8;
                if (*curr).marked as i32 & 7 == 3 && (*pfirstold1).is_null() {
                    *pfirstold1 = curr;
                }
            }
            p = &mut (*curr).next;
...
pub unsafe extern "C" fn markold(g: *mut Global, from: *mut Object, to: *mut Object) { unsafe {
    ...
    while p != to {
        if (*p).marked as i32 & 7 == 3 {
            (*p).marked = ((*p).marked as i32 ^ (3 ^ 4)) as u8;
            if (*p).marked as i32 & 1 << 5 != 0 {
...
pub unsafe extern "C" fn luac_freeallobjects(state: *mut State) { unsafe {
    ...
    deletelist(
        state,
        (*g).allgc,
        &mut (*((*g).mainthread as *mut GCUnion)).gc,
    );
...
pub unsafe extern "C" fn atomic(state: *mut State) -> u64 { unsafe {
    ...
    (*g).grayagain = std::ptr::null_mut();
    (*g).gcstate = 2 as u8;
    if (*state).marked as i32 & (1 << 3 | 1 << 4) != 0 {
...
pub unsafe extern "C" fn callclosemethod(
    ...
    (*io1).value = (*io2).value;
    (*io1).tag = (*io2).tag;
    let io1_0: *mut TValue = &mut (*top.offset(1 as isize)).val;
    let io2_0: *const TValue = obj;
    (*io1_0).value = (*io2_0).value;
    (*io1_0).tag = (*io2_0).tag;
    let io1_1: *mut TValue = &mut (*top.offset(2 as isize)).val;
...
pub unsafe extern "C" fn prepcallclosemth(
    ...
    if status == -1 {
        ...
        errobj = &mut (*level.offset(1 as isize)).val;
...
pub unsafe extern "C" fn luaf_newtbcupval(state: *mut State, level: StkId) { unsafe {
    if (*level).val.tag as i32 == 1 | 0 << 4
        || (*level).val.tag as i32 & 0xf as i32 == 0
    {
        return;
    }
...
pub unsafe extern "C" fn luaf_closeupval(state: *mut State, level: StkId) { unsafe {
    loop {
        ...
        (*uv).v.p = slot;
        if (*uv).marked as i32 & (1 << 3 | 1 << 4) == 0 {
            (*uv).marked = ((*uv).marked as i32 | 1 << 5) as u8;
            if (*slot).tag as i32 & 1 << 6 != 0 {
                if (*uv).marked as i32 & 1 << 5 != 0
                    && (*(*slot).value.gc).marked as i32 & (1 << 3 | 1 << 4) != 0
                {
                    luac_barrier_(
                        state,
                        &mut (*(uv as *mut GCUnion)).gc,
                        &mut (*((*slot).value.gc as *mut GCUnion)).gc,
                    );
                } else {
                };
            } else {
...
pub unsafe extern "C" fn luas_init(state: *mut State) { unsafe {
    ...
    luac_fix(state, &mut (*((*g).memerrmsg as *mut GCUnion)).gc);
    let mut i: i32 = 0;
    while i < 53 as i32 {
        let mut j: i32 = 0;
        while j < 2 {
...
pub unsafe extern "C" fn luas_createlngstrobj(state: *mut State, l: u64) -> *mut TString { unsafe {
    let ts: *mut TString =
        createstrobj(state, l, 4 | 1 << 4, (*(*state).global).seed);
    (*ts).u.long_length = l;
    (*ts).short_length = 0xff as i32 as u8;
    return ts;
}}
...
pub unsafe extern "C" fn internshrstr(
    ...
    ts = createstrobj(state, l, 4 | 0 << 4, h);
...
pub unsafe extern "C" fn luas_newudata(
    ...
    i = 0;
    while i < nuvalue {
        (*((*u).uv).as_mut_ptr().offset(i as isize)).uv.tag = (0 | 0 << 4) as u8;
        i += 1;
    }
    return u;
}}
...
pub unsafe extern "C" fn load_constants(load_state: *mut LoadState, f: *mut Prototype) { unsafe {
    ...
    i = 0;
    while i < n {
        (*((*f).k).offset(i as isize)).tag = (0 | 0 << 4) as u8;
        i += 1;
    }
    i = 0;
    while i < n {
        ...
        match t {
            ...
            4 | 20 => {
                ...
                (*io_1).tag = ((*x_).tag as i32 | 1 << 6) as u8;
            }
            _ => {}
        }
        i += 1;
...
pub unsafe extern "C" fn load_prototypes(load_state: *mut LoadState, f: *mut Prototype) { unsafe {
    ...
    while i < n {
        ...
        *fresh28 = luaf_newproto((*load_state).state);
        if (*f).marked as i32 & 1 << 5 != 0
            && (**((*f).p).offset(i as isize)).marked as i32 & (1 << 3 | 1 << 4)
                != 0
        {
            luac_barrier_(
                (*load_state).state,
...
pub unsafe extern "C" fn check_literal(load_state: *mut LoadState, s: *const i8, msg: *const i8) { unsafe {
...
pub unsafe extern "C" fn dump_byte(dump_state: *mut DumpState, y: i32) { unsafe {
...
pub unsafe extern "C" fn dump_constants(dump_state: *mut DumpState, f: *const Prototype) { unsafe {
    ...
    while i < n {
        ...
        let tag: i32 = (*o).tag as i32 & 0x3f as i32;
        ...
        match tag {
            ...
            4 | 20 => {
                dump_string(dump_state, &mut (*((*o).value.gc as *mut GCUnion)).ts);
            }
            _ => {}
        }
        i += 1;
...
pub unsafe extern "C" fn dump_upvalues(dump_state: *mut DumpState, f: *const Prototype) { unsafe {
    ...
    while i < n {
        ...
        dump_byte(dump_state, (*((*f).upvalues).offset(i as isize)).index as i32);
        dump_byte(dump_state, (*((*f).upvalues).offset(i as isize)).kind as i32);
        i += 1;
    }
}}
...
pub unsafe extern "C" fn dump_function(
    ...
    dump_byte(dump_state, (*f).count_parameters as i32);
    dump_byte(dump_state, (*f).is_variable_arguments as i32);
    dump_byte(dump_state, (*f).maximum_stack_size as i32);
...
pub unsafe extern "C" fn dump_header(dump_state: *mut DumpState) { unsafe {
    ...
    dump_byte(dump_state, ::core::mem::size_of::<u32>() as u64 as i32);
    dump_byte(dump_state, ::core::mem::size_of::<i64>() as u64 as i32);
    dump_byte(dump_state, ::core::mem::size_of::<f64>() as u64 as i32);
...
pub unsafe extern "C" fn luau_dump(
    ...
    dump_byte(&mut dump_state, (*f).size_upvalues);
    dump_function(&mut dump_state, f, std::ptr::null_mut());
    return dump_state.status;
}}
pub unsafe extern "C" fn error_expected(ls: *mut LexState, token: i32) -> ! { unsafe {
    luax_syntaxerror(
        ls,
        luao_pushfstring(
            (*ls).state,
...
pub unsafe extern "C" fn testnext(ls: *mut LexState, c: i32) -> i32 { unsafe {
    if (*ls).t.token == c {
        luax_next(ls);
        return 1;
    } else {
...
pub unsafe extern "C" fn check(ls: *mut LexState, c: i32) { unsafe {
    if (*ls).t.token != c {
        error_expected(ls, c);
    }
}}
pub unsafe extern "C" fn checknext(ls: *mut LexState, c: i32) { unsafe {
    check(ls, c);
    luax_next(ls);
}}
pub unsafe extern "C" fn check_match(
    ...
    if ((testnext(ls, what) == 0) as i32 != 0) as i32 as i64 != 0 {
...
pub unsafe extern "C" fn str_checkname(ls: *mut LexState) -> *mut TString { unsafe {
    check(ls, TK_NAME as i32);
    let ts: *mut TString = (*ls).t.seminfo.ts;
    luax_next(ls);
    return ts;
}}
...
pub unsafe extern "C" fn codename(ls: *mut LexState, e: *mut ExpressionDescription) { unsafe {
...
pub unsafe extern "C" fn registerlocalvar(
    ...
    *fresh35 = varname;
    ...
    if (*f).marked as i32 & 1 << 5 != 0
        && (*varname).marked as i32 & (1 << 3 | 1 << 4) != 0
    {
        luac_barrier_(
            (*ls).state,
            &mut (*(f as *mut GCUnion)).gc,
...
pub unsafe extern "C" fn new_localvar(ls: *mut LexState, name: *mut TString) -> i32 { unsafe {
    let state: *mut State = (*ls).state;
    let fs: *mut FunctionState = (*ls).fs;
    let dynamic_data: *mut DynamicData = (*ls).dynamic_data;
...
pub unsafe extern "C" fn getlocalvardesc(
    ...
    return &mut *((*(*(*fs).ls).dynamic_data).active_variable.arr)
...
pub unsafe extern "C" fn check_readonly(ls: *mut LexState, e: *mut ExpressionDescription) { unsafe {
    let fs: *mut FunctionState = (*ls).fs;
    let mut varname: *mut TString = std::ptr::null_mut();
    match (*e).k as u32 {
        11 => {
            varname = (*((*(*ls).dynamic_data).active_variable.arr).offset((*e).u.info as isize))
                .vd
                .name;
        }
        9 => {
    ...
    if !varname.is_null() {
        let msg: *const i8 = luao_pushfstring(
            (*ls).state,
...
pub unsafe extern "C" fn adjustlocalvars(ls: *mut LexState, nvars: i32) { unsafe {
    let fs: *mut FunctionState = (*ls).fs;
...
pub unsafe extern "C" fn removevars(fs: *mut FunctionState, tolevel: i32) { unsafe {
    (*(*(*fs).ls).dynamic_data).active_variable.n -= (*fs).count_active_variables as i32 - tolevel;
...
pub unsafe extern "C" fn singlevar(ls: *mut LexState, var: *mut ExpressionDescription) { unsafe {
...
pub unsafe extern "C" fn jumpscopeerror(ls: *mut LexState, gt: *mut LabelDescription) -> ! { unsafe {
    let varname: *const i8 =
        ((*(*getlocalvardesc((*ls).fs, (*gt).count_active_variables as i32))
            .vd
            .name)
            .contents)
            .as_mut_ptr();
...
pub unsafe extern "C" fn solvegoto(
    ...
    luak_patchlist((*ls).fs, (*gt).program_counter, (*label).program_counter);
    i = g;
    while i < (*gl).n - 1 {
...
pub unsafe extern "C" fn createlabel(
    ...
    let fs: *mut FunctionState = (*ls).fs;
    let ll: *mut Labellist = &mut (*(*ls).dynamic_data).label;
...
pub unsafe extern "C" fn undefgoto(ls: *mut LexState, gt: *mut LabelDescription) -> ! { unsafe {
    ...
    {
        msg = b"break outside loop at line %d\0" as *const u8 as *const i8;
        msg = luao_pushfstring((*ls).state, msg, (*gt).line);
    } else {
        msg = b"no visible label '%s' for <goto> at line %d\0" as *const u8 as *const i8;
        msg = luao_pushfstring(
            (*ls).state,
            msg,
            ((*(*gt).name).contents).as_mut_ptr(),
...
pub unsafe extern "C" fn open_func(
    ...
    (*f).source = (*ls).source;
    if (*f).marked as i32 & 1 << 5 != 0
        && (*(*f).source).marked as i32 & (1 << 3 | 1 << 4) != 0
    {
        luac_barrier_(
            (*ls).state,
            &mut (*(f as *mut GCUnion)).gc,
...
pub unsafe extern "C" fn block_follow(ls: *mut LexState, withuntil: i32) -> i32 { unsafe {
    match (*ls).t.token {
...
pub unsafe extern "C" fn statlist(ls: *mut LexState) { unsafe {
...
pub unsafe extern "C" fn fieldsel(ls: *mut LexState, v: *mut ExpressionDescription) { unsafe {
    let fs: *mut FunctionState = (*ls).fs;
    let mut key: ExpressionDescription = ExpressionDescription {
        k: VVOID,
        u: RawValue { ival: 0 },
...
pub unsafe extern "C" fn yindex(ls: *mut LexState, v: *mut ExpressionDescription) { unsafe {
    luax_next(ls);
    expr(ls, v);
    luak_exp2val((*ls).fs, v);
    checknext(ls, ']' as i32);
}}
pub unsafe extern "C" fn recfield(ls: *mut LexState, cc: *mut ConstructorControl) { unsafe {
    let fs: *mut FunctionState = (*ls).fs;
    let reg: i32 = (*(*ls).fs).freereg as i32;
    let mut key: ExpressionDescription = ExpressionDescription {
        k: VVOID,
        u: RawValue { ival: 0 },
        t: 0,
...
pub unsafe extern "C" fn listfield(ls: *mut LexState, cc: *mut ConstructorControl) { unsafe {
    expr(ls, &mut (*cc).v);
    (*cc).tostore += 1;
    (*cc).tostore;
}}
pub unsafe extern "C" fn field(ls: *mut LexState, cc: *mut ConstructorControl) { unsafe {
    match (*ls).t.token {
        291 => {
...
pub unsafe extern "C" fn constructor(ls: *mut LexState, t: *mut ExpressionDescription) { unsafe {
    let fs: *mut FunctionState = (*ls).fs;
...
pub unsafe extern "C" fn parlist(ls: *mut LexState) { unsafe {
    let fs: *mut FunctionState = (*ls).fs;
    let f: *mut Prototype = (*fs).f;
    ...
    if (*ls).t.token != ')' as i32 {
        loop {
            match (*ls).t.token {
                ...
                _ => {
                    luax_syntaxerror(ls, b"<name> or '...' expected\0" as *const u8 as *const i8);
                }
            }
            if !(!is_variable_arguments && testnext(ls, ',' as i32) != 0) {
                break;
            }
        }
    }
...
pub unsafe extern "C" fn explist(ls: *mut LexState, v: *mut ExpressionDescription) -> i32 { unsafe {
    let mut n: i32 = 1;
    expr(ls, v);
...
pub unsafe extern "C" fn funcargs(ls: *mut LexState, f: *mut ExpressionDescription) { unsafe {
    let fs: *mut FunctionState = (*ls).fs;
...
pub unsafe extern "C" fn primaryexp(ls: *mut LexState, v: *mut ExpressionDescription) { unsafe {
    match (*ls).t.token {
        40 => {
            let line: i32 = (*ls).linenumber;
            luax_next(ls);
        ...
        _ => {
            luax_syntaxerror(ls, b"unexpected symbol\0" as *const u8 as *const i8);
        }
    };
}}
pub unsafe extern "C" fn suffixedexp(ls: *mut LexState, v: *mut ExpressionDescription) { unsafe {
    let fs: *mut FunctionState = (*ls).fs;
    primaryexp(ls, v);
    loop {
        match (*ls).t.token {
...
pub unsafe extern "C" fn simpleexp(ls: *mut LexState, v: *mut ExpressionDescription) { unsafe {
    match (*ls).t.token {
        TK_FLT => {
            init_exp(v, VKFLT, 0);
...
pub unsafe extern "C" fn subexpr(
    ...
    let mut op: u32 = getbinopr((*ls).t.token);
    while op as u32 != OPR_NOBINOPR as i32 as u32 && PRIORITY[op as usize].left as i32 > limit {
        ...
        let line_0: i32 = (*ls).linenumber;
        luax_next(ls);
        luak_infix((*ls).fs, op, v);
        let nextop: u32 = subexpr(ls, &mut v2, PRIORITY[op as usize].right as i32);
        luak_posfix((*ls).fs, op, v, &mut v2, line_0);
        op = nextop;
    }
    (*(*ls).state).count_c_calls = ((*(*ls).state).count_c_calls).wrapping_sub(1);
    (*(*ls).state).count_c_calls;
    return op;
}}
pub unsafe extern "C" fn expr(ls: *mut LexState, v: *mut ExpressionDescription) { unsafe {
    subexpr(ls, v, 0);
}}
...
pub unsafe extern "C" fn restassign(ls: *mut LexState, lh: *mut LHSAssign, nvars: i32) { unsafe {
    ...
    if testnext(ls, ',' as i32) != 0 {
        ...
        checknext(ls, '=' as i32);
        let nexps: i32 = explist(ls, &mut e);
        if nexps != nvars {
    ...
    init_exp(&mut e, VNONRELOC, (*(*ls).fs).freereg as i32 - 1);
    luak_storevar((*ls).fs, &mut (*lh).v, &mut e);
}}
pub unsafe extern "C" fn cond(ls: *mut LexState) -> i32 { unsafe {
...
pub unsafe extern "C" fn gotostat(ls: *mut LexState) { unsafe {
    let fs: *mut FunctionState = (*ls).fs;
    let line: i32 = (*ls).linenumber;
    let name: *mut TString = str_checkname(ls);
    let lb: *mut LabelDescription = findlabel(ls, name);
    if lb.is_null() {
...
pub unsafe extern "C" fn breakstat(ls: *mut LexState) { unsafe {
    let line: i32 = (*ls).linenumber;
    luax_next(ls);
    newgotoentry(
        ls,
...
pub unsafe extern "C" fn checkrepeated(ls: *mut LexState, name: *mut TString) { unsafe {
    let lb: *mut LabelDescription = findlabel(ls, name);
...
pub unsafe extern "C" fn labelstat(ls: *mut LexState, name: *mut TString, line: i32) { unsafe {
...
pub unsafe extern "C" fn repeatstat(ls: *mut LexState, line: i32) { unsafe {
    let fs: *mut FunctionState = (*ls).fs;
...
pub unsafe extern "C" fn exp1(ls: *mut LexState) { unsafe {
    let mut e: ExpressionDescription = ExpressionDescription {
        k: VVOID,
        u: RawValue { ival: 0 },
        t: 0,
...
pub unsafe extern "C" fn fornum(ls: *mut LexState, varname: *mut TString, line: i32) { unsafe {
    let fs: *mut FunctionState = (*ls).fs;
...
pub unsafe extern "C" fn forlist(ls: *mut LexState, indexname: *mut TString) { unsafe {
    let fs: *mut FunctionState = (*ls).fs;
    let mut e: ExpressionDescription = ExpressionDescription {
        k: VVOID,
        u: RawValue { ival: 0 },
...
pub unsafe extern "C" fn forstat(ls: *mut LexState, line: i32) { unsafe {
    ...
    match (*ls).t.token {
        ...
        _ => {
            luax_syntaxerror(ls, b"'=' or 'in' expected\0" as *const u8 as *const i8);
        }
    }
...
pub unsafe extern "C" fn ifstat(ls: *mut LexState, line: i32) { unsafe {
    let fs: *mut FunctionState = (*ls).fs;
...
pub unsafe extern "C" fn localfunc(ls: *mut LexState) { unsafe {
...
pub unsafe extern "C" fn getlocalattribute(ls: *mut LexState) -> i32 { unsafe {
    if testnext(ls, '<' as i32) != 0 {
        let attr: *const i8 = ((*str_checkname(ls)).contents).as_mut_ptr();
        checknext(ls, '>' as i32);
...
pub unsafe extern "C" fn funcname(ls: *mut LexState, v: *mut ExpressionDescription) -> i32 { unsafe {
...
pub unsafe extern "C" fn retstat(ls: *mut LexState) { unsafe {
    let fs: *mut FunctionState = (*ls).fs;
    let mut e: ExpressionDescription = ExpressionDescription {
        k: VVOID,
        u: RawValue { ival: 0 },
...
pub unsafe extern "C" fn statement(ls: *mut LexState) { unsafe {
    let line: i32 = (*ls).linenumber;
    luae_inccstack((*ls).state);
    match (*ls).t.token {
        CHARACTER_SEMICOLON => {
...
pub unsafe extern "C" fn luay_parser(
    ...
    let mut lexstate: LexState = LexState::new();
    ...
    if (*funcstate.f).marked as i32 & 1 << 5 != 0
        && (*(*funcstate.f).source).marked as i32 & (1 << 3 | 1 << 4) != 0
    {
        luac_barrier_(
            state,
...
pub unsafe extern "C" fn luax_init(state: *mut State) { unsafe {
    ...
    luac_fix(state, &mut (*(e as *mut GCUnion)).gc);
    i = 0;
    while i < TK_WHILE as i32 - (127 as i32 * 2 + 1 + 1) + 1 {
        let ts: *mut TString = luas_new(state, TOKENS[i as usize]);
        luac_fix(state, &mut (*(ts as *mut GCUnion)).gc);
        (*ts).extra = (i + 1) as u8;
        i += 1;
    }
}}
pub unsafe extern "C" fn luax_token2str(ls: *mut LexState, token: i32) -> *const i8 { unsafe {
    if token < 127 as i32 * 2 + 1 + 1 {
        if CHARACTER_TYPE[(token + 1) as usize] as i32 & 1 << 2 != 0 {
            return luao_pushfstring((*ls).state, b"'%c'\0" as *const u8 as *const i8, token);
        } else {
            return luao_pushfstring((*ls).state, b"'<\\%d>'\0" as *const u8 as *const i8, token);
        }
    } else {
        let s: *const i8 = TOKENS[(token - (127 as i32 * 2 + 1 + 1)) as usize];
        if token < TK_EOS as i32 {
            return luao_pushfstring((*ls).state, b"'%s'\0" as *const u8 as *const i8, s);
        } else {
            return s;
        }
    };
}}
pub unsafe extern "C" fn text_token(ls: *mut LexState, token: i32) -> *const i8 { unsafe {
    match token {
        291 | 292 | 289 | 290 => {
            save(ls, '\0' as i32);
            return luao_pushfstring(
...
pub unsafe extern "C" fn lexerror(ls: *mut LexState, mut msg: *const i8, token: i32) -> ! { unsafe {
    msg = luag_addinfo((*ls).state, msg, (*ls).source, (*ls).linenumber);
    if token != 0 {
        luao_pushfstring(
            (*ls).state,
    ...
    luad_throw((*ls).state, 3);
}}
pub unsafe extern "C" fn luax_syntaxerror(ls: *mut LexState, msg: *const i8) -> ! { unsafe {
...
pub unsafe extern "C" fn luax_newstring(
    ...
    let o: *const TValue = luah_getstr((*ls).h, ts);
    if !((*o).tag as i32 & 0xf as i32 == 0) {
        ts = &mut (*((*(o as *mut Node)).u.key_value.gc as *mut GCUnion)).ts;
    } else {
        let fresh50 = (*state).top.p;
        (*state).top.p = ((*state).top.p).offset(1);
        let stv: *mut TValue = &mut (*fresh50).val;
...
pub unsafe extern "C" fn inclinenumber(ls: *mut LexState) { unsafe {
    let old: i32 = (*ls).current;
...
pub unsafe extern "C" fn luax_setinput(
    ...
    (*(*ls).buffer).pointer = luam_saferealloc_(
        (*ls).state,
        (*(*ls).buffer).pointer as *mut libc::c_void,
        ((*(*ls).buffer).size).wrapping_mul(::core::mem::size_of::<i8>() as u64),
        (32 as i32 as u64).wrapping_mul(::core::mem::size_of::<i8>() as u64),
    ) as *mut i8;
    (*(*ls).buffer).size = 32 as i32 as u64;
}}
pub unsafe extern "C" fn check_next1(ls: *mut LexState, c: i32) -> i32 { unsafe {
    if (*ls).current == c {
...
pub unsafe extern "C" fn check_next2(ls: *mut LexState, set: *const i8) -> i32 { unsafe {
    if (*ls).current == *set.offset(0 as isize) as i32
        || (*ls).current == *set.offset(1 as isize) as i32
    {
        save(ls, (*ls).current);
...
pub unsafe extern "C" fn read_numeral(
    ...
    if first == '0' as i32 && check_next2(ls, b"xX\0" as *const u8 as *const i8) != 0 {
        expo = b"Pp\0" as *const u8 as *const i8;
    }
    loop {
        if check_next2(ls, expo) != 0 {
            check_next2(ls, b"-+\0" as *const u8 as *const i8);
        } else {
            if !(CHARACTER_TYPE[((*ls).current + 1) as usize] as i32 & 1 << 4 != 0
                || (*ls).current == '.' as i32)
            {
                break;
            }
    ...
    if obj.tag as i32 == 3 | 0 << 4 {
        (*seminfo).i = obj.value.i;
        return TK_INT as i32;
    } else {
        (*seminfo).r = obj.value.n;
        return TK_FLT as i32;
    };
}}
pub unsafe extern "C" fn skip_sep(ls: *mut LexState) -> u64 { unsafe {
    let mut count: u64 = 0;
    let s: i32 = (*ls).current;
    save(ls, (*ls).current);
...
pub unsafe extern "C" fn read_long_string(
    ...
    loop {
        match (*ls).current {
            -1 => {
                let what: *const i8 = if !seminfo.is_null() {
                    b"string\0" as *const u8 as *const i8
                } else {
    ...
    if !seminfo.is_null() {
        (*seminfo).ts = luax_newstring(
            ls,
            ((*(*ls).buffer).pointer).offset(sep as isize),
            ((*(*ls).buffer).length).wrapping_sub((2 as u64).wrapping_mul(sep)),
        );
    }
}}
pub unsafe extern "C" fn esccheck(ls: *mut LexState, c: i32, msg: *const i8) { unsafe {
    if c == 0 {
        if (*ls).current != -1 {
            save(ls, (*ls).current);
...
pub unsafe extern "C" fn gethexa(ls: *mut LexState) -> i32 { unsafe {
    save(ls, (*ls).current);
    ...
    esccheck(
        ls,
        CHARACTER_TYPE[((*ls).current + 1) as usize] as i32 & 1 << 4,
        b"hexadecimal digit expected\0" as *const u8 as *const i8,
    );
    return luao_hexavalue((*ls).current);
}}
pub unsafe extern "C" fn readhexaesc(ls: *mut LexState) -> i32 { unsafe {
    let mut r: i32 = gethexa(ls);
    r = (r << 4) + gethexa(ls);
    (*(*ls).buffer).length =
        ((*(*ls).buffer).length as u64).wrapping_sub(2 as u64) as u64 as u64;
    return r;
}}
pub unsafe extern "C" fn readutf8esc(ls: *mut LexState) -> u64 { unsafe {
    ...
    loop {
        ...
        r = (r << 4).wrapping_add(luao_hexavalue((*ls).current) as u64);
    }
    esccheck(
        ls,
        ((*ls).current == '}' as i32) as i32,
    ...
    (*(*ls).buffer).length = ((*(*ls).buffer).length as u64).wrapping_sub(i as u64) as u64 as u64;
    return r;
}}
pub unsafe extern "C" fn utf8esc(ls: *mut LexState) { unsafe {
    let mut buffer: [i8; 8] = [0; 8];
    let mut n: i32 = luao_utf8esc(buffer.as_mut_ptr(), readutf8esc(ls));
    while n > 0 {
        save(ls, buffer[(8 - n) as usize] as i32);
        n -= 1;
    }
}}
pub unsafe extern "C" fn readdecesc(ls: *mut LexState) -> i32 { unsafe {
    let mut i: i32;
    let mut r: i32 = 0;
    i = 0;
    while i < 3 && CHARACTER_TYPE[((*ls).current + 1) as usize] as i32 & 1 << 1 != 0 {
        r = 10 as i32 * r + (*ls).current - '0' as i32;
        save(ls, (*ls).current);
...
pub unsafe extern "C" fn read_string(
            ...
            92 => {
                ...
                match (*ls).current {
                    ...
                    122 => {
                        ...
                        while CHARACTER_TYPE[((*ls).current + 1) as usize] as i32 & 1 << 3
                            != 0
                        {
                            if (*ls).current == '\n' as i32 || (*ls).current == '\r' as i32 {
                                inclinenumber(ls);
                    ...
                    _ => {
                        esccheck(
                            ls,
                            CHARACTER_TYPE[((*ls).current + 1) as usize] as i32 & 1 << 1,
    ...
    (*seminfo).ts = luax_newstring(
        ls,
        ((*(*ls).buffer).pointer).offset(1 as isize),
        ((*(*ls).buffer).length).wrapping_sub(2 as u64),
    );
}}
pub unsafe extern "C" fn llex(ls: *mut LexState, seminfo: *mut SemanticInfo) -> i32 { unsafe {
    (*(*ls).buffer).length = 0;
    loop {
        ...
        match (*ls).current {
            ...
            45 => {
                ...
                if (*ls).current == '[' as i32 {
                    let sep: u64 = skip_sep(ls);
                    (*(*ls).buffer).length = 0;
                    if sep >= 2 as u64 {
                        read_long_string(ls, std::ptr::null_mut(), sep);
                        (*(*ls).buffer).length = 0;
            ...
            46 => {
                ...
                } else if CHARACTER_TYPE[((*ls).current + 1) as usize] as i32 & 1 << 1 == 0
                {
                    return '.' as i32;
                } else {
            ...
            _ => {
                if CHARACTER_TYPE[((*ls).current + 1) as usize] as i32 & 1 << 0 != 0 {
                    loop {
                        ...
                        if !(CHARACTER_TYPE[((*ls).current + 1) as usize] as i32
                            & (1 << 0 | 1 << 1)
                            != 0)
                        {
                            break;
...
pub unsafe extern "C" fn luax_next(ls: *mut LexState) { unsafe {
...
pub unsafe extern "C" fn luax_lookahead(ls: *mut LexState) -> i32 { unsafe {
...
pub unsafe extern "C" fn hashint(t: *const Table, i: i64) -> *mut Node { unsafe {
    ...
    if ui <= 2147483647 as i32 as u32 as u64 {
        return &mut *((*t).node)
            .offset((ui as i32 % ((1 << (*t).lsizenode as i32) - 1 | 1)) as isize)
            as *mut Node;
    } else {
        return &mut *((*t).node).offset(
            ui.wrapping_rem(((1 << (*t).lsizenode as i32) - 1 | 1) as u64) as isize,
        ) as *mut Node;
    };
}}
...
pub unsafe extern "C" fn mainpositiontv(t: *const Table, key: *const TValue) -> *mut Node { unsafe {
    match (*key).tag as i32 & 0x3f as i32 {
        3 => {
        ...
        19 => {
            ...
            return &mut *((*t).node).offset(
                ((l_hashfloat as unsafe extern "C" fn(f64) -> i32)(n)
                    % ((1 << (*t).lsizenode as i32) - 1 | 1)) as isize,
            ) as *mut Node;
        }
        4 => {
            let ts: *mut TString = &mut (*((*key).value.gc as *mut GCUnion)).ts;
            return &mut *((*t).node).offset(
                ((*ts).hash & ((1 << (*t).lsizenode as i32) - 1) as u32) as i32 as isize,
            ) as *mut Node;
        }
        20 => {
            let ts_0: *mut TString = &mut (*((*key).value.gc as *mut GCUnion)).ts;
            return &mut *((*t).node).offset(
                ((luas_hashlongstr as unsafe extern "C" fn(*mut TString) -> u32)(ts_0)
                    & ((1 << (*t).lsizenode as i32) - 1) as u32) as i32
                    as isize,
            ) as *mut Node;
        }
        1 => {
            return &mut *((*t).node)
                .offset((0 & (1 << (*t).lsizenode as i32) - 1) as isize)
                as *mut Node;
        }
        17 => {
            return &mut *((*t).node)
                .offset((1 & (1 << (*t).lsizenode as i32) - 1) as isize)
                as *mut Node;
        }
        2 => {
            ...
            return &mut *((*t).node).offset(
                ((p as u64
                    & (2147483647 as i32 as u32)
                        .wrapping_mul(2 as u32)
                        .wrapping_add(1 as u32) as u64) as u32)
                    .wrapping_rem(((1 << (*t).lsizenode as i32) - 1 | 1) as u32)
                    as isize,
            ) as *mut Node;
        }
        22 => {
...
pub unsafe extern "C" fn equalkey(
    ...
    if (*k1).tag as i32 != (*n2).u.key_tag as i32
        && !(deadok != 0
            && (*n2).u.key_tag as i32 == 9 as i32 + 2
            && (*k1).tag as i32 & 1 << 6 != 0)
    {
        return 0;
    }
    match (*n2).u.key_tag as i32 {
        0 | 1 | 17 => return 1,
        3 => return ((*k1).value.i == (*n2).u.key_value.i) as i32,
        19 => return ((*k1).value.n == (*n2).u.key_value.n) as i32,
        2 => return ((*k1).value.p == (*n2).u.key_value.p) as i32,
        22 => return ((*k1).value.f == (*n2).u.key_value.f) as i32,
        84 => {
            return luas_eqlngstr(
                &mut (*((*k1).value.gc as *mut GCUnion)).ts,
                &mut (*((*n2).u.key_value.gc as *mut GCUnion)).ts,
            );
        }
        _ => return ((*k1).value.gc == (*n2).u.key_value.gc) as i32,
    };
}}
pub unsafe extern "C" fn luah_realasize(t: *const Table) -> u32 { unsafe {
    if (*t).flags as i32 & 1 << 7 == 0
        || (*t).alimit & ((*t).alimit).wrapping_sub(1 as u32) == 0u32
    {
        return (*t).alimit;
    } else {
...
pub unsafe extern "C" fn ispow2realasize(t: *const Table) -> i32 { unsafe {
    return ((*t).flags as i32 & 1 << 7 != 0
        || (*t).alimit & ((*t).alimit).wrapping_sub(1 as u32) == 0u32) as i32;
}}
pub unsafe extern "C" fn setlimittosize(t: *mut Table) -> u32 { unsafe {
    (*t).alimit = luah_realasize(t);
    (*t).flags = ((*t).flags as i32 & !(1 << 7) as u8 as i32) as u8;
    return (*t).alimit;
}}
...
pub unsafe extern "C" fn findindex(
    ...
    let mut i: u32;
    if (*key).tag as i32 & 0xf as i32 == 0 {
        return 0u32;
    }
    i = if (*key).tag as i32 == 3 | 0 << 4 {
    ...
    if i.wrapping_sub(1 as u32) < asize {
        ...
        if (((*n).tag as i32 == 0 | (2) << 4) as i32 != 0) as i32 as i64 != 0 {
...
pub unsafe extern "C" fn luah_next(
    ...
    let asize: u32 = luah_realasize(t);
    let mut i: u32 = findindex(state, t, &mut (*key).val, asize);
    while i < asize {
        if !((*((*t).array).offset(i as isize)).tag as i32 & 0xf as i32 == 0) {
    ...
    while (i as i32) < 1 << (*t).lsizenode as i32 {
        if !((*((*t).node).offset(i as isize)).i_value.tag as i32 & 0xf as i32 == 0) {
            let n: *mut Node = &mut *((*t).node).offset(i as isize) as *mut Node;
            let io_: *mut TValue = &mut (*key).val;
            let n_: *const Node = n;
            (*io_).value = (*n_).u.key_value;
            (*io_).tag = (*n_).u.key_tag;
            let io1_0: *mut TValue = &mut (*key.offset(1 as isize)).val;
...
pub unsafe extern "C" fn freehash(state: *mut State, t: *mut Table) { unsafe {
    if !((*t).last_free).is_null() {
        (*state).free_memory(
            (*t).node as *mut libc::c_void,
            ((1 << (*t).lsizenode as i32) as u64)
...
pub unsafe extern "C" fn numusearray(t: *const Table, nums: *mut u32) -> u32 { unsafe {
            ...
            .wrapping_sub(1 as u64) as i32
    {
        ...
        while i <= lim {
            if !((*((*t).array).offset(i.wrapping_sub(1 as u32) as isize)).tag as i32
                & 0xf as i32
                == 0)
            {
...
pub unsafe extern "C" fn numusehash(
    ...
    let mut ause: i32 = 0;
    let mut i: i32 = 1 << (*t).lsizenode as i32;
    loop {
        let fresh131 = i;
        i = i - 1;
        if !(fresh131 != 0) {
...
pub unsafe extern "C" fn setnodevector(state: *mut State, t: *mut Table, mut size: u32) { unsafe {
    if size == 0u32 {
        (*t).node = &DUMMY_NODE as *const Node as *mut Node;
        (*t).lsizenode = 0;
        ...
        (*t).last_free = &mut *((*t).node).offset(size as isize) as *mut Node;
    };
}}
pub unsafe extern "C" fn reinsert(state: *mut State, ot: *mut Table, t: *mut Table) { unsafe {
    let mut j: i32;
    let size: i32 = 1 << (*ot).lsizenode as i32;
    j = 0;
    while j < size {
        let old: *mut Node = &mut *((*ot).node).offset(j as isize) as *mut Node;
        if !((*old).i_value.tag as i32 & 0xf as i32 == 0) {
            let mut k: TValue = TValue {
                value: Value {
                    gc: std::ptr::null_mut(),
                },
            ...
            (*io_).value = (*n_).u.key_value;
            (*io_).tag = (*n_).u.key_tag;
            luah_set(state, t, &mut k, &mut (*old).i_value);
        }
        j += 1;
    }
}}
pub unsafe extern "C" fn luah_resize(
    ...
    while i < new_array_size {
        (*((*t).array).offset(i as isize)).tag = (0 | 1 << 4) as u8;
        i = i.wrapping_add(1);
    }
...
pub unsafe extern "C" fn luah_new(state: *mut State) -> *mut Table { unsafe {
    ...
    (*new_table).alimit = 0u32;
    setnodevector(state, new_table, 0u32);
    return new_table;
}}
pub unsafe extern "C" fn luah_free(state: *mut State, t: *mut Table) { unsafe {
    freehash(state, t);
(*state).free_memory(
        (*t).array as *mut libc::c_void,
        (luah_realasize(t) as u64).wrapping_mul(::core::mem::size_of::<TValue>() as u64),
    );
(*state).free_memory(
...
pub unsafe extern "C" fn luah_newkey(
    ...
    if (((*key).tag as i32 & 0xf as i32 == 0) as i32 != 0) as i32 as i64 != 0 {
        luag_runerror(state, b"table index is nil\0" as *const u8 as *const i8);
    } else if (*key).tag as i32 == 3 | 1 << 4 {
    ...
    if (*value).tag as i32 & 0xf as i32 == 0 {
        return;
    }
    mp = mainpositiontv(t, key);
    if !((*mp).i_value.tag as i32 & 0xf as i32 == 0) || ((*t).last_free).is_null() {
        let mut othern: *mut Node;
        let f_0: *mut Node = (*t).get_free_position();
        if f_0.is_null() {
            rehash(state, t, key);
            luah_set(state, t, key, value);
            return;
        }
    ...
    (*n_).u.key_tag = (*io_).tag;
    if (*key).tag as i32 & 1 << 6 != 0 {
        if (*(t as *mut GCUnion)).gc.marked as i32 & 1 << 5 != 0
            && (*(*key).value.gc).marked as i32 & (1 << 3 | 1 << 4) != 0
        {
            luac_barrierback_(state, &mut (*(t as *mut GCUnion)).gc);
        } else {
        };
    ...
    let io1: *mut TValue = &mut (*mp).i_value;
    let io2: *const TValue = value;
    (*io1).value = (*io2).value;
    (*io1).tag = (*io2).tag;
}}
pub unsafe extern "C" fn luah_getint(t: *mut Table, key: i64) -> *const TValue { unsafe {
    let alimit: u64 = (*t).alimit as u64;
    if (key as u64).wrapping_sub(1 as u32 as u64) < alimit {
        return &mut *((*t).array).offset((key - 1) as isize) as *mut TValue;
    } else if (*t).flags as i32 & 1 << 7 != 0
            ...
            < alimit
    {
        ...
        loop {
            if (*n).u.key_tag as i32 == 3 | 0 << 4 && (*n).u.key_value.i == key {
                return &mut (*n).i_value;
            } else {
                let nx: i32 = (*n).u.next;
                if nx == 0 {
...
pub unsafe extern "C" fn luah_getshortstr(
    ...
    let mut n: *mut Node = &mut *((*t).node)
        .offset(((*key).hash & ((1 << (*t).lsizenode as i32) - 1) as u32) as i32 as isize)
        as *mut Node;
    loop {
...
pub unsafe extern "C" fn luah_set(
    ...
    luah_finishset(state, t, key, slot, value);
}}
...
pub unsafe extern "C" fn hash_search(t: *mut Table, mut j: u64) -> u64 { unsafe {
    let mut i: u64;
    if j == 0u64 {
        j = j.wrapping_add(1);
    }
    loop {
        ...
        if j <= (9223372036854775807 as i64 as u64).wrapping_div(2 as u64) {
            ...
            if (*luah_getint(t, j as i64)).tag as i32 & 0xf as i32 == 0 {
                break;
            }
        } else {
    ...
    while j.wrapping_sub(i) > 1 as u32 as u64 {
        ...
        if (*luah_getint(t, m as i64)).tag as i32 & 0xf as i32 == 0 {
            j = m;
        } else {
            i = m;
        }
...
pub unsafe extern "C" fn binsearch(array: *const TValue, mut i: u32, mut j: u32) -> u32 { unsafe {
    while j.wrapping_sub(i) > 1 as u32 {
        ...
        if (*array.offset(m.wrapping_sub(1 as u32) as isize)).tag as i32 & 0xf as i32 == 0 {
            j = m;
        } else {
            i = m;
        }
...
pub unsafe extern "C" fn luah_getn(t: *mut Table) -> u64 { unsafe {
    let mut limit: u32 = (*t).alimit;
    if limit > 0u32
        && (*((*t).array).offset(limit.wrapping_sub(1 as u32) as isize)).tag as i32 & 0xf as i32
            == 0
    {
        if limit >= 2 as u32
            && !((*((*t).array).offset(limit.wrapping_sub(2 as u32) as isize)).tag as i32
                & 0xf as i32
                == 0)
        {
            if ispow2realasize(t) != 0
                && !(limit.wrapping_sub(1 as u32)
                    ...
                    == 0u32)
            {
                (*t).alimit = limit.wrapping_sub(1 as u32);
                (*t).flags = ((*t).flags as i32 | 1 << 7) as u8;
            }
            return limit.wrapping_sub(1 as u32) as u64;
        } else {
            let boundary: u32 = binsearch((*t).array, 0u32, limit);
            if ispow2realasize(t) != 0 && boundary > (luah_realasize(t)).wrapping_div(2 as u32) {
                (*t).alimit = boundary;
                (*t).flags = ((*t).flags as i32 | 1 << 7) as u8;
            }
    ...
    if !((*t).flags as i32 & 1 << 7 == 0
        || (*t).alimit & ((*t).alimit).wrapping_sub(1 as u32) == 0u32)
    {
        if (*((*t).array).offset(limit as isize)).tag as i32 & 0xf as i32 == 0 {
            return limit as u64;
        }
        limit = luah_realasize(t);
        if (*((*t).array).offset(limit.wrapping_sub(1 as u32) as isize)).tag as i32 & 0xf as i32
            == 0
        {
            let boundary_0: u32 = binsearch((*t).array, (*t).alimit, limit);
            (*t).alimit = boundary_0;
            return boundary_0 as u64;
        }
    }
    if ((*t).last_free).is_null()
        || (*luah_getint(t, limit.wrapping_add(1 as u32) as i64)).tag as i32 & 0xf as i32 == 0
    {
        return limit as u64;
    } else {
...
pub unsafe extern "C" fn luak_semerror(ls: *mut LexState, msg: *const i8) -> ! { unsafe {
    (*ls).t.token = 0;
...
pub unsafe extern "C" fn const2val(
    ...
    return &mut (*((*(*(*fs).ls).dynamic_data).active_variable.arr).offset((*e).u.info as isize))
        .k;
}}
...
pub unsafe extern "C" fn addk(
    ...
    let mut k: i32;
    if (*index).tag as i32 == 3 | 0 << 4 {
        k = (*index).value.i as i32;
        if k < (*fs).nk
            && (*((*f).k).offset(k as isize)).tag as i32 & 0x3f as i32
                == (*v).tag as i32 & 0x3f as i32
    ...
    while old_size < (*f).size_k {
        ...
        old_size = old_size + 1;
        (*((*f).k).offset(fresh135 as isize)).tag = (0 | 0 << 4) as u8;
    }
    ...
    (*fs).nk += 1;
    (*fs).nk;
    if (*v).tag as i32 & 1 << 6 != 0 {
        if (*f).marked as i32 & 1 << 5 != 0
            && (*(*v).value.gc).marked as i32 & (1 << 3 | 1 << 4) != 0
        {
            luac_barrier_(
                state,
                &mut (*(f as *mut GCUnion)).gc,
                &mut (*((*v).value.gc as *mut GCUnion)).gc,
            );
        } else {
        };
    } else {
...
pub unsafe extern "C" fn const2exp(v: *mut TValue, e: *mut ExpressionDescription) { unsafe {
    match (*v).tag as i32 & 0x3f as i32 {
        3 => {
            (*e).k = VKINT;
            (*e).u.ival = (*v).value.i;
        }
        ...
        4 | 20 => {
            (*e).k = VKSTR;
            (*e).u.strval = &mut (*((*v).value.gc as *mut GCUnion)).ts;
        }
        _ => {}
    };
}}
...
pub unsafe extern "C" fn is_k_string(
    ...
    return ((*e).k as u32 == VK as i32 as u32
        && !((*e).t != (*e).f)
        && (*e).u.info <= (1 << 8) - 1
        && (*((*(*fs).f).k).offset((*e).u.info as isize)).tag as i32
            == 4 | 0 << 4 | 1 << 6) as i32;
}}
...
pub unsafe extern "C" fn l_strton(obj: *const TValue, result: *mut TValue) -> i32 { unsafe {
    if !((*obj).tag as i32 & 0xf as i32 == 4) {
        return 0;
    } else {
        let st: *mut TString = &mut (*((*obj).value.gc as *mut GCUnion)).ts;
...
pub unsafe extern "C" fn luav_tointegerns(
    ...
    if (*obj).tag as i32 == 3 | 1 << 4 {
        return luav_flttointeger((*obj).value.n, p, mode);
    } else if (*obj).tag as i32 == 3 | 0 << 4 {
        *p = (*obj).value.i;
        return 1;
    } else {
        return 0;
...
pub unsafe extern "C" fn forprep(state: *mut State, ra: StkId) -> i32 { unsafe {
    ...
    if (*pinit).tag as i32 == 3 | 0 << 4 && (*pstep).tag as i32 == 3 | 0 << 4
    {
        ...
        } != 0
        {
            ...
            (*io_3).value.n = init_0;
            (*io_3).tag = (3 | 1 << 4) as u8;
            let io_4: *mut TValue = &mut (*ra.offset(3 as isize)).val;
            (*io_4).value.n = init_0;
            (*io_4).tag = (3 | 1 << 4) as u8;
        }
    }
    return 0;
}}
pub unsafe extern "C" fn floatforloop(ra: StkId) -> i32 { unsafe {
    let step: f64 = (*ra.offset(2 as isize)).val.value.n;
    let limit: f64 = (*ra.offset(1 as isize)).val.value.n;
    let mut index: f64 = (*ra).val.value.n;
    index = index + step;
    if if (0.0) < step {
        (index <= limit) as i32
    } else {
...
pub unsafe extern "C" fn luav_finishget(
    ...
    while loop_0 < 2000 as i32 {
        if slot.is_null() {
            ...
            tm = if ((*((*t).value.gc as *mut GCUnion)).h.metatable).is_null() {
                std::ptr::null()
            } else if (*(*((*t).value.gc as *mut GCUnion)).h.metatable).flags as u32
                ...
                != 0
            {
                ...
                luat_gettm(
                    (*((*t).value.gc as *mut GCUnion)).h.metatable,
                    TM_INDEX,
                    (*(*state).global).tmname[TM_INDEX as i32 as usize],
                )
            };
            if tm.is_null() {
                (*val).val.tag = (0 | 0 << 4) as u8;
                return;
            }
        }
        if (*tm).tag as i32 & 0xf as i32 == 6 {
            luat_calltmres(state, tm, t, key, val);
            return;
        }
        t = tm;
        if if !((*t).tag as i32 == 5 | 0 << 4 | 1 << 6) {
            ...
            slot = luah_get(&mut (*((*t).value.gc as *mut GCUnion)).h, key);
            !((*slot).tag as i32 & 0xf as i32 == 0) as i32
        } != 0
        {
...
pub unsafe extern "C" fn luav_finishset(
    ...
    while loop_0 < 2000 as i32 {
        let tm: *const TValue;
        if !slot.is_null() {
            let h: *mut Table = &mut (*((*t).value.gc as *mut GCUnion)).h;
            ...
            if tm.is_null() {
                ...
                if (*val).tag as i32 & 1 << 6 != 0 {
                    if (*(h as *mut GCUnion)).gc.marked as i32 & 1 << 5 != 0
                        && (*(*val).value.gc).marked as i32 & (1 << 3 | 1 << 4) != 0
                    {
                        luac_barrierback_(state, &mut (*(h as *mut GCUnion)).gc);
                    } else {
                    };
                } else {
                };
        ...
        if (*tm).tag as i32 & 0xf as i32 == 6 {
            luat_calltm(state, tm, t, key, val);
            return;
        }
        t = tm;
        if if !((*t).tag as i32 == 5 | 0 << 4 | 1 << 6) {
...
pub unsafe extern "C" fn ltnum(l: *const TValue, r: *const TValue) -> i32 { unsafe {
    if (*l).tag as i32 == 3 | 0 << 4 {
        let li: i64 = (*l).value.i;
        if (*r).tag as i32 == 3 | 0 << 4 {
...
pub unsafe extern "C" fn lessthanothers(
    ...
    if (*l).tag as i32 & 0xf as i32 == 4 && (*r).tag as i32 & 0xf as i32 == 4 {
        return (l_strcmp(
            &mut (*((*l).value.gc as *mut GCUnion)).ts,
            &mut (*((*r).value.gc as *mut GCUnion)).ts,
        ) < 0) as i32;
    } else {
...
pub unsafe extern "C" fn luav_lessthan(
    ...
    if (*l).tag as i32 & 0xf as i32 == 3 && (*r).tag as i32 & 0xf as i32 == 3 {
        return ltnum(l, r);
    } else {
...
pub unsafe extern "C" fn luav_equalobj(
    ...
    let mut tm: *const TValue;
    if (*t1).tag as i32 & 0x3f as i32 != (*t2).tag as i32 & 0x3f as i32 {
        if (*t1).tag as i32 & 0xf as i32 != (*t2).tag as i32 & 0xf as i32
            || (*t1).tag as i32 & 0xf as i32 != 3
        {
            return 0;
        } else {
            let mut i1: i64 = 0;
    ...
    match (*t1).tag as i32 & 0x3f as i32 {
        0 | 1 | 17 => return 1,
        3 => return ((*t1).value.i == (*t2).value.i) as i32,
        19 => return ((*t1).value.n == (*t2).value.n) as i32,
        2 => return ((*t1).value.p == (*t2).value.p) as i32,
        22 => return ((*t1).value.f == (*t2).value.f) as i32,
        4 => {
            return (&mut (*((*t1).value.gc as *mut GCUnion)).ts as *mut TString
                == &mut (*((*t2).value.gc as *mut GCUnion)).ts as *mut TString)
                as i32;
        }
        20 => {
            return luas_eqlngstr(
                &mut (*((*t1).value.gc as *mut GCUnion)).ts,
                &mut (*((*t2).value.gc as *mut GCUnion)).ts,
            );
        }
        7 => {
            ...
            tm = if ((*((*t1).value.gc as *mut GCUnion)).u.metatable).is_null() {
                std::ptr::null()
            } else if (*(*((*t1).value.gc as *mut GCUnion)).u.metatable).flags as u32
                ...
                != 0
            {
                ...
                luat_gettm(
                    (*((*t1).value.gc as *mut GCUnion)).u.metatable,
                    TM_EQ,
                    (*(*state).global).tmname[TM_EQ as i32 as usize],
                )
            };
            if tm.is_null() {
                tm = if ((*((*t2).value.gc as *mut GCUnion)).u.metatable).is_null() {
                    std::ptr::null()
                } else if (*(*((*t2).value.gc as *mut GCUnion)).u.metatable).flags as u32
                    & (1 as u32) << TM_EQ as i32
                    != 0
                {
                    std::ptr::null()
                    ...
                    luat_gettm(
                        (*((*t2).value.gc as *mut GCUnion)).u.metatable,
                        TM_EQ,
                        (*(*state).global).tmname[TM_EQ as i32 as usize],
                    )
                };
        ...
        5 => {
            if &mut (*((*t1).value.gc as *mut GCUnion)).h as *mut Table
                == &mut (*((*t2).value.gc as *mut GCUnion)).h as *mut Table
            {
                return 1;
            } else if state.is_null() {
                return 0;
            }
            tm = if ((*((*t1).value.gc as *mut GCUnion)).h.metatable).is_null() {
                std::ptr::null()
            } else if (*(*((*t1).value.gc as *mut GCUnion)).h.metatable).flags as u32
                & (1 as u32) << TM_EQ as i32
                != 0
            {
                std::ptr::null()
                ...
                luat_gettm(
                    (*((*t1).value.gc as *mut GCUnion)).h.metatable,
                    TM_EQ,
                    (*(*state).global).tmname[TM_EQ as i32 as usize],
                )
            };
            if tm.is_null() {
                tm = if ((*((*t2).value.gc as *mut GCUnion)).h.metatable).is_null() {
                    std::ptr::null()
                } else if (*(*((*t2).value.gc as *mut GCUnion)).h.metatable).flags as u32
                    & (1 as u32) << TM_EQ as i32
                    != 0
                {
                    std::ptr::null()
                    ...
                    luat_gettm(
                        (*((*t2).value.gc as *mut GCUnion)).h.metatable,
                        TM_EQ,
                        (*(*state).global).tmname[TM_EQ as i32 as usize],
                    )
                };
        ...
        _ => return ((*t1).value.gc == (*t2).value.gc) as i32,
    }
    if tm.is_null() {
        return 0;
    } else {
...
pub unsafe extern "C" fn copy2buff(top: StkId, mut n: i32, buffer: *mut i8) { unsafe {
    ...
    loop {
        let st: *mut TString =
            &mut (*((*top.offset(-(n as isize))).val.value.gc as *mut GCUnion)).ts;
...
pub unsafe extern "C" fn luav_concat(state: *mut State, mut total: i32) { unsafe {
    ...
    loop {
        let top: StkId = (*state).top.p;
        let mut n: i32 = 2;
        if !((*top.offset(-(2 as isize))).val.tag as i32 & 0xf as i32 == 4
            || (*top.offset(-(2 as isize))).val.tag as i32 & 0xf as i32 == 3)
            || !((*top.offset(-(1 as isize))).val.tag as i32 & 0xf as i32 == 4
                || (*top.offset(-(1 as isize))).val.tag as i32 & 0xf as i32 == 3 && {
                    luao_tostring(state, &mut (*top.offset(-(1 as isize))).val);
                    1 != 0
                })
        {
            luat_tryconcattm(state);
        } else if (*top.offset(-(1 as isize))).val.tag as i32
            == 4 | 0 << 4 | 1 << 6
            && (*((*top.offset(-(1 as isize))).val.value.gc as *mut GCUnion))
                ...
                == 0
        {
            ((*top.offset(-(2 as isize))).val.tag as i32 & 0xf as i32 == 4
                || (*top.offset(-(2 as isize))).val.tag as i32 & 0xf as i32 == 3 && {
                    luao_tostring(state, &mut (*top.offset(-(2 as isize))).val);
                    1 != 0
                }) as i32;
        } else if (*top.offset(-(2 as isize))).val.tag as i32
            == 4 | 0 << 4 | 1 << 6
            && (*((*top.offset(-(2 as isize))).val.value.gc as *mut GCUnion))
                ...
                == 0
        {
            let io1: *mut TValue = &mut (*top.offset(-(2 as isize))).val;
            let io2: *const TValue = &mut (*top.offset(-(1 as isize))).val;
            (*io1).value = (*io2).value;
            (*io1).tag = (*io2).tag;
        } else {
            let mut tl: u64 = if (*((*top.offset(-(1 as isize))).val.value.gc as *mut GCUnion))
                .ts
                .short_length as i32
                != 0xff as i32
            {
                ...
                && ((*top.offset(-(n as isize)).offset(-(1 as isize)))
                    ...
                    || (*top.offset(-(n as isize)).offset(-(1 as isize)))
                        ...
                        && {
                            luao_tostring(
                                state,
                                &mut (*top.offset(-(n as isize)).offset(-(1 as isize))).val,
                            );
                            1 != 0
                        })
            {
            ...
            let io: *mut TValue = &mut (*top.offset(-(n as isize))).val;
            let x_: *mut TString = ts;
            (*io).value.gc = &mut (*(x_ as *mut GCUnion)).gc;
            (*io).tag = ((*x_).tag as i32 | 1 << 6) as u8;
        }
...
pub unsafe extern "C" fn luav_objlen(state: *mut State, ra: StkId, rb: *const TValue) { unsafe {
    let tm: *const TValue;
    match (*rb).tag as i32 & 0x3f as i32 {
        5 => {
...
pub unsafe extern "C" fn pushclosure(
    ...
    while i < nup {
        ...
        if (*ncl).marked as i32 & 1 << 5 != 0
            && (**((*ncl).upvalues).as_mut_ptr().offset(i as isize)).marked as i32
                & (1 << 3 | 1 << 4)
                != 0
        {
            luac_barrier_(
                ...
                &mut (*(*((*ncl).upvalues).as_mut_ptr().offset(i as isize) as *mut GCUnion)).gc,
            );
        } else {
        };
        i += 1;
...
pub unsafe extern "C" fn luav_finishop(state: *mut State) { unsafe {
    ...
    match op as u32 {
        ...
        49 | 50 | 52 | 11 | 12 | 13 | 14 | 20 => {
            ...
            .val;
            (*state).top.p = ((*state).top.p).offset(-1);
            let io2_0: *const TValue = &mut (*(*state).top.p).val;
            (*io1_0).value = (*io2_0).value;
            (*io1_0).tag = (*io2_0).tag;
        }
        58 | 59 | 62 | 63 | 64 | 65 | 57 => {
            let res: i32 = !((*((*state).top.p).offset(-(1 as isize))).val.tag as i32
                == 1 | 0 << 4
                || (*((*state).top.p).offset(-(1 as isize))).val.tag as i32 & 0xf as i32 == 0)
                as i32;
            (*state).top.p = ((*state).top.p).offset(-1);
            (*state).top.p;
        ...
        53 => {
            ...
            let io1_1: *mut TValue = &mut (*top.offset(-(2 as isize))).val;
            let io2_1: *const TValue = &mut (*top).val;
            (*io1_1).value = (*io2_1).value;
            (*io1_1).tag = (*io2_1).tag;
            ...
            loop {
                ...
                match (i >> 0 & !(!(0u32) << 7) << 0) as u32 as u32 {
                    ...
                    3 => {
                        ...
                        let io1_0: *mut TValue = &mut (*ra_2).val;
                    ...
                    4 => {
                        ...
                        program_counter = program_counter.offset(1);
                        let io1_1: *mut TValue = &mut (*ra_3).val;
                    ...
                    9 => {
                        ...
                        let b_2: i32 =
                            (i >> 0 + 7 + 8 + 1 & !(!(0u32) << 8) << 0) as i32;
                        let io1_2: *mut TValue = &mut (*ra_8).val;
                    ...
                    10 => {
                        ...
                        (*io1_3).tag = (*io2_3).tag;
                        if (*ra_9).val.tag as i32 & 1 << 6 != 0 {
                            if (*uv).marked as i32 & 1 << 5 != 0
                                && (*(*ra_9).val.value.gc).marked as i32
                                    & (1 << 3 | 1 << 4)
                                    != 0
                            {
                                luac_barrier_(
                                    state,
                                    &mut (*(uv as *mut GCUnion)).gc,
                                    &mut (*((*ra_9).val.value.gc as *mut GCUnion)).gc,
                                );
                            } else {
                            };
                        } else {
                    ...
                    11 => {
                        ...
                        let key: *mut TString = &mut (*((*rc).value.gc as *mut GCUnion)).ts;
                        if if !((*count_upvalues).tag as i32
                            == 5 | 0 << 4 | 1 << 6)
                        {
                            slot = std::ptr::null();
                            0
                            ...
                            slot = luah_getshortstr(
                                &mut (*((*count_upvalues).value.gc as *mut GCUnion)).h,
                                key,
                            );
                            !((*slot).tag as i32 & 0xf as i32 == 0) as i32
                        } != 0
                        {
                            let io1_4: *mut TValue = &mut (*ra_10).val;
                    ...
                    12 => {
                        ...
                        .val;
                        let n: u64;
                        if if (*rc_0).tag as i32 == 3 | 0 << 4 {
                            n = (*rc_0).value.i as u64;
                            if !((*rb_1).tag as i32 == 5 | 0 << 4 | 1 << 6) {
                                slot_0 = std::ptr::null();
                                0
                            } else {
                                slot_0 = if n.wrapping_sub(1 as u32 as u64)
                                    < (*((*rb_1).value.gc as *mut GCUnion)).h.alimit as u64
                                {
                                    &mut *((*((*rb_1).value.gc as *mut GCUnion)).h.array)
                                        .offset(n.wrapping_sub(1 as u64) as isize)
                                        as *mut TValue
                                        as *const TValue
                                } else {
                                    luah_getint(
                                        &mut (*((*rb_1).value.gc as *mut GCUnion)).h,
                                        n as i64,
                                    )
                                };
                                !((*slot_0).tag as i32 & 0xf as i32 == 0) as i32
                            }
                        } else if !((*rb_1).tag as i32 == 5 | 0 << 4 | 1 << 6) {
                            slot_0 = std::ptr::null();
                            0
                        } else {
                            slot_0 = luah_get(&mut (*((*rb_1).value.gc as *mut GCUnion)).h, rc_0);
                            !((*slot_0).tag as i32 & 0xf as i32 == 0) as i32
                        } != 0
                        {
                            let io1_5: *mut TValue = &mut (*ra_11).val;
                    ...
                    13 => {
                        ...
                        let c: i32 = (i >> 0 + 7 + 8 + 1 + 8
                            & !(!(0u32) << 8) << 0)
                            as i32;
                        if if !((*rb_2).tag as i32 == 5 | 0 << 4 | 1 << 6) {
                            slot_1 = std::ptr::null();
                            0
                        } else {
                            slot_1 = if (c as u64).wrapping_sub(1 as u32 as u64)
                                < (*((*rb_2).value.gc as *mut GCUnion)).h.alimit as u64
                            {
                                &mut *((*((*rb_2).value.gc as *mut GCUnion)).h.array)
                                    .offset((c - 1) as isize)
                                    as *mut TValue as *const TValue
                            } else {
                                luah_getint(&mut (*((*rb_2).value.gc as *mut GCUnion)).h, c as i64)
                            };
                            !((*slot_1).tag as i32 & 0xf as i32 == 0) as i32
                        } != 0
                        {
                            let io1_6: *mut TValue = &mut (*ra_12).val;
                    ...
                    14 => {
                        ...
                        let key_1: *mut TString = &mut (*((*rc_1).value.gc as *mut GCUnion)).ts;
                        if if !((*rb_3).tag as i32 == 5 | 0 << 4 | 1 << 6) {
                            slot_2 = std::ptr::null();
                            0
                        } else {
                            slot_2 = luah_getshortstr(
                                &mut (*((*rb_3).value.gc as *mut GCUnion)).h,
                                key_1,
                            );
                            !((*slot_2).tag as i32 & 0xf as i32 == 0) as i32
                        } != 0
                        {
                            let io1_7: *mut TValue = &mut (*ra_13).val;
                    ...
                    15 => {
                        ...
                        let key_2: *mut TString = &mut (*((*rb_4).value.gc as *mut GCUnion)).ts;
                        if if !((*upval_0).tag as i32 == 5 | 0 << 4 | 1 << 6) {
                            slot_3 = std::ptr::null();
                            0
                        } else {
                            slot_3 = luah_getshortstr(
                                &mut (*((*upval_0).value.gc as *mut GCUnion)).h,
                                key_2,
                            );
                            !((*slot_3).tag as i32 & 0xf as i32 == 0) as i32
                        } != 0
                        {
                            ...
                            (*io1_8).tag = (*io2_8).tag;
                            if (*rc_2).tag as i32 & 1 << 6 != 0 {
                                if (*(*upval_0).value.gc).marked as i32 & 1 << 5 != 0
                                    && (*(*rc_2).value.gc).marked as i32
                                        & (1 << 3 | 1 << 4)
                                        != 0
                                {
                                    luac_barrierback_(state, (*upval_0).value.gc);
                                } else {
                                };
                            } else {
                            };
                    ...
                    16 => {
                        ...
                        let n_0: u64;
                        if if (*rb_5).tag as i32 == 3 | 0 << 4 {
                            n_0 = (*rb_5).value.i as u64;
                            if !((*ra_14).val.tag as i32 == 5 | 0 << 4 | 1 << 6)
                            {
                                slot_4 = std::ptr::null();
                                0
                            } else {
                                slot_4 = if n_0.wrapping_sub(1 as u32 as u64)
                                    < (*((*ra_14).val.value.gc as *mut GCUnion)).h.alimit as u64
                                {
                                    &mut *((*((*ra_14).val.value.gc as *mut GCUnion)).h.array)
                                        .offset(n_0.wrapping_sub(1 as u64) as isize)
                                        as *mut TValue
                                        as *const TValue
                                } else {
                                    luah_getint(
                                        &mut (*((*ra_14).val.value.gc as *mut GCUnion)).h,
                                        n_0 as i64,
                                    )
                                };
                                !((*slot_4).tag as i32 & 0xf as i32 == 0) as i32
                            }
                        } else if !((*ra_14).val.tag as i32
                            == 5 | 0 << 4 | 1 << 6)
                        {
                            slot_4 = std::ptr::null();
                            0
                            ...
                            slot_4 =
                                luah_get(&mut (*((*ra_14).val.value.gc as *mut GCUnion)).h, rb_5);
                            !((*slot_4).tag as i32 & 0xf as i32 == 0) as i32
                        } != 0
                        {
                            ...
                            (*io1_9).tag = (*io2_9).tag;
                            if (*rc_3).tag as i32 & 1 << 6 != 0 {
                                if (*(*ra_14).val.value.gc).marked as i32 & 1 << 5 != 0
                                    && (*(*rc_3).value.gc).marked as i32
                                        & (1 << 3 | 1 << 4)
                                        != 0
                                {
                                    luac_barrierback_(state, (*ra_14).val.value.gc);
                                } else {
                                };
                            } else {
                            };
                    ...
                    17 => {
                        ...
                        if if !((*ra_15).val.tag as i32 == 5 | 0 << 4 | 1 << 6) {
                            slot_5 = std::ptr::null();
                            0
                        } else {
                            slot_5 = if (c_0 as u64).wrapping_sub(1 as u32 as u64)
                                < (*((*ra_15).val.value.gc as *mut GCUnion)).h.alimit as u64
                            {
                                &mut *((*((*ra_15).val.value.gc as *mut GCUnion)).h.array)
                                    .offset((c_0 - 1) as isize)
                                    as *mut TValue as *const TValue
                            } else {
                                luah_getint(
                                    &mut (*((*ra_15).val.value.gc as *mut GCUnion)).h,
                                    c_0 as i64,
                                )
                            };
                            !((*slot_5).tag as i32 & 0xf as i32 == 0) as i32
                        } != 0
                        {
                            let io1_10: *mut TValue = slot_5 as *mut TValue;
                            let io2_10: *const TValue = rc_4;
                            (*io1_10).value = (*io2_10).value;
                            (*io1_10).tag = (*io2_10).tag;
                            if (*rc_4).tag as i32 & 1 << 6 != 0 {
                                if (*(*ra_15).val.value.gc).marked as i32 & 1 << 5 != 0
                                    && (*(*rc_4).value.gc).marked as i32
                                        & (1 << 3 | 1 << 4)
                                        != 0
                                {
                                    luac_barrierback_(state, (*ra_15).val.value.gc);
                                } else {
                                };
                            } else {
                            };
                    ...
                    18 => {
                        ...
                        let key_4: *mut TString = &mut (*((*rb_6).value.gc as *mut GCUnion)).ts;
                        if if !((*ra_16).val.tag as i32 == 5 | 0 << 4 | 1 << 6) {
                            slot_6 = std::ptr::null();
                            0
                        } else {
                            slot_6 = luah_getshortstr(
                                &mut (*((*ra_16).val.value.gc as *mut GCUnion)).h,
                                key_4,
                            );
                            !((*slot_6).tag as i32 & 0xf as i32 == 0) as i32
                        } != 0
                        {
                            let io1_11: *mut TValue = slot_6 as *mut TValue;
                            let io2_11: *const TValue = rc_5;
                            (*io1_11).value = (*io2_11).value;
                            (*io1_11).tag = (*io2_11).tag;
                            if (*rc_5).tag as i32 & 1 << 6 != 0 {
                                if (*(*ra_16).val.value.gc).marked as i32 & 1 << 5 != 0
                                    && (*(*rc_5).value.gc).marked as i32
                                        & (1 << 3 | 1 << 4)
                                        != 0
                                {
                                    luac_barrierback_(state, (*ra_16).val.value.gc);
                                } else {
                                };
                            } else {
                            };
                    ...
                    19 => {
                        ...
                        t = luah_new(state);
                        let io_3: *mut TValue = &mut (*ra_17).val;
                        let x_: *mut Table = t;
                        (*io_3).value.gc = &mut (*(x_ as *mut GCUnion)).gc;
                        (*io_3).tag = (5 | 0 << 4 | 1 << 6) as u8;
                        if b_3 != 0 || c_1 != 0 {
                    ...
                    20 => {
                        ...
                        let key_5: *mut TString = &mut (*((*rc_6).value.gc as *mut GCUnion)).ts;
                        let io1_12: *mut TValue = &mut (*ra_18.offset(1 as isize)).val;
                        let io2_12: *const TValue = rb_7;
                        (*io1_12).value = (*io2_12).value;
                        (*io1_12).tag = (*io2_12).tag;
                        if if !((*rb_7).tag as i32 == 5 | 0 << 4 | 1 << 6) {
                            slot_7 = std::ptr::null();
                            0
                        } else {
                            slot_7 =
                                luah_getstr(&mut (*((*rb_7).value.gc as *mut GCUnion)).h, key_5);
                            !((*slot_7).tag as i32 & 0xf as i32 == 0) as i32
                        } != 0
                        {
                            let io1_13: *mut TValue = &mut (*ra_18).val;
                    ...
                    22 => {
                            ...
                            && (*v2).tag as i32 == 3 | 0 << 4
                        {
                            ...
                            (*io_6).tag = (3 | 0 << 4) as u8;
                        } else {
                            let mut n1: f64 = 0.0;
                            let mut n2: f64 = 0.0;
                            if (if (*v1_0).tag as i32 == 3 | 1 << 4 {
                                n1 = (*v1_0).value.n;
                                1
                            } else {
                                if (*v1_0).tag as i32 == 3 | 0 << 4 {
                                ...
                                && (if (*v2).tag as i32 == 3 | 1 << 4 {
                                    n2 = (*v2).value.n;
                                    1
                                } else {
                                    if (*v2).tag as i32 == 3 | 0 << 4 {
                    ...
                    OP_SUBK => {
                        ...
                        if (*v1_1).tag as i32 == 3 | 0 << 4
                            && (*v2_0).tag as i32 == 3 | 0 << 4
                        {
                            ...
                            (*io_8).tag = (3 | 0 << 4) as u8;
                        } else {
                            let mut n1_0: f64 = 0.0;
                            let mut n2_0: f64 = 0.0;
                            if (if (*v1_1).tag as i32 == 3 | 1 << 4 {
                                n1_0 = (*v1_1).value.n;
                                1
                            } else {
                                if (*v1_1).tag as i32 == 3 | 0 << 4 {
                                ...
                                && (if (*v2_0).tag as i32 == 3 | 1 << 4 {
                                    n2_0 = (*v2_0).value.n;
                                    1
                                } else {
                                    if (*v2_0).tag as i32 == 3 | 0 << 4 {
                    ...
                    OP_MULK => {
                        ...
                        if (*v1_2).tag as i32 == 3 | 0 << 4
                            && (*v2_1).tag as i32 == 3 | 0 << 4
                        {
                            ...
                            (*io_10).tag = (3 | 0 << 4) as u8;
                        } else {
                            let mut n1_1: f64 = 0.0;
                            let mut n2_1: f64 = 0.0;
                            if (if (*v1_2).tag as i32 == 3 | 1 << 4 {
                                n1_1 = (*v1_2).value.n;
                                1
                            } else {
                                if (*v1_2).tag as i32 == 3 | 0 << 4 {
                                ...
                                && (if (*v2_1).tag as i32 == 3 | 1 << 4 {
                                    n2_1 = (*v2_1).value.n;
                                    1
                                } else {
                                    if (*v2_1).tag as i32 == 3 | 0 << 4 {
                    ...
                    OP_MODK => {
                        ...
                        if (*v1_3).tag as i32 == 3 | 0 << 4
                            && (*v2_2).tag as i32 == 3 | 0 << 4
                        {
                            ...
                            (*io_12).tag = (3 | 0 << 4) as u8;
                        } else {
                            let mut n1_2: f64 = 0.0;
                            let mut n2_2: f64 = 0.0;
                            if (if (*v1_3).tag as i32 == 3 | 1 << 4 {
                                n1_2 = (*v1_3).value.n;
                                1
                            } else {
                                if (*v1_3).tag as i32 == 3 | 0 << 4 {
                                ...
                                && (if (*v2_2).tag as i32 == 3 | 1 << 4 {
                                    n2_2 = (*v2_2).value.n;
                                    1
                                } else {
                                    if (*v2_2).tag as i32 == 3 | 0 << 4 {
                    ...
                    OP_POWK => {
                        ...
                        if (if (*v1_4).tag as i32 == 3 | 1 << 4 {
                            n1_3 = (*v1_4).value.n;
                            1
                        } else {
                            if (*v1_4).tag as i32 == 3 | 0 << 4 {
                            ...
                            && (if (*v2_3).tag as i32 == 3 | 1 << 4 {
                                n2_3 = (*v2_3).value.n;
                                1
                            } else {
                                if (*v2_3).tag as i32 == 3 | 0 << 4 {
                        ...
                        {
                            program_counter = program_counter.offset(1);
                            let io_14: *mut TValue = &mut (*ra_24).val;
                            (*io_14).value.n = if n2_3 == 2.0 {
                                n1_3 * n1_3
                            } else {
                                n1_3.powf(n2_3)
                            };
                            (*io_14).tag = (3 | 1 << 4) as u8;
                        }
                        continue;
                    }
                    OP_DIVK => {
                        ...
                        if (if (*v1_5).tag as i32 == 3 | 1 << 4 {
                            n1_4 = (*v1_5).value.n;
                            1
                        } else {
                            if (*v1_5).tag as i32 == 3 | 0 << 4 {
                            ...
                            && (if (*v2_4).tag as i32 == 3 | 1 << 4 {
                                n2_4 = (*v2_4).value.n;
                                1
                            } else {
                                if (*v2_4).tag as i32 == 3 | 0 << 4 {
                    ...
                    OP_IDIVK => {
                        ...
                        if (*v1_6).tag as i32 == 3 | 0 << 4
                            && (*v2_5).tag as i32 == 3 | 0 << 4
                        {
                            ...
                            (*io_16).tag = (3 | 0 << 4) as u8;
                        } else {
                            let mut n1_5: f64 = 0.0;
                            let mut n2_5: f64 = 0.0;
                            if (if (*v1_6).tag as i32 == 3 | 1 << 4 {
                                n1_5 = (*v1_6).value.n;
                                1
                            } else {
                                if (*v1_6).tag as i32 == 3 | 0 << 4 {
                                ...
                                && (if (*v2_5).tag as i32 == 3 | 1 << 4 {
                                    n2_5 = (*v2_5).value.n;
                                    1
                                } else {
                                    if (*v2_5).tag as i32 == 3 | 0 << 4 {
                    ...
                    OP_BANDK => {
                        ...
                        if if (((*v1_7).tag as i32 == 3 | 0 << 4) as i32 != 0) as i32
                            as i64
                            != 0
                        {
                            i1_4 = (*v1_7).value.i;
                    ...
                    OP_BORK => {
                        ...
                        if if (((*v1_8).tag as i32 == 3 | 0 << 4) as i32 != 0) as i32
                            as i64
                            != 0
                        {
                            i1_5 = (*v1_8).value.i;
                    ...
                    OP_BXORK => {
                        ...
                        if if (((*v1_9).tag as i32 == 3 | 0 << 4) as i32 != 0) as i32
                            as i64
                            != 0
                        {
                            i1_6 = (*v1_9).value.i;
                    ...
                    32 => {
                        ...
                        let ic: i32 = (i >> 0 + 7 + 8 + 1 + 8
                            ...
                            - ((1 << 8) - 1 >> 1);
                        let mut ib: i64 = 0;
                        if if (((*rb_8).tag as i32 == 3 | 0 << 4) as i32 != 0) as i32
                            as i64
                            != 0
                        {
                            ib = (*rb_8).value.i;
                    ...
                    33 => {
                        ...
                        let ic_0: i32 = (i >> 0 + 7 + 8 + 1 + 8
                            ...
                            - ((1 << 8) - 1 >> 1);
                        let mut ib_0: i64 = 0;
                        if if (((*rb_9).tag as i32 == 3 | 0 << 4) as i32 != 0) as i32
                            as i64
                            != 0
                        {
                            ib_0 = (*rb_9).value.i;
                    ...
                    34 => {
                        ...
                        if (*v1_10).tag as i32 == 3 | 0 << 4
                            && (*v2_9).tag as i32 == 3 | 0 << 4
                        {
                            ...
                            (*io_23).tag = (3 | 0 << 4) as u8;
                        } else {
                            let mut n1_6: f64 = 0.0;
                            let mut n2_6: f64 = 0.0;
                            if (if (*v1_10).tag as i32 == 3 | 1 << 4 {
                                n1_6 = (*v1_10).value.n;
                                1
                            } else {
                                if (*v1_10).tag as i32 == 3 | 0 << 4 {
                                ...
                                && (if (*v2_9).tag as i32 == 3 | 1 << 4 {
                                    n2_6 = (*v2_9).value.n;
                                    1
                                } else {
                                    if (*v2_9).tag as i32 == 3 | 0 << 4 {
                    ...
                    OP_SUB => {
                        ...
                        if (*v1_11).tag as i32 == 3 | 0 << 4
                            && (*v2_10).tag as i32 == 3 | 0 << 4
                        {
                            ...
                            (*io_25).tag = (3 | 0 << 4) as u8;
                        } else {
                            let mut n1_7: f64 = 0.0;
                            let mut n2_7: f64 = 0.0;
                            if (if (*v1_11).tag as i32 == 3 | 1 << 4 {
                                n1_7 = (*v1_11).value.n;
                                1
                            } else {
                                if (*v1_11).tag as i32 == 3 | 0 << 4 {
                                ...
                                && (if (*v2_10).tag as i32 == 3 | 1 << 4 {
                                    n2_7 = (*v2_10).value.n;
                                    1
                                } else {
                                    if (*v2_10).tag as i32 == 3 | 0 << 4 {
                    ...
                    OP_MUL => {
                        ...
                        if (*v1_12).tag as i32 == 3 | 0 << 4
                            && (*v2_11).tag as i32 == 3 | 0 << 4
                        {
                            ...
                            (*io_27).tag = (3 | 0 << 4) as u8;
                        } else {
                            let mut n1_8: f64 = 0.0;
                            let mut n2_8: f64 = 0.0;
                            if (if (*v1_12).tag as i32 == 3 | 1 << 4 {
                                n1_8 = (*v1_12).value.n;
                                1
                            } else {
                                if (*v1_12).tag as i32 == 3 | 0 << 4 {
                                ...
                                && (if (*v2_11).tag as i32 == 3 | 1 << 4 {
                                    n2_8 = (*v2_11).value.n;
                                    1
                                } else {
                                    if (*v2_11).tag as i32 == 3 | 0 << 4 {
                    ...
                    OP_MOD => {
                        ...
                        if (*v1_13).tag as i32 == 3 | 0 << 4
                            && (*v2_12).tag as i32 == 3 | 0 << 4
                        {
                            ...
                            (*io_29).tag = (3 | 0 << 4) as u8;
                        } else {
                            let mut n1_9: f64 = 0.0;
                            let mut n2_9: f64 = 0.0;
                            if (if (*v1_13).tag as i32 == 3 | 1 << 4 {
                                n1_9 = (*v1_13).value.n;
                                1
                            } else {
                                if (*v1_13).tag as i32 == 3 | 0 << 4 {
                                ...
                                && (if (*v2_12).tag as i32 == 3 | 1 << 4 {
                                    n2_9 = (*v2_12).value.n;
                                    1
                                } else {
                                    if (*v2_12).tag as i32 == 3 | 0 << 4 {
                    ...
                    OP_POW => {
                        ...
                        if (if (*v1_14).tag as i32 == 3 | 1 << 4 {
                            n1_10 = (*v1_14).value.n;
                            1
                        } else {
                            if (*v1_14).tag as i32 == 3 | 0 << 4 {
                            ...
                            && (if (*v2_13).tag as i32 == 3 | 1 << 4 {
                                n2_10 = (*v2_13).value.n;
                                1
                            } else {
                                if (*v2_13).tag as i32 == 3 | 0 << 4 {
                        ...
                        {
                            program_counter = program_counter.offset(1);
                            let io_31: *mut TValue = &mut (*ra_36).val;
                            (*io_31).value.n = if n2_10 == 2.0 {
                                n1_10 * n1_10
                            } else {
                                n1_10.powf(n2_10)
                            };
                            (*io_31).tag = (3 | 1 << 4) as u8;
                        }
                        continue;
                    }
                    OP_DIV => {
                        ...
                        if (if (*v1_15).tag as i32 == 3 | 1 << 4 {
                            n1_11 = (*v1_15).value.n;
                            1
                        } else {
                            if (*v1_15).tag as i32 == 3 | 0 << 4 {
                            ...
                            && (if (*v2_14).tag as i32 == 3 | 1 << 4 {
                                n2_11 = (*v2_14).value.n;
                                1
                            } else {
                                if (*v2_14).tag as i32 == 3 | 0 << 4 {
                    ...
                    OP_IDIV => {
                        ...
                        if (*v1_16).tag as i32 == 3 | 0 << 4
                            && (*v2_15).tag as i32 == 3 | 0 << 4
                        {
                            ...
                            (*io_33).tag = (3 | 0 << 4) as u8;
                        } else {
                            let mut n1_12: f64 = 0.0;
                            let mut n2_12: f64 = 0.0;
                            if (if (*v1_16).tag as i32 == 3 | 1 << 4 {
                                n1_12 = (*v1_16).value.n;
                                1
                            } else {
                                if (*v1_16).tag as i32 == 3 | 0 << 4 {
                                ...
                                && (if (*v2_15).tag as i32 == 3 | 1 << 4 {
                                    n2_12 = (*v2_15).value.n;
                                    1
                                } else {
                                    if (*v2_15).tag as i32 == 3 | 0 << 4 {
                    ...
                    OP_BAND => {
                        ...
                        let mut i1_12: i64 = 0;
                        let mut i2_12: i64 = 0;
                        if (if (((*v1_17).tag as i32 == 3 | 0 << 4) as i32 != 0)
                            as i32 as i64
                            != 0
                        {
                            i1_12 = (*v1_17).value.i;
                            ...
                            && (if (((*v2_16).tag as i32 == 3 | 0 << 4) as i32 != 0)
                                as i32 as i64
                                != 0
                            {
                                i2_12 = (*v2_16).value.i;
                    ...
                    OP_BOR => {
                        ...
                        let mut i1_13: i64 = 0;
                        let mut i2_13: i64 = 0;
                        if (if (((*v1_18).tag as i32 == 3 | 0 << 4) as i32 != 0)
                            as i32 as i64
                            != 0
                        {
                            i1_13 = (*v1_18).value.i;
                            ...
                            && (if (((*v2_17).tag as i32 == 3 | 0 << 4) as i32 != 0)
                                as i32 as i64
                                != 0
                            {
                                i2_13 = (*v2_17).value.i;
                    ...
                    OP_BXOR => {
                        ...
                        let mut i1_14: i64 = 0;
                        let mut i2_14: i64 = 0;
                        if (if (((*v1_19).tag as i32 == 3 | 0 << 4) as i32 != 0)
                            as i32 as i64
                            != 0
                        {
                            i1_14 = (*v1_19).value.i;
                            ...
                            && (if (((*v2_18).tag as i32 == 3 | 0 << 4) as i32 != 0)
                                as i32 as i64
                                != 0
                            {
                                i2_14 = (*v2_18).value.i;
                    ...
                    OP_SHR => {
                        ...
                        let mut i1_15: i64 = 0;
                        let mut i2_15: i64 = 0;
                        if (if (((*v1_20).tag as i32 == 3 | 0 << 4) as i32 != 0)
                            as i32 as i64
                            != 0
                        {
                            i1_15 = (*v1_20).value.i;
                            ...
                            && (if (((*v2_19).tag as i32 == 3 | 0 << 4) as i32 != 0)
                                as i32 as i64
                                != 0
                            {
                                i2_15 = (*v2_19).value.i;
                    ...
                    OP_SHL => {
                        ...
                        let mut i1_16: i64 = 0;
                        let mut i2_16: i64 = 0;
                        if (if (((*v1_21).tag as i32 == 3 | 0 << 4) as i32 != 0)
                            as i32 as i64
                            != 0
                        {
                            i1_16 = (*v1_21).value.i;
                            ...
                            && (if (((*v2_20).tag as i32 == 3 | 0 << 4) as i32 != 0)
                                as i32 as i64
                                != 0
                            {
                                i2_16 = (*v2_20).value.i;
                    ...
                    49 => {
                        ...
                        if (*rb_11).tag as i32 == 3 | 0 << 4 {
                            ...
                            nb_0 = (*rb_11).value.n;
                            1
                        } else if (*rb_11).tag as i32 == 3 | 0 << 4 {
                    ...
                    50 => {
                        ...
                        .val;
                        let mut ib_2: i64 = 0;
                        if if (((*rb_12).tag as i32 == 3 | 0 << 4) as i32 != 0) as i32
                            as i64
                            != 0
                        {
                            ib_2 = (*rb_12).value.i;
                    ...
                    51 => {
                        ...
                        .val;
                        if (*rb_13).tag as i32 == 1 | 0 << 4
                            || (*rb_13).tag as i32 & 0xf as i32 == 0
                        {
                            (*ra_49).val.tag = (1 | 1 << 4) as u8;
                        } else {
                            (*ra_49).val.tag = (1 | 0 << 4) as u8;
                        }
                        continue;
                    }
                    OP_LEN => {
                    ...
                    58 => {
                        ...
                        .val;
                        if (*ra_55).val.tag as i32 == 3 | 0 << 4
                            && (*rb_15).tag as i32 == 3 | 0 << 4
                        {
                            let ia: i64 = (*ra_55).val.value.i;
                            let ib_3: i64 = (*rb_15).value.i;
                            cond_1 = (ia < ib_3) as i32;
                        } else if (*ra_55).val.tag as i32 & 0xf as i32 == 3
                            && (*rb_15).tag as i32 & 0xf as i32 == 3
                        {
                    ...
                    OP_LE => {
                        ...
                        .val;
                        if (*ra_56).val.tag as i32 == 3 | 0 << 4
                            && (*rb_16).tag as i32 == 3 | 0 << 4
                        {
                            let ia_0: i64 = (*ra_56).val.value.i;
                            let ib_4: i64 = (*rb_16).value.i;
                            cond_2 = (ia_0 <= ib_4) as i32;
                        } else if (*ra_56).val.tag as i32 & 0xf as i32 == 3
                            && (*rb_16).tag as i32 & 0xf as i32 == 3
                        {
                    ...
                    60 => {
                        ...
                        let cond_3: i32 =
                            luav_equalobj(std::ptr::null_mut(), &mut (*ra_57).val, rb_17);
                    ...
                    61 => {
                        ...
                        let im: i32 =
                            (i >> 0 + 7 + 8 + 1 & !(!(0u32) << 8) << 0) as i32
                                - ((1 << 8) - 1 >> 1);
                        if (*ra_58).val.tag as i32 == 3 | 0 << 4 {
                            cond_4 = ((*ra_58).val.value.i == im as i64) as i32;
                        } else if (*ra_58).val.tag as i32 == 3 | 1 << 4 {
                    ...
                    62 => {
                        ...
                        let im_0: i32 =
                            (i >> 0 + 7 + 8 + 1 & !(!(0u32) << 8) << 0) as i32
                                - ((1 << 8) - 1 >> 1);
                        if (*ra_59).val.tag as i32 == 3 | 0 << 4 {
                            cond_5 = ((*ra_59).val.value.i < im_0 as i64) as i32;
                        } else if (*ra_59).val.tag as i32 == 3 | 1 << 4 {
                    ...
                    OP_LEI => {
                        ...
                        let im_1: i32 =
                            (i >> 0 + 7 + 8 + 1 & !(!(0u32) << 8) << 0) as i32
                                - ((1 << 8) - 1 >> 1);
                        if (*ra_60).val.tag as i32 == 3 | 0 << 4 {
                            cond_6 = ((*ra_60).val.value.i <= im_1 as i64) as i32;
                        } else if (*ra_60).val.tag as i32 == 3 | 1 << 4 {
                    ...
                    64 => {
                        ...
                        let im_2: i32 =
                            (i >> 0 + 7 + 8 + 1 & !(!(0u32) << 8) << 0) as i32
                                - ((1 << 8) - 1 >> 1);
                        if (*ra_61).val.tag as i32 == 3 | 0 << 4 {
                            cond_7 = ((*ra_61).val.value.i > im_2 as i64) as i32;
                        } else if (*ra_61).val.tag as i32 == 3 | 1 << 4 {
                    ...
                    OP_GEI => {
                        ...
                        let im_3: i32 =
                            (i >> 0 + 7 + 8 + 1 & !(!(0u32) << 8) << 0) as i32
                                - ((1 << 8) - 1 >> 1);
                        if (*ra_62).val.tag as i32 == 3 | 0 << 4 {
                            cond_8 = ((*ra_62).val.value.i >= im_3 as i64) as i32;
                        } else if (*ra_62).val.tag as i32 == 3 | 1 << 4 {
                    ...
                    66 => {
                        ...
                        let cond_9: i32 = !((*ra_63).val.tag as i32 == 1 | 0 << 4
                            || (*ra_63).val.tag as i32 & 0xf as i32 == 0)
                            as i32;
                    ...
                    67 => {
                        ...
                        .val;
                        if ((*rb_18).tag as i32 == 1 | 0 << 4
                            || (*rb_18).tag as i32 & 0xf as i32 == 0)
                            as i32
                            == (i >> 0 + 7 + 8 & !(!(0u32) << 1) << 0) as i32
                        {
                            program_counter = program_counter.offset(1);
                        } else {
                            let io1_14: *mut TValue = &mut (*ra_64).val;
                            let io2_14: *const TValue = rb_18;
                            (*io1_14).value = (*io2_14).value;
                            (*io1_14).tag = (*io2_14).tag;
                        ...
                        if ((*state).hook_mask != 0) as i32 as i64 != 0 {
                            ...
                            if nres_0 == 0 {
                                ...
                                let io1_15: *mut TValue =
                                    &mut (*base.offset(-(1 as isize))).val;
                                let io2_15: *const TValue = &mut (*ra_70).val;
                                (*io1_15).value = (*io2_15).value;
                                (*io1_15).tag = (*io2_15).tag;
                                (*state).top.p = base;
                                while ((nres_0 > 1) as i32 != 0) as i32 as i64 != 0 {
                                    let fresh142 = (*state).top.p;
                                    (*state).top.p = ((*state).top.p).offset(1);
                                    (*fresh142).val.tag = (0 | 0 << 4) as u8;
                                    nres_0 -= 1;
                                }
                            }
                        }
                    ...
                    78 => {
                        ...
                        let mut last: u32 = (i >> 0 + 7 + 8 + 1 + 8
                            & !(!(0u32) << 8) << 0)
                            as i32 as u32;
                        let h: *mut Table = &mut (*((*ra_76).val.value.gc as *mut GCUnion)).h;
                        if n_4 == 0 {
                        ...
                        while n_4 > 0 {
                            let val: *mut TValue = &mut (*ra_76.offset(n_4 as isize)).val;
                            ...
                            (*io1_17).tag = (*io2_17).tag;
                            last = last.wrapping_sub(1);
                            if (*val).tag as i32 & 1 << 6 != 0 {
                                if (*(h as *mut GCUnion)).gc.marked as i32 & 1 << 5 != 0
                                    && (*(*val).value.gc).marked as i32
                                        & (1 << 3 | 1 << 4)
                                        != 0
                                {
                                    luac_barrierback_(state, &mut (*(h as *mut GCUnion)).gc);
                ...
                if !((*ra_75.offset(4 as isize)).val.tag as i32 & 0xf as i32 == 0) {
                    let io1_16: *mut TValue = &mut (*ra_75.offset(2 as isize)).val;
                    let io2_16: *const TValue = &mut (*ra_75.offset(4 as isize)).val;
                    (*io1_16).value = (*io2_16).value;
                    (*io1_16).tag = (*io2_16).tag;
                    program_counter = program_counter.offset(
                        -((i >> 0 + 7 + 8 & !(!(0u32) << 8 + 8 + 1) << 0)
                            as i32 as isize),
                    );
...
pub unsafe extern "C" fn lual_typeerror(
    ...
    return lual_argerror(state, arg, msg);
}}
...
pub unsafe extern "C" fn lual_fileresult(
    ...
    if stat != 0 {
        ...
        if !fname.is_null() {
            lua_pushfstring(state, b"%s: %s\0" as *const u8 as *const i8, fname, msg);
        } else {
...
pub unsafe extern "C" fn panic(state: *mut State) -> i32 { unsafe {
    let msg: *const i8 = if lua_type(state, -1) == 4 {
        lua_tolstring(state, -1, std::ptr::null_mut())
    } else {
        b"error object is not a string\0" as *const u8 as *const i8
    };
...
pub unsafe extern "C" fn str_find_aux(state: *mut State, find: i32) -> i32 { unsafe {
    let mut ls: u64 = 0;
    let mut lp: u64 = 0;
    let s: *const i8 = lual_checklstring(state, 1, &mut ls);
    ...
    if find != 0 && (lua_toboolean(state, 4) != 0 || nospecials(p, lp) != 0) {
        let s2: *const i8 = lmemfind(s.offset(init as isize), ls.wrapping_sub(init), p, lp);
        if !s2.is_null() {
        ...
        prepstate(&mut ms, state, s, ls, p, lp);
        loop {
            let res: *const i8;
            reprepstate(&mut ms);
...
pub unsafe extern "C" fn gmatch(state: *mut State) -> i32 { unsafe {
    ...
    gm = lua_newuserdatauv(state, ::core::mem::size_of::<GMatchState>() as u64, 0)
        as *mut GMatchState;
    if init > ls {
        init = ls.wrapping_add(1 as u64);
    }
    prepstate(&mut (*gm).ms, state, s, ls, p, lp);
...
pub unsafe extern "C" fn utf8_decode(
    ...
    if c < 0x80 as i32 as u32 {
        ...
        while c & 0x40 as i32 as u32 != 0 {
            ...
            res = res << 6 | cc & 0x3f as i32 as u32;
            c <<= 1;
        }
...
pub unsafe extern "C" fn db_traceback(state: *mut State) -> i32 { unsafe {
    let mut arg: i32 = 0;
    let other_state: *mut State = getthread(state, &mut arg);
    let msg: *const i8 = lua_tolstring(state, arg + 1, std::ptr::null_mut());
    if msg.is_null() && !(lua_type(state, arg + 1) <= 0) {
        lua_pushvalue(state, arg + 1);
    } else {
        ...
        lual_traceback(state, other_state, msg, level);
    }
    return 1;
}}
...
pub unsafe extern "C" fn l_message(pname: *const i8, msg: *const i8) { unsafe {
    if !pname.is_null() {
...
pub unsafe extern "C" fn report(state: *mut State, status: i32) -> i32 { unsafe {
    if status != 0 {
        let mut msg: *const i8 = lua_tolstring(state, -1, std::ptr::null_mut());
        if msg.is_null() {
            msg = b"(error message not a string)\0" as *const u8 as *const i8;
        }
...
pub unsafe extern "C" fn incomplete(state: *mut State, status: i32) -> i32 { unsafe {
    if status == 3 {
        let mut lmsg: u64 = 0;
        let msg: *const i8 = lua_tolstring(state, -1, &mut lmsg);
        if lmsg
            >= (::core::mem::size_of::<[i8; 6]>() as u64)
                .wrapping_div(::core::mem::size_of::<i8>() as u64)
                .wrapping_sub(1 as u64)
